...
const eslintConfig = defineConfig([
  ...
  globalIgnores([
    ...
    'packages/pptxgenjs/**',
    // Our own @praxis/* packages: lint the source, but skip build output,
    // installed deps, and the vendored JS sources under importer/src1.
    'packages/@praxis/*/dist/**',
    'packages/@praxis/*/node_modules/**',
    'packages/@praxis/importer/src1/**',
  ...
  // Package boundary (machine-enforced): @praxis/renderer is a standalone,
  ...
  // isolation (only `@praxis/dsl` + declared peers external), not by this rule.
  {
    files: ['packages/@praxis/renderer/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        ...
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          ...
          message:
            '@praxis/renderer must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @praxis/dsl and declared peers, and inject host concerns (stores, undo, media resolution, i18n, hotkeys) via props/callbacks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@praxis/renderer must not reference a host-app path (@/…) in a template literal. Depend only on @praxis/dsl and declared peers; inject host concerns via props/callbacks.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @praxis/storage is a standalone,
  // app-agnostic persistence package. Same policy as the @praxis/renderer
  ...
  // It depends only on @praxis/dsl; host wiring (which store persists where)
  // lives in the app, which imports the package, never the reverse.
  {
    files: ['packages/@praxis/storage/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@praxis/storage must not reference a host-app path (@/…). This package authors no `@/…` strings — depend only on @praxis/dsl. The app wires its stores through the package, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@praxis/storage must not reference a host-app path (@/…) in a template literal. Depend only on @praxis/dsl.',
        },
      ],
    },
  },
  // Package boundary (machine-enforced): @praxis/generation is a standalone,
  ...
  // with package.json.
  {
    files: ['packages/@praxis/generation/**/*.{ts,tsx,js,jsx,mjs,cjs}'],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@praxis/generation must not reference a host-app path (@/…). Depend only on @praxis/dsl, Node built-ins, and relative modules.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@praxis/generation must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation may import only from @praxis/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation may re-export only from @praxis/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|(jsonrepair|katex|nanoid|partial-json)(\\/|$)|node:|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation may re-export only from @praxis/dsl, approved leaf runtime dependencies, Node built-ins, or relative modules (./… or ../…).',
        },
        {
          ...
          message:
            '@praxis/generation must use static imports from @praxis/dsl, Node built-ins, or relative modules.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@praxis/generation must not use require().',
        },
      ],
    },
  },
  ...
  {
    files: [
      'packages/@praxis/generation/test/**/*.{ts,tsx,js,jsx,mjs,cjs}',
      'packages/@praxis/generation/vitest.config.ts',
    ],
    rules: {
      'no-restricted-syntax': [
        'error',
        ...AI_SDK_DYNAMIC_IMPORT_BAN,
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            '@praxis/generation tests and test config must not reference a host-app path (@/…). Read app prompt assets as files for parity checks.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            '@praxis/generation tests and test config must not reference a host-app path (@/…) in a template literal.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|@praxis\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation tests and test config may import only @praxis/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|@praxis\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation tests and test config may re-export only @praxis/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|@praxis\\/generation(\\/|$)|node:|vitest(\\/|$)|\\.\\.?\\/).+/]',
          message:
            '@praxis/generation tests and test config may re-export only @praxis/dsl, the package public entry, Node built-ins, Vitest, or relative modules (./… or ../…).',
        },
        {
          selector: 'ImportExpression',
          message: '@praxis/generation tests and test config must use static imports.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message: '@praxis/generation tests and test config must not use require().',
        },
      ],
    },
  },
  ...
  //      on @praxis/dsl (types + the fire-and-forget partition) and relative
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…). It authors no `@/…` strings — depend only on @praxis/dsl and relative siblings, so the exporter can interpret it in pure Node. The app and exporter import it, not the reverse.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/choreography must not reference a host-app path (@/…) in a template literal. Depend only on @praxis/dsl and relative siblings.',
        },
        // Import allowlist (static imports/re-exports): the ONLY permitted
        // sources are `@praxis/dsl`(/subpaths), `zod`, and in-folder relatives
        ...
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may import only from @praxis/dsl, zod, or in-folder relatives (./…). No parent-escape (../…) into the app and no other packages — keep it pure so the exporter runs in plain Node.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @praxis/dsl, zod, or in-folder relatives (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/).+/]',
          message:
            'lib/choreography may re-export only from @praxis/dsl, zod, or in-folder relatives (./…).',
        },
        ...
        // can pull in a render backend at runtime.
        {
          selector: 'ImportExpression',
          message:
            'lib/choreography must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @praxis/dsl, zod, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
    ...
    rules: {
      'no-restricted-syntax': [
        'error',
        {
          selector: 'Literal[value=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…). Live app state enters only through the injected TimingProbe / AssetSource — depend only on @praxis/dsl, zod, ../choreography, and relative siblings, so the compiler runs in pure Node.',
        },
        {
          selector: 'TemplateElement[value.cooked=/^@\\//]',
          message:
            'lib/video-export must not reference a host-app path (@/…) in a template literal. Depend only on @praxis/dsl, zod, ../choreography, and relative siblings.',
        },
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may import only from @praxis/dsl, zod, ../choreography, or in-module children (./…). A ../… into anything but ../choreography escapes the module and is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @praxis/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/choreography(\\/|$)).+/]',
          message:
            'lib/video-export root files may re-export only from @praxis/dsl, zod, ../choreography, or in-module children (./…).',
        },
        {
          selector: 'ImportExpression',
          message:
            'lib/video-export must not use dynamic import() — it bypasses the static import allowlist. Use a top-level import from @praxis/dsl, zod, ../choreography, or a relative sibling.',
        },
        {
          selector: "CallExpression[callee.name='require']",
          message:
      ...
      'no-restricted-syntax': [
        ...
        // `../../…` escape is rejected UNLESS it is exactly `../../choreography`.
        {
          selector:
            'ImportDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may import only from @praxis/dsl, zod, ../../choreography, or in-module relatives (./… or a single ../… that stays inside the module). A ../../ escape into the rest of the app is rejected.',
        },
        {
          selector:
            'ExportNamedDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
          message:
            'lib/video-export passes may re-export only from @praxis/dsl, zod, ../../choreography, or in-module relatives.',
        },
        {
          selector:
            'ExportAllDeclaration > Literal.source[value=/^(?!@praxis\\/dsl(\\/|$)|zod(\\/|$)|\\.\\/|\\.\\.\\/\\.\\.\\/choreography(\\/|$)|\\.\\.\\/(?!\\.\\.\\/)).+/]',
  ...
  // than by an argument:
  //   - packages/@praxis/renderer, packages/@praxis/storage, and
  //     packages/@praxis/generation — the same
  ...
  //     against @praxis/dsl; review showed `void import('ai')` under the renderer
  ...
  //     ImportExpression outright, which subsumes this one.
  {
    ...
    ignores: [
      ...
      'packages/@praxis/renderer/**',
      'packages/@praxis/storage/**',
      'packages/@praxis/generation/**',
    ],
    rules: {
