...
async function seedDatabase(page: import('@playwright/test').Page) {
  await page.addInitScript((settings) => {
    localStorage.setItem('praxis:account:settings-storage', settings);
  }, SETTINGS_STORAGE);
  ...
  await page.evaluate(
    ({ stageId, interactiveId, html, theme }) => {
      return new Promise<void>((resolve, reject) => {
        const request = indexedDB.open('praxis-documents', 1);
        ...
        request.onsuccess = (event) => {
          ...
          localStorage.setItem(
            `praxis:device:editor-current-scene:${stageId}`,
