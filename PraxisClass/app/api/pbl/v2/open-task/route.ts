...
import { analyzeBio, updateProjectAssessment } from '@/lib/pbl/v2/operations/kernel/proficiency';
...
interface OpenTaskRequest {
  ...
  /** Optional learner PROFILE.md + MEMORY.md digest piggybacked from
   *  the chat renderer. Folded into `project.proficiencyAssessment`
   *  as a static bio signal before the Instructor runs. */
  userBio?: string;
}

/** Hard cap on the piggybacked bio digest (matches the client slice). */
const USER_BIO_MAX_CHARS = 4096;
...
export async function POST(req: NextRequest) {
  ...
  // WP-E H2: the learner's own PROFILE/MEMORY digest is a static bio
  // signal. Folded before the quiz snapshot so a measured quiz result
  // still gets the last word on the tier.
  if (body.phase === 'greeting' && typeof body.userBio === 'string') {
    const bioSignal = analyzeBio(body.userBio.slice(0, USER_BIO_MAX_CHARS));
    if (bioSignal) {
      const { transition } = updateProjectAssessment(body.project, bioSignal);
      log.info(
        `User-bio recalibration: tier=${body.project.proficiency} ` +
