...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const vocationalContext = buildVocationalGenerationContext(requirements, outline, allOutlines);
    ...
    const aiCall = async (
      ...
      const personalized = withTeachingMemory(
        systemPrompt,
        withGenerationRequirements(withVocationalGenerationContext(userPrompt, vocationalContext), requirements),
        teachingMemory,
        requirements?.requirement,
      );
      systemPrompt = personalized.system;
      userPrompt = personalized.user;
    ...
    let content = await generateSceneContent(effectiveOutline, aiCall, {
      assignedImages,
            ...
            pblLoopFallback: (input) =>
              generatePBLV2Project(
                ...
                (params, ...rest) => {
                  const personalized = withTeachingMemory(
                    params.system,
                    withGenerationRequirements(withVocationalGenerationContext(params.prompt, vocationalContext), requirements),
                    teachingMemory,
                    requirements?.requirement,
                  );
                  return callLLM(
                    { ...params, system: personalized.system, prompt: personalized.user },
                    ...rest,
                  );
                },
                { logger: log },
    ...
    if (effectiveOutline.type === 'interactive' && 'html' in content) {
      content = await repairInteractiveContent(content, effectiveOutline, aiCall);
    }
    log.info(`Content generated successfully: "${effectiveOutline.title}"`, { elapsedMs: Date.now() - generationStarted });
