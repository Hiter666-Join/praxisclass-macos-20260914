...
import type { AgentInfo } from '@praxis/generation';
import { DEFAULT_LANGUAGE_DIRECTIVE } from '@praxis/generation';
...
import { reviewGenerationPlan } from '@/lib/server/generation-plan-review';
...
// (@praxis/generation outline parser): ignore whitespace-only titles and cap
...
function normalizeTaskEngineOutline(outline: SceneOutline, requirement: string): SceneOutline {
  // Reuse native diagnosis and adaptive project scenes without converting them
  // to slides. Their configs are normalized by the shared outline pipeline.
  if (outline.type === 'quiz' || outline.type === 'pbl') {
...
function sanitizeNonTaskEngineOutline(outline: SceneOutline): SceneOutline {
  ...
  // procedural-skill is gated behind taskEngineMode to protect ordinary PraxisClass generation.
  return {
    ...outline,
    type: 'interactive',
    widgetType: 'diagram',
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const teachingMemory = await loadTeachingMemory(req);
    prompts = { ...prompts, ...withTeachingMemory(prompts.system, prompts.user, teachingMemory) };

    log.info(
      `Generating outlines: "${requirements.requirement.substring(0, 50)}" [model=${modelString}]`,
    );

    // Map goals during the original planning call, rather than paying for a
    // second model to rediscover the same request on every successful plan.
    prompts.system += '\nBefore returning the outline, map every explicit user requirement to the existing page description, keyPoints and acceptance criteria. Preserve named technologies, required examples, quantities, deliverables and interaction behavior. A title alone is not coverage. Keep all active Skill requirements and the existing output schema; use the appropriate page fields to make the intended checks concrete.';
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          if (parsedOutlines.length > 0) {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'review' })}\n\n`));
            const reviewed = await reviewGenerationPlan(parsedOutlines, requirements, async (system, user) => {
              const response = await callLLM({
                model: languageModel, system, prompt: user, maxOutputTokens: modelInfo?.outputWindow,
                maxRetries: 0, abortSignal: req.signal,
              }, 'scene-outlines-review', undefined, thinkingConfig);
              return response.text;
            }, { documentText: pdfText?.substring(0, MAX_PDF_CONTENT_CHARS), researchContext });
            // Replace sequential gen_img_N/gen_vid_N with globally unique IDs
            const uniquifiedOutlines = uniquifyMediaElementIds(reviewed.outlines);
            ...
            const doneEvent = JSON.stringify({
              ...
              taskEngineMode,
              requirementCoverage: reviewed.coverage,
              review: { changedPages: reviewed.changedPages, elapsedMs: reviewed.elapsedMs },
            });
    ...
    return new Response(stream, {
      headers: {
        ...
        'X-Teacher-Memory': teachingMemory.status,
