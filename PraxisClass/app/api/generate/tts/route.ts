...
import { withProviderRequest } from '@/lib/server/provider-request-context';
...
import { XfyunTTSError } from '@/lib/audio/xfyun-tts';

const log = createLogger('TTS API');

export const maxDuration = 30;

export const POST = withProviderRequest(handlePOST);
...
async function handlePOST(req: NextRequest) {
  ...
  try {
    ...
    if (error instanceof XfyunTTSError) {
      return apiError(error.code, error.httpStatus, error.message);
    }
