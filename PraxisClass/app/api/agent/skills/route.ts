...
export async function GET(req: NextRequest) {
  if (!isAgentRuntimeConfigured()) {
    // The shipped library can be browsed without a database or background runner.
    ...
    return NextResponse.json(
      ...
      { headers: { 'X-Agent-Runtime-Ready': 'false', 'Cache-Control': 'no-store' } },
    );
  }
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    responseHeaders.set('X-Agent-Runtime-Ready', 'true');
