...
import { AgentSessionAccessError } from '@praxis/storage';
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    ...
    try {
      let serviceSettings;
      try {
        serviceSettings = await parseRuntimeSettings(body.serviceSettings, req.headers.get('x-praxis-role'));
      } catch (error) {
        return apiError('INVALID_REQUEST', 400, error instanceof Error ? error.message : '服务配置无效');
      }
      const materials = materialIds.length
        ? await bindOwnerMaterialsToSession(id, ownerId, materialIds)
        : [];
      const posted = await withRuntimeServiceSettings(serviceSettings, () => store.postUserMessage(
        id,
        {
          text,
