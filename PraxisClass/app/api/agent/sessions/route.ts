...
export async function POST(req: NextRequest) {
  ...
  let serviceSettings;
  try {
    serviceSettings = await parseRuntimeSettings(body.serviceSettings, req.headers.get('x-praxis-role'), !existingCourse);
  } catch (error) {
    return apiError('INVALID_REQUEST', 400, error instanceof Error ? error.message : '服务配置无效');
  }
  return withRuntimeServiceSettings(serviceSettings, () => withRequestOwnerId(req, async (ownerId, responseHeaders) => {
