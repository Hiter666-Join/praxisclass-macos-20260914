...
import { createMaterialId } from '@praxis/storage';
import type { ConnectableQueryable } from '@praxis/storage/server/reference';
...
export async function POST(req: NextRequest) {
  ...
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    try {
      ...
      // Anonymous owner IDs contain a colon, which is not a Windows directory name.
      // Store the encoded key in the reservation so reads and cleanup use the same path.
      const ossKey = `materials/${encodeURIComponent(ownerId)}/${createdMaterialId}`;
