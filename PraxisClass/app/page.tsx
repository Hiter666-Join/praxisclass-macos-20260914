...
import { ReminderBell } from '@/components/platform/reminder-bell';
...
import { BackToPlatformLink } from '@/components/platform/back-to-platform-link';
...
import type { Slide } from '@praxis/dsl';
...
function HomePage() {
  ...
  const [homeView, setHomeView] = useState<'create' | 'library'>('create');
  ...
  // Knowledge points handed over from /knowledge → 插入到大纲需求.
  const knowledgePointsRef = useRef(false);
  useEffect(() => {
    if (knowledgePointsRef.current) return;
    knowledgePointsRef.current = true;
    try {
      const points = sessionStorage.getItem('praxis.knowledgePoints');
      if (!points) return;
      sessionStorage.removeItem('praxis.knowledgePoints');
      setForm((prev) => ({
        ...prev,
        requirement: prev.requirement ? `${prev.requirement}\n${points}` : points,
      }));
    } catch {
  ...
  const openLibrary = (search = false) => {
    setHomeView('library');
  ...
  useEffect(() => {
    if (homeView !== 'library' || !searchOpen) return;
    const frame = requestAnimationFrame(() => searchInputRef.current?.focus());
    return () => cancelAnimationFrame(frame);
  }, [homeView, searchOpen]);

  return (
    <div data-home-layout="focus" className="home-focus-page">
      <input
        ref={fileInputRef}
        type="file"
        accept=".zip"
      ...
      <nav className="home-focus-rail" aria-label={t('home.focus.navigation')}>
        <BackToPlatformLink compact />
        <div className="mt-5 flex flex-col gap-2">
          {[
            {
              icon: Pencil,
              label: t('home.focus.create'),
              selected: homeView === 'create',
              action: () => {
                setHomeView('create');
            ...
            {
              icon: Search,
              label: t('classroom.searchAriaLabel'),
              selected: false,
              action: () => openLibrary(true),
            },
            {
              icon: BookOpen,
              label: t('home.focus.library'),
              selected: homeView === 'library',
              action: () => openLibrary(),
            },
          ].map(({ icon: Icon, label, selected, action }) => (
            <Tooltip key={label}>
              <TooltipTrigger asChild>
                <button
                  ...
                  onClick={action}
                  className={cn('home-focus-rail-button', selected && 'is-active')}
                >
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              aria-label={t('platform.nav.settings')}
              onClick={() => setSettingsOpen(true)}
              className="home-focus-rail-button mt-auto"
            >
              <Settings className="size-[19px]" />
            </button>
          </TooltipTrigger>
          <TooltipContent side="right">{t('platform.nav.settings')}</TooltipContent>
        </Tooltip>
      </nav>
      <Tabs
        value={homeView}
        onValueChange={(value) => {
          setHomeView(value as 'create' | 'library');
          if (value === 'library') persistRecentOpen(true);
        }}
        className="home-focus-workspace"
      >
        <header className="home-focus-header">
          <div className="home-focus-brand" data-pro-morph="lockup">
            <BrandLogo size="sm" className="sm:text-xl sm:[&_img]:size-9" />
            {workbenchEntryEnabled && <ProBadge active={false} onToggle={enterWorkbench} />}
          </div>
          <div className="home-focus-mobile-home">
            <BackToPlatformLink />
          </div>
          <TabsList className="home-focus-view-switch">
            <TabsTrigger value="create">{t('home.focus.create')}</TabsTrigger>
            <TabsTrigger value="library">{t('home.focus.library')}</TabsTrigger>
          </TabsList>
          <div ref={toolbarRef} className="home-focus-preferences">
            ...
            <button
              ...
              aria-label={t('platform.nav.settings')}
              onClick={() => setSettingsOpen(true)}
              className="home-focus-mobile-settings"
            >
        ...
        <main className="home-focus-main">
          <TabsContent value="create" className="home-focus-create">
            <motion.div
            ...
            >
              <h1 className="home-focus-title">{t('home.focus.title')}</h1>
              <GenerationToolbar
                ...
                composer={{
                  content: (
                    <textarea
                      ref={textareaRef}
                      aria-label={t('home.focus.placeholder')}
                      placeholder={t('home.focus.placeholder')}
                      className="home-focus-textarea"
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            aria-label={t('home.focus.submit')}
                            className="home-focus-send"
                          >
                            {preparingGenerate ? (
                        ...
                        <TooltipContent>{t('home.focus.submit')}</TooltipContent>
                      </Tooltip>
                    </>
                  ),
                  tools: (
                ...
                <div className="home-focus-suggestions">
                  {[
                    ...
                    <button
                      ...
                      key={key}
                      onClick={() => {
                        updateForm('requirement', t('home.focus.prompt' + key));
                        requestAnimationFrame(() => textareaRef.current?.focus());
                      }}
                      className="home-focus-suggestion"
                    >
                      <Icon className="size-4" />
                      <span>{t('home.focus.example' + key)}</span>
                    </button>
                  ))}
                </div>
              )}
          ...
          <TabsContent value="library" className="home-focus-library">
            <h1 className="text-2xl font-medium tracking-tight sm:text-3xl">
              {t('home.focus.library')}
            </h1>
            <p className="mt-3 text-sm text-muted-foreground">{t('home.focus.libraryHint')}</p>
          ...
          import. One stable action surface across root, folder, and empty. */}
            {hydrated && (
              <motion.div
                ...
                className="relative z-10 mt-8 flex w-full flex-col items-center"
              >
              ...
              path appears/disappears (entering vs leaving a folder). */}
                <div className="group flex w-full flex-wrap items-center gap-4 border-b border-border/60 pb-4">
                  <div className="flex w-full min-w-0 flex-wrap items-center gap-3 text-sm text-muted-foreground">
                    <button
                      onClick={() => {
                        if (currentFolderId) setCurrentFolderId(undefined);
                        else persistRecentOpen(!recentOpen);
                      }}
                      className="mr-auto flex min-h-10 items-center gap-2 rounded-lg font-medium text-foreground transition-colors hover:text-primary"
                    >
                    ...
                    <button
                      ...
                      disabled={importing}
                      className="inline-flex min-h-9 items-center gap-2 rounded-lg border border-border/70 px-3 text-xs text-foreground transition-colors hover:bg-muted disabled:opacity-50"
                    >
                      <Upload className="size-3" />
                      <span className="whitespace-nowrap">{t('import.classroom')}</span>
                    </button>
                    {PPTX_IMPORT_ENABLED && (
                      <button
                      ...
                      >
                        ...
                        <span className="whitespace-nowrap">{t('import.pptx')}</span>
                      </button>
                    )}
                                ...
                                folders.map((folder, i) => (
                                  ...
                                  >
                                    <FolderCard
                                      ...
                                      onDropCourse={(stageId) =>
                                        handleMoveCourse(stageId, folder.id)
                                      }
                                    />
                                  </motion.div>
                                ))}
      ...
      <footer className="home-focus-footer">{t('home.slogan')}</footer>
    </div>
  );
}
...
function GreetingBar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div
      ref={containerRef}
      className={cn('relative w-auto', !compact && 'pl-4 pr-2 pt-3.5 pb-1')}
      onKeyDown={(event) => {
        if (event.key === 'Escape' && open) {
          setOpen(false);
          setEditingName(false);
          setAvatarPickerOpen(false);
          containerRef.current?.querySelector<HTMLButtonElement>('[data-profile-trigger]')?.focus();
        }
      }}
    >
      <input
      ...
      {(!open || compact) && (
        <button
          ...
          aria-expanded={open}
          className={cn(
            'group flex items-center gap-2.5 rounded-full border text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring',
            compact ? 'border-transparent p-1' : 'border-border/50 px-2.5 py-1.5',
          )}
        ...
        >
          <div className="shrink-0 relative">
            ...
            <div
              className={cn(
                'absolute -bottom-0.5 -right-0.5 size-3.5 rounded-full bg-white dark:bg-slate-800 border border-border/40 flex items-center justify-center opacity-60 group-hover:opacity-100 transition-opacity',
                compact && 'hidden',
              )}
            >
          ...
          <div className={cn('flex-1 min-w-0', compact && 'sr-only')}>
            <Tooltip>
              <TooltipTrigger asChild>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className={cn(
              'absolute z-50 w-64',
              compact ? 'right-0 top-full mt-2' : 'left-4 top-3.5',
            )}
          >
