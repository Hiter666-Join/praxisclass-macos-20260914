...
} from '@/components/classroom/classroom-states';
...
import { useServerGenerationState } from '@/lib/classroom/use-server-generation-state';
import {
...
export default function ClassroomDetailPage() {
  ...
  const hasUnfinishedCourse = useStageStore(
    (s) => s.stage?.id === classroomId && s.outlines.length > 0 && !s.generationComplete,
  );
  const generationPaused = useStageStore((s) => s.generationStatus === 'paused');
  const serverDraft = useStageStore((s) => s.outlineProducer === 'server-job' && !s.generationComplete);
  useServerGenerationState(classroomId);
  ...
  return (
    <ThemeProvider>
      <MediaStageProvider value={classroomId}>
        <div data-classroom-viewport className="h-dvh flex flex-col overflow-hidden bg-background">
          {!loading && !error && hasUnfinishedCourse && (
            <GenerationNotice paused={generationPaused} serverDraft={serverDraft} />
          )}
          {loading ? (
            <ClassroomLoadingState />
          ) : error ? (
