...
export async function register(): Promise<void> {
  ...
  const { startScheduleTick } = await import('@/lib/platform/schedule/tick');
  const scheduleTick = startScheduleTick();
  ...
  const shutdown = (): Promise<void> => {
    shutdownPromise ??= (async () => {
      ...
      try {
        ...
        console.error('[instrumentation] Platform schedule tick drain failed', error);
      }
      try {
