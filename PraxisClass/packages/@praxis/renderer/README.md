...
React component for rendering PPTist-style `Slide` JSON. Extracted from PraxisClass.

> `@praxis/renderer` is the read-only canvas package. Editing lives in the
> separate `@praxis/editor` package, which depends on this renderer.

## Migrating from 0.0.x

Version `0.1.0` removes the experimental `@praxis/renderer/editing` subpath.
Install `@praxis/editor` and migrate editing imports to its explicit layers:

```ts
import { EditableSlideCanvas } from '@praxis/editor/react';
import { EditableSlideCanvasWithUI } from '@praxis/editor/ui';
```

Read-only rendering imports from `@praxis/renderer` are unchanged.

## Install

```bash
pnpm add @praxis/renderer
# or
npm install @praxis/renderer
```
...
```tsx
import { SlideCanvas, type Slide } from '@praxis/renderer';
...
```tsx
import { SlideRendererProvider, SlideCanvas, useSlideContext } from '@praxis/renderer';
...
### Granular components — `@praxis/renderer/elements`
...
export default {
  content: [
    './src/**/*.{ts,tsx}',
    './node_modules/@praxis/renderer/dist/**/*.{js,cjs}',
  ],
};
```
...
installed on the viewer's machine. The package ships a generated `fonts.css`;
import it once at your app shell when using the self-hosted CJK whitelist:

```ts
import '@praxis/renderer/fonts.css';
```

> 自托管：设置 `RENDERER_FONT_BASE_URL` 并重新生成。
>
...
[`@praxis/importer`](../importer) converts `.pptx` files to the same `Slide[]` shape, so you can do `.pptx → @praxis/renderer` end-to-end.
