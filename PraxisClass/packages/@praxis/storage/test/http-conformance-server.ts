...
function validatePayloadForKind(session: RuntimeSession, payload: unknown): void {
  if (session.kind === 'chat' && !isChatMessageSkeleton(payload)) {
    throw new ConformanceHttpError(
      400,
      'VALIDATION_FAILED',
      '@praxis/storage: invalid runtime record: /payload: chat payload must match ' +
  ...
  if (session.kind === 'quizAttempt' && !isQuizAttemptSkeleton(payload)) {
    throw new ConformanceHttpError(
      400,
      'VALIDATION_FAILED',
      '@praxis/storage: invalid runtime record: /payload: quizAttempt payload must match ' +
  ...
  if (method === 'POST' && parts.length === 2 && parts[1] === 'sessions') {
    ...
    if (await store.getSession(init.id)) {
      throw new ConformanceHttpError(
        ...
        `@praxis/storage: runtime session ${JSON.stringify(init.id)} already exists`,
      );
    }
      ...
      if (session === undefined) {
        sendJson(res, 404, {
          error: {
            ...
            message: `@praxis/storage: no session ${JSON.stringify(sessionId)}`,
          },
        });
      } else {
  ...
  if (parts[1] === 'learners' && parts[2] === 'merge' && parts.length === 3 && method === 'POST') {
    ...
    if (
      ...
      throw new ConformanceHttpError(
        400,
        'VALIDATION_FAILED',
        '@praxis/storage: learner keys must be non-empty strings',
      );
    }
