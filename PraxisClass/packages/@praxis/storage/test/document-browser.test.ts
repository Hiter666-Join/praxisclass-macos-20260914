...
import { DSL_VERSION, DSL_VERSION_KEY, validateScene, validateStage } from '@praxis/dsl';
import {
...
describe('BrowserDocumentStore with an app-widened scene union', () => {
  ...
  const holoDeckDoc: PraxisDocument<HoloDeckScene> = {
  ...
  test('the default store (DSL validator) rejects an app-only scene kind', async () => {
    ...
    await expect(store.saveDocument(holoDeckDoc as unknown as PraxisDocument)).rejects.toThrow();
  });
  ...
  test('enforces the phantom-partition guard even under a permissive validator', async () => {
    ...
    const mismatched: PraxisDocument<HoloDeckScene> = {
