...
import { RUNTIME_DSL_VERSION } from '@praxis/dsl';
import type { RuntimePayload, RuntimeRecord, RuntimeRecordInit } from '@praxis/dsl';
      ...
      new Response(
        JSON.stringify({
          error: {
            code: 'FUTURE_VERSION',
            message:
              '@praxis/storage: session "future" was written at runtime DSL version "99.0.0", newer than this client\'s 1.0.0',
          },
        }),
...
describe('HttpRuntimeStore HTTP hardening', () => {
  test('validates append envelopes before sending a request', async () => {
    ...
    expect((failure as Error).message).toBe(
      `@praxis/storage: invalid runtime record ${JSON.stringify(init.id)}: /createdAt: expected ISO 8601 \`createdAt\``,
    );
