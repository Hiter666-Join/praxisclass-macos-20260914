...
import type { PraxisDocument, StageFreshnessManifest } from '../src/document/types.js';
import {
...
if (process.env.STORAGE_PG_CONTRACT_REQUIRED === '1' && !contractUrl) {
  throw new Error(
    '@praxis/storage: STORAGE_PG_CONTRACT_REQUIRED=1 requires PG_CONTRACT_URL; ' +
...
function makeDoc(stageId: string, scenes: { id: string; order: number }[]): PraxisDocument {
  return {
