  ...
  for await (const chunk of req) {
    ...
    if (total > maxBodyBytes) {
      throw new ConformanceHttpError(
        413,
        'PAYLOAD_TOO_LARGE',
        `@praxis/storage: request body exceeds ${maxBodyBytes} bytes`,
      );
    }
    chunks.push(buffer);
  }
  ...
  for (const part of rawParts) {
    ...
    try {
      ...
      throw new ConformanceHttpError(
        400,
        'VALIDATION_FAILED',
        '@praxis/storage: request path is not valid percent-encoded UTF-8',
      );
    }
    parts.push(decoded);
  }
...
function assertNoScopeChannel(req: IncomingMessage, url: URL, parts: string[]): void {
  ...
  if (parts[1] === 'device' || parts[1] === 'account') {
    throw new ConformanceHttpError(
      400,
      'VALIDATION_FAILED',
      '@praxis/storage: kv requests must not carry a scope path segment — this contract is ' +
  ...
  if (url.searchParams.has('scope')) {
    throw new ConformanceHttpError(
      400,
      'VALIDATION_FAILED',
      '@praxis/storage: kv requests must not carry a scope query parameter — this contract is ' +
        'account-scoped and the principal is derived server-side',
    );
  }
  for (const header of PROHIBITED_SCOPE_HEADERS) {
    if (req.headers[header] !== undefined) {
      throw new ConformanceHttpError(
        400,
        'VALIDATION_FAILED',
        `@praxis/storage: kv requests must not carry a scope header (${header})`,
      );
    }
  }
}
...
async function assertNoRequestBody(req: IncomingMessage, maxBodyBytes: number): Promise<void> {
  ...
  if (body.length > 0) {
    throw new ConformanceHttpError(
      400,
      'VALIDATION_FAILED',
      `@praxis/storage: ${req.method ?? 'GET'} must not carry a request body — this contract is ` +
        'account-scoped and the principal is derived server-side',
    );
  }
}
    ...
    if (method === 'GET') {
      ...
      if (raw === undefined) {
        throw new ConformanceHttpError(
          404,
          'KEY_NOT_FOUND',
          `@praxis/storage: no kv entry ${JSON.stringify(key)}`,
        );
      }
    ...
    if (method === 'PUT') {
      ...
      if (!('value' in body)) {
        throw new ConformanceHttpError(
          400,
          'VALIDATION_FAILED',
          '@praxis/storage: kv write body must carry "value"',
        );
      }
      if ('scope' in body) {
        throw new ConformanceHttpError(
          400,
          'VALIDATION_FAILED',
          '@praxis/storage: kv write body must not carry a scope — this contract is ' +
            'account-scoped and the principal is derived server-side',
        );
      }
      try {
  ...
  const route = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    if (!authenticate(req)) {
      throw new ConformanceHttpError(
        401,
        'UNAUTHENTICATED',
        '@praxis/storage: missing or invalid credential',
      );
    }
    if (!authorize(req, 'kv')) {
      throw new ConformanceHttpError(
        403,
        'FORBIDDEN_KV',
        '@praxis/storage: principal may not perform this operation',
      );
    }
