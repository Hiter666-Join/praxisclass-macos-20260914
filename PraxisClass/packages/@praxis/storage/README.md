...
The PraxisClass pluggable persistence layer: small, swappable-backend primitives for
persisting app state, depending only on [`@praxis/dsl`](../dsl).
...
| `StorageProvider` (from `@praxis/dsl`) | the asset seam: `put(blob) → ref`, `resolve(ref) → url`, `remove(ref)` | `BrowserAssetStore` over IndexedDB (`assets` registry + `blobs`) + object URLs |
...
data lives in the new `praxis-asset-pool` database.
...
- [x] `StorageProvider` (in `@praxis/dsl`) + browser asset registry: allocated
