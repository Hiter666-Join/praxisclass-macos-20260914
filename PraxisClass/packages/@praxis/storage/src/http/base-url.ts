...
export function assertHttpBaseUrl(baseUrl: string, label: string): string {
  if (baseUrl === '') {
    throw new Error(`@praxis/storage: ${label} baseUrl must be non-empty`);
  }
  ...
  if (baseUrl !== baseUrl.trim()) {
    throw new Error(`@praxis/storage: ${label} baseUrl must not begin or end with whitespace`);
  }
  ...
  if (URL_CONTROL_CHARACTERS.test(trimmed)) {
    throw new Error(`@praxis/storage: ${label} baseUrl must not contain control characters`);
  }
  ...
  if (trimmed.includes('?') || trimmed.includes('#')) {
    throw new Error(`@praxis/storage: ${label} baseUrl must not carry a query or fragment`);
  }
  if (!ABSOLUTE_URL.test(trimmed)) {
    ...
    if (!trimmed.startsWith('/')) {
      throw new Error(
        `@praxis/storage: ${label} baseUrl must be an absolute http(s) URL or a path ` +
          `beginning with "/"`,
      );
    }
    ...
    if (trimmed[1] === '/' || trimmed[1] === '\\') {
      throw new Error(
        `@praxis/storage: ${label} baseUrl must be a path, not a reference to another origin`,
      );
    }
    return trimmed;
  }
  ...
  if (!RESOLVABLE_SCHEMES.has(scheme)) {
    throw new Error(
      `@praxis/storage: ${label} baseUrl scheme ${JSON.stringify(scheme)} is not http(s)`,
    );
  }
  if (!SCHEME_AUTHORITY_FORM.test(trimmed)) {
    throw new Error(
      `@praxis/storage: ${label} baseUrl must be http(s) in the "scheme://host" form ` +
  ...
  try {
    ...
    throw new Error(`@praxis/storage: ${label} baseUrl must be a valid URL`);
  }
  ...
  if (parsed.username !== '' || parsed.password !== '') {
    throw new Error(
      `@praxis/storage: ${label} baseUrl must not carry userinfo — fetch refuses to build a ` +
