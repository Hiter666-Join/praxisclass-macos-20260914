...
const OWNER_EVENT_TYPE_CONSTRAINT_V2_SENTINEL = '__PRAXIS_OWNER_EVENT_TYPE_KNOWN_V2__';
...
function quoteIdentifier(identifier: string): string {
  if (!/^[a-z_][a-z0-9_]*$/.test(identifier)) {
    throw new Error(
      `@praxis/storage: invalid agent-session table name ${JSON.stringify(identifier)}`,
    );
  }
  return `"${identifier}"`;
}
    ...
    AgentSessionUrlStore
{
  ...
  async appendUserMessage(
    ...
    return this.transaction(async (tx) => {
      const parent = await this.lockForAppend(tx, sessionId, null);
      if (!parent)
        throw new Error(`@praxis/storage: unknown session ${JSON.stringify(sessionId)}`);
      return this.insertEvent(tx, sessionId, {
        ts: this.clock(),
        attempt: 0,
