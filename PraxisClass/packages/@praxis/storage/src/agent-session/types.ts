...
export class AgentSessionAccessError extends Error {
  ...
  constructor(readonly sessionId: string) {
    super(
      `@praxis/storage: session ${JSON.stringify(sessionId)} is not accessible by this owner`,
    );
  }
}
...
export class AgentSessionLeaseLostError extends Error {
  ...
  constructor(
    ...
    super(
      `@praxis/storage: session ${JSON.stringify(sessionId)} lease or attempt fence was lost ` +
...
export class AgentSessionEntryTreeError extends Error {
  ...
  constructor(
    ...
    super(
      `@praxis/storage: invalid entry tree for session ${JSON.stringify(sessionId)}: ${reason}`,
    );
  }
}
