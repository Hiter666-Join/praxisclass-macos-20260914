...
function assertAccountScope(scope: AccountScope | undefined): void {
  ...
  throw new KVScopeViolationError(
    `@praxis/storage: HttpAccountKV serves the account scope only and was asked for ` +
...
function notEncodable(label: string, reason: string): HttpKVStoreError {
  return new HttpKVStoreError(
    ...
    0,
    'KEY_NOT_ENCODABLE',
    `@praxis/storage: this ${label} cannot be carried over the HTTP URL transport — ${reason}. ` +
...
export class HttpAccountKV {
  ...
  constructor(options: HttpAccountKVOptions) {
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@praxis/storage: HttpKVStore requires a fetch implementation');
    }
  ...
  private async request<T>(
    ...
    if (body !== undefined) {
      ...
      if (headers['content-type'] !== undefined) {
        throw new HttpKVStoreError(
          ...
          '@praxis/storage: the headers hook must not set Content-Type — KV request bodies ' +
    ...
    if (!response.ok) {
      ...
      const message =
        typeof errorBody?.error?.message === 'string'
          ...
          : `@praxis/storage: KVStore HTTP request failed with status ${response.status}`;
    ...
    try {
      ...
      throw new HttpKVStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: KVStore HTTP response body was not valid JSON',
      );
    }
  }
  ...
  async get<T>(key: string, scope?: AccountScope): Promise<T | null> {
    ...
    if (typeof body !== 'object' || body === null || !('value' in body)) {
      throw new HttpKVStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: KVStore HTTP get response must be an object carrying "value"',
      );
    }
  ...
  async keys(prefix = '', scope?: AccountScope): Promise<string[]> {
    ...
    if (!Array.isArray(response.body) || response.body.some((key) => typeof key !== 'string')) {
      throw new HttpKVStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: KVStore HTTP keys response must be an array of strings',
      );
    }
...
export class HttpKVStore implements DeviceSafeKVStore {
  ...
  constructor(options: HttpKVStoreOptions) {
    ...
    if (
      ...
      throw new Error(
        '@praxis/storage: HttpKVStore deviceStore must be a local KVStore (isLocalKVStore) — ' +
