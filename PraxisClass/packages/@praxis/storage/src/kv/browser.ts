...
export interface BrowserKVStoreOptions {
   ...
   * `localStorage` keys the app writes. Defaults to `praxis`.
   */
  namespace?: string;
}
...
export class BrowserKVStore implements LocalKVStore {
  ...
  constructor(options: BrowserKVStoreOptions = {}) {
    ...
    this.namespace = options.namespace ?? 'praxis';
  }
  ...
  async set<T>(key: string, value: T, scope: KVScope = DEFAULT_KV_SCOPE): Promise<void> {
    ...
    if (json === undefined) {
      ...
      throw new Error(
        `@praxis/storage: kv value for key ${JSON.stringify(key)} serialized to undefined ` +
