import { migrate, validateScene, validateStage } from '@praxis/dsl';
...
function assertAddressableSegment(value: string): void {
  if (value === '.' || value === '..') {
    throw new Error(`@praxis/storage: URL path segment must not be ${JSON.stringify(value)}`);
  }
}
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(options: HttpDocumentStoreOptions) {
    if (options.baseUrl === '') {
      throw new Error('@praxis/storage: HttpDocumentStore baseUrl must be non-empty');
    }
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@praxis/storage: HttpDocumentStore requires a fetch implementation');
    }
    ...
    if (!response.ok) {
      ...
      const message =
        typeof errorBody?.error?.message === 'string'
          ...
          : `@praxis/storage: DocumentStore HTTP request failed with status ${response.status}`;
      if (
        response.status === 409 &&
        code === 'FUTURE_VERSION' &&
  ...
  async saveDocument(document: PraxisDocument<TScene, TStage>): Promise<void> {
  ...
  async loadDocument(stageId: string): Promise<PraxisDocument<TScene, TStage> | null> {
    try {
      const document = await this.request<PraxisDocument<TScene, TStage>>(
        'GET',
  ...
  async listDocuments(): Promise<DocumentSummary[]> {
    ...
    if (!Array.isArray(summaries)) {
      throw new HttpDocumentStoreError(
        200,
        'MALFORMED_RESPONSE',
        '@praxis/storage: DocumentStore HTTP listDocuments response must be an array',
      );
    }
