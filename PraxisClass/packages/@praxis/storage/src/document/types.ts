 ...
 */
import type { Stage, Scene } from '@praxis/dsl';
 ...
 */
export interface PraxisDocument<TScene extends SceneLike = Scene, TStage extends Stage = Stage> {
  stage: TStage;
  scenes: TScene[];
  outline?: unknown;
  dslVersion?: string;
...
export class DocumentFolderLimitError extends Error {
  ...
  constructor(readonly limit: number) {
    super(`@praxis/storage: document folder limit reached (${limit})`);
  }
}
...
export interface DocumentStore<TScene extends SceneLike = Scene, TStage extends Stage = Stage> {
   ...
   * {@link putScene}.)
   */
  saveDocument(doc: PraxisDocument<TScene, TStage>): Promise<void>;
   ...
   */
  loadDocument(stageId: string): Promise<PraxisDocument<TScene, TStage> | null>;
