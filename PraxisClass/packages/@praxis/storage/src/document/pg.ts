...
import type { Scene, Stage } from '@praxis/dsl';
...
function assertValid(
  ...
  throw new Error(`@praxis/storage: invalid ${label}: ${detail}`);
}
...
function encodeJson(value: unknown, label: string): string {
  try {
    ...
    throw new Error(`@praxis/storage: ${label} is not JSON-serializable`, { cause: error });
  }
}
...
function migrateDocument<TScene extends SceneLike, TStage extends Stage>(
  doc: PraxisDocument<TScene, TStage>,
): PraxisDocument<TScene, TStage> {
  const { outline, ...core } = doc;
  const migrated = migrate(core) as PraxisDocument<TScene, TStage>;
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof candidate.id !== 'string') {
    throw new Error(
      `@praxis/storage: scene id must be a string, got ${JSON.stringify(candidate.id)}`,
    );
  }
  if (candidate.stageId !== stageId) {
    throw new Error(
      `@praxis/storage: scene ${JSON.stringify(candidate.id)} has stageId ` +
  ...
  if (typeof candidate.order !== 'number' || !Number.isFinite(candidate.order)) {
    throw new Error(
      `@praxis/storage: scene ${JSON.stringify(candidate.id)} order must be a finite number, ` +
  ...
  implements DocumentStore<TScene, TStage>, DocumentFolderStore, StageFreshnessManifestStore
{
  ...
  constructor(queryable: Queryable, options: PgDocumentStoreOptions) {
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@praxis/storage: withTransaction is required and must pin a fresh connection and ' +
    ...
    if (options.ownerId !== undefined && !isPgQueryableKey(options.ownerId)) {
      throw new Error('@praxis/storage: PgDocumentStore ownerId must be lossless JSON text');
    }
  ...
  private requireOwner(operation: string): string {
    if (this.ownerId === null) {
      throw new Error(`@praxis/storage: ${operation} requires an owner-bound document store`);
    }
    return this.ownerId;
  }
  ...
  private async loadStage(
    ...
    if (!isPlainObject(decoded)) {
      throw new Error(
        `@praxis/storage: corrupt stored row for document ${JSON.stringify(stageId)}: ` +
  ...
  private currentVersionError(
    ...
    return new DocumentVersionError(
      ...
      `@praxis/storage: cannot ${operation} document ${JSON.stringify(stageId)} at DSL ` +
  ...
  private validateForSave(
    ...
    for (const scene of doc.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@praxis/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ` +
  ...
  private async persistStage(queryable: Queryable, stageRow: StageRow<TStage>): Promise<void> {
    ...
    if (result.rows.length === 0) {
      throw new DocumentNotFoundError(
        stageRow.id,
        `@praxis/storage: document ${JSON.stringify(stageRow.id)} belongs to another scope`,
      );
    }
  }

  async saveDocument(doc: PraxisDocument<TScene, TStage>): Promise<void> {
    if (isFutureVersioned(doc)) {
      throw new DocumentVersionError(
        doc.stage.id,
        'future',
        doc.dslVersion,
        `@praxis/storage: refusing to save document ${JSON.stringify(doc.stage.id)} — it was ` +
    ...
    await this.transaction(async (queryable) => {
      ...
      if (existingStage && isFutureVersioned(existingStage)) {
        throw new DocumentVersionError(
          ...
          `@praxis/storage: refusing to overwrite document ${JSON.stringify(stageId)} — the ` +
  ...
  async loadDocument(stageId: string): Promise<PraxisDocument<TScene, TStage> | null> {
  ...
  async createFolder(
    ...
    if (!isPgQueryableKey(folderId) || !isPgQueryableKey(name)) {
      throw new Error('@praxis/storage: folder id and name must be lossless JSON text');
    }
  ...
  async putStage(stageId: string, stage: TStage): Promise<void> {
    ...
    if (stage.id !== stageId) {
      throw new Error(
        `@praxis/storage: stage ${JSON.stringify(stage.id)} does not belong to document ` +
          JSON.stringify(stageId),
      );
    }
    ...
    await this.transaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@praxis/storage: cannot putStage into missing document ${JSON.stringify(stageId)}`,
        );
      }
  ...
  async putScene(stageId: string, scene: TScene): Promise<void> {
    ...
    await this.transaction(async (queryable) => {
      ...
      if (!stored) {
        throw new DocumentNotFoundError(
          stageId,
          `@praxis/storage: cannot putScene into missing document ${JSON.stringify(stageId)}`,
        );
      }
      if (dslVersionOf(stored) !== DSL_VERSION) {
