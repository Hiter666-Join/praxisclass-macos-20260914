...
export interface BrowserDocumentStoreOptions {
  ...
  /** Database name. Defaults to `praxis-documents`. */
  dbName?: string;
  /**
...
function assertStorableScene(scene: SceneLike, stageId: string): void {
  ...
  if (typeof s.id !== 'string') {
    throw new Error(`@praxis/storage: scene id must be a string, got ${JSON.stringify(s.id)}`);
  }
  if (s.stageId !== stageId) {
    throw new Error(
      `@praxis/storage: scene ${JSON.stringify(s.id)} has stageId ` +
        `${JSON.stringify(s.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof s.order !== 'number' || !Number.isFinite(s.order)) {
    throw new Error(
      `@praxis/storage: scene ${JSON.stringify(s.id)} order must be a finite number, got ` +
...
> implements DocumentStore<TScene, TStage> {
  ...
  constructor(options: BrowserDocumentStoreOptions = {}) {
    ...
    this.dbName = options.dbName ?? 'praxis-documents';
  ...
  async saveDocument(doc: PraxisDocument<TScene, TStage>): Promise<void> {
    ...
    for (const scene of normalized.scenes) {
      ...
      if (seen.has(scene.id)) {
        throw new Error(
          `@praxis/storage: duplicate scene id ${JSON.stringify(scene.id)} in document ${JSON.stringify(stageId)}`,
        );
      }
      seen.add(scene.id);
    }
    ...
    await this.txRun([STAGES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          `@praxis/storage: cannot putStage into document ${JSON.stringify(stageId)} at DSL ` +
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@praxis/storage: cannot putScene into document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
    ...
    await this.txRun([STAGES, SCENES], 'readwrite', async (tx) => {
      ...
      if (dslVersionOf(stageRow) !== DSL_VERSION) {
        throw new DocumentVersionError(
          ...
          'not-current',
          stageRow[DSL_VERSION_KEY],
          `@praxis/storage: cannot deleteScene from document ${JSON.stringify(stageId)} at DSL ` +
            `version ${JSON.stringify(dslVersionOf(stageRow))} — load and save it to bring it ` +
            `to ${DSL_VERSION} first`,
        );
      }
