import { migrateRuntime, validateRuntimeRecord, validateRuntimeSession } from '@praxis/dsl';
import type {
  RuntimePayload,
  RuntimeRecord,
  RuntimeRecordInit,
...
function assertValidSession(session: RuntimeSession): RuntimeSession {
  ...
  throw new Error(
    `@praxis/storage: invalid stored runtime session ${JSON.stringify(id)}: ${detail}`,
  );
}
...
function assertValidRecord<TPayload extends RuntimePayload>(
  ...
  throw new Error(
    `@praxis/storage: invalid stored runtime record ${JSON.stringify(id)}: ${detail}`,
  );
}
...
function assertValidRecordInit<TPayload extends RuntimePayload>(
  ...
  throw new HttpRuntimeStoreError(
    400,
    'VALIDATION_FAILED',
    `@praxis/storage: invalid runtime record ${JSON.stringify(init.id)}: ${detail}`,
  );
}
...
export class HttpRuntimeStore implements RuntimeStore {
  ...
  constructor(options: HttpRuntimeStoreOptions) {
    if (options.baseUrl === '') {
      throw new Error('@praxis/storage: HttpRuntimeStore baseUrl must be non-empty');
    }
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@praxis/storage: HttpRuntimeStore requires a fetch implementation');
    }
    ...
    if (!response.ok) {
      ...
      const message =
        typeof errorBody?.error?.message === 'string'
          ...
          : `@praxis/storage: RuntimeStore HTTP request failed with status ${response.status}`;
  ...
  async listSessions(stageId: string, learnerKey: string): Promise<RuntimeSession[]> {
    ...
    if (!Array.isArray(response.body)) {
      throw new HttpRuntimeStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: RuntimeStore HTTP listSessions response must be an array',
      );
    }
  ...
  async listRecords(sessionId: string, opts?: { sceneId?: string }): Promise<RuntimeRecord[]> {
    ...
    if (!Array.isArray(response.body)) {
      throw new HttpRuntimeStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: RuntimeStore HTTP listRecords response must be an array',
      );
    }
  ...
  async mergeLearner(fromLearnerKey: string, toLearnerKey: string): Promise<number> {
    ...
    if (typeof moved !== 'number' || !Number.isInteger(moved) || moved < 0) {
      throw new HttpRuntimeStoreError(
        response.status,
        'MALFORMED_RESPONSE',
        '@praxis/storage: RuntimeStore HTTP mergeLearner response moved must be a non-negative integer',
      );
    }
    return moved;
  }
