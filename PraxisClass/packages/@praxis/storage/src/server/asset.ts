...
import type { AssetMeta } from '@praxis/dsl';
...
function assertPositiveSafeInteger(value: number, label: string): void {
  if (!Number.isSafeInteger(value) || value <= 0) {
    throw new Error(`@praxis/storage: ${label} must be a positive safe integer`);
  }
}
...
function assertMultipartContentType(contentType: string | undefined): string {
  if (
    ...
    throw new AssetHttpError(
      ...
      '@praxis/storage: asset writes require multipart/form-data',
    );
  }
  return contentType;
}
  ...
  for await (const chunk of req) {
    ...
    if (total > maxRequestBytes) {
      throw payloadTooLarge(
        `@praxis/storage: request body exceeds maxRequestBytes (${maxRequestBytes})`,
      );
    }
    chunks.push(buffer);
  }
...
async function assertBodyless(req: IncomingMessage): Promise<void> {
  for await (const chunk of req) {
    ...
    if (size > 0) {
      throw validationFailure('@praxis/storage: this asset route does not accept a body');
    }
  }
}
...
function assertServerMetadataValue(value: unknown): void {
  const visit = (member: unknown): void => {
    if (typeof member === 'string' && member.includes('\u0000')) {
      throw validationFailure('@praxis/storage: asset metadata contains U+0000');
    }
    if (typeof member === 'number' && Object.is(member, -0)) {
      throw validationFailure('@praxis/storage: asset metadata contains negative zero');
    }
...
async function parseMeta(part: Blob): Promise<AssetMeta> {
  if (part.type.split(';', 1)[0]?.trim().toLowerCase() !== 'application/json') {
    throw validationFailure('@praxis/storage: the meta part must be application/json');
  }
  let text: string;
  try {
    text = new TextDecoder('utf-8', { fatal: true }).decode(await part.arrayBuffer());
  } catch {
    throw validationFailure('@praxis/storage: the meta part must contain valid UTF-8');
  }
  let value: unknown;
  try {
    value = JSON.parse(text) as unknown;
  } catch {
    throw validationFailure('@praxis/storage: the meta part must contain valid JSON');
  }
  if (typeof value !== 'object' || value === null || Array.isArray(value)) {
    throw validationFailure('@praxis/storage: the meta part must contain a JSON object');
  }
  if ('principal' in value || 'contentHash' in value) {
    throw validationFailure('@praxis/storage: asset metadata contains a prohibited member');
  }
...
async function readWrite(
  ...
  if (req.headers['content-encoding'] !== undefined) {
    throw validationFailure('@praxis/storage: Content-Encoding is not accepted on asset writes');
  }
  ...
  try {
    ...
    throw validationFailure('@praxis/storage: malformed multipart body');
  }
  const parts = [...form.entries()];
  if (parts.length > limits.maxParts) {
    throw payloadTooLarge(
      `@praxis/storage: asset write body exceeds maxParts (${limits.maxParts})`,
    );
  }
  const named = new Map<'meta' | 'bytes', string | Blob>();
  for (const [name, part] of parts) {
    if (name !== 'meta' && name !== 'bytes') {
      throw validationFailure('@praxis/storage: asset write body contains an unrecognized part');
    }
    if (named.has(name)) {
      throw validationFailure('@praxis/storage: asset write body contains a duplicate part');
    }
    named.set(name, part);
  }
  const expectedLengths = requiredMeta ? [2] : [1, 2];
  if (!expectedLengths.includes(parts.length)) {
    throw validationFailure('@praxis/storage: asset write body has the wrong number of parts');
  }
  const metaPart = named.get('meta');
  const bytesPart = named.get('bytes');
  if (bytesPart === undefined) {
    throw validationFailure('@praxis/storage: asset write body must carry a "bytes" part');
  }
  if (requiredMeta && metaPart === undefined) {
    throw validationFailure('@praxis/storage: asset write body must carry a "meta" part');
  }
  if (metaPart !== undefined && parts[0]?.[0] !== 'meta') {
    throw validationFailure('@praxis/storage: the meta part must precede the bytes part');
  }
  if (typeof bytesPart === 'string') {
    throw validationFailure('@praxis/storage: the bytes part must be sent as a file');
  }
  if (typeof metaPart === 'string') {
    throw validationFailure('@praxis/storage: the meta part must be sent as a file');
  }
  if (bytesPart.size > limits.maxAssetBytes) {
    throw payloadTooLarge(
      `@praxis/storage: bytes part exceeds maxAssetBytes (${limits.maxAssetBytes})`,
    );
  }
  if (metaPart !== undefined) {
    if (metaPart.size > limits.maxMetaBytes) {
      throw payloadTooLarge(
        `@praxis/storage: meta part exceeds maxMetaBytes (${limits.maxMetaBytes})`,
      );
    }
  }
...
function parsePath(req: IncomingMessage): string[] {
  ...
  if (target.includes('?')) {
    throw validationFailure('@praxis/storage: asset routes do not accept a query string');
  }
  ...
  try {
    ...
    throw validationFailure('@praxis/storage: request path is not valid percent-encoded UTF-8');
  }
}
...
function assertMethod(
  ...
  if (!accepted) {
    throw new AssetHttpError(
      405,
      'METHOD_NOT_ALLOWED',
      '@praxis/storage: method not allowed for this asset route',
      undefined,
      { allow },
    );
  }
...
function missingAsset(): AssetHttpError {
  return new AssetHttpError(
    404,
    'ASSET_NOT_FOUND',
    '@praxis/storage: no asset is stored under that id',
  );
}
...
function classifyStoreError(error: unknown): never {
  ...
  if (error instanceof AssetQuotaExceededError) {
    throw new AssetHttpError(
      ...
      '@praxis/storage: asset quota exceeded for this principal',
    );
  }
  throw error;
}
...
function mappedError(error: unknown): {
  ...
  return {
    status: 500,
    body: {
      error: { code: 'INTERNAL_ERROR', message: '@praxis/storage: internal server error' },
    },
    headers: {},
  };
}
...
async function route(
  ...
  if (candidate === undefined) {
    throw new AssetHttpError(401, 'UNAUTHENTICATED', '@praxis/storage: authentication required');
  }
  if (typeof candidate !== 'object' || candidate === null) {
    throw new Error('@praxis/storage: asset authenticator returned a malformed principal');
  }
  if (!('key' in candidate)) {
    throw new AssetHttpError(
      403,
      'FORBIDDEN_ASSETS',
      '@praxis/storage: asset authorization required',
    );
  }
  ...
  if (shape.kind === 'collection') {
    ...
    if (typeof id !== 'string') {
      throw new Error('@praxis/storage: asset store returned a malformed id');
    }
    sendJson(
      req,
      res,
  ...
  if (method === 'PUT') {
    ...
    if (!Number.isSafeInteger(revision) || revision < 1) {
      throw new Error('@praxis/storage: asset store returned a malformed revision');
    }
    sendNoContent(res, {
    ...
    if (typeof resolveIndirect === 'function') {
      ...
      if (indirect !== undefined) {
        if (typeof indirect.url !== 'string' || indirect.url === '') {
          throw new Error('@praxis/storage: asset store returned a malformed signed URL');
        }
        if (!isAbsoluteHttpUrl(indirect.url)) {
          ...
          throw new Error('@praxis/storage: asset store returned a non-http(s) signed URL');
        }
  ...
  if ('byteLength' in asset && (!Number.isSafeInteger(asset.byteLength) || asset.byteLength < 0)) {
    throw new Error('@praxis/storage: asset store returned a malformed byte length');
  }
...
export function createAssetHttpHandler(
  ...
  if (!store) throw new Error('@praxis/storage: createAssetHttpHandler requires an asset store');
  if (typeof options?.authenticate !== 'function') {
    throw new Error('@praxis/storage: createAssetHttpHandler requires authenticate');
  }
  ...
  if (maxParts < 2) {
    throw new Error('@praxis/storage: maxParts must allow the two required POST parts');
  }
  if (maxRequestBytes <= maxAssetBytes + maxMetaBytes) {
    throw new Error(
      '@praxis/storage: maxRequestBytes must exceed maxAssetBytes + maxMetaBytes for multipart framing',
    );
  }
  ...
  if (configuredTypes.some((value) => excluded.has(value.toLowerCase()))) {
    throw new Error('@praxis/storage: renderableTypes contains an excluded executable type');
  }
  if (configuredTypes.some((value) => value.toLowerCase() === ASSET_DESCRIPTOR_MEDIA_TYPE)) {
    ...
    throw new Error(
      '@praxis/storage: renderableTypes must not contain the asset descriptor media type',
    );
  }
  if (configuredTypes.some((value) => value !== value.trim() || value.includes(';'))) {
    throw new Error('@praxis/storage: renderableTypes must contain exact media types');
  }
  ...
  if (egress !== 'direct') {
    if (typeof egress !== 'object' || egress === null || egress.mode !== 'redirect') {
      throw new Error(
        '@praxis/storage: byteEgress must be "direct" or { mode: "redirect", collectionGraceMs }',
      );
    }
    byteEgress = 'redirect';
    if (egress.signedUrlTtlSeconds !== undefined) {
      ...
      if (egress.signedUrlTtlSeconds > MAX_SIGNED_URL_TTL_SECONDS) {
        throw new Error(
          `@praxis/storage: signedUrlTtlSeconds must not exceed ${MAX_SIGNED_URL_TTL_SECONDS}`,
        );
      }
  ...
  return (req, res) => {
    void route(req, res, store, options, config).catch((error: unknown) => {
      ...
      if (!(error instanceof AssetHttpError) || error.status >= 500) {
        console.error('@praxis/storage: Asset HTTP handler internal error');
      }
