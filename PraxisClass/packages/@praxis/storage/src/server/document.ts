...
import type { Scene, Stage, ValidationResult } from '@praxis/dsl';
...
function validationError(result: ValidationResult, label: string): void {
  ...
  throw validationFailure(`@praxis/storage: invalid ${label}: ${detail}`, details);
}

function assertAddressableSegment(value: string): void {
  if (value === '.' || value === '..') {
    throw validationFailure(
      `@praxis/storage: URL path segment must not be ${JSON.stringify(value)}`,
    );
  }
}
...
function missingDocument(stageId: string): DocumentHttpError {
  return new DocumentHttpError(
    404,
    'DOCUMENT_NOT_FOUND',
    `@praxis/storage: missing document ${JSON.stringify(stageId)}`,
  );
}
...
function missingScene(stageId: string, sceneId: string): DocumentHttpError {
  return new DocumentHttpError(
    404,
    'SCENE_NOT_FOUND',
    `@praxis/storage: no scene ${JSON.stringify(sceneId)} in document ${JSON.stringify(stageId)}`,
  );
}
...
function migrateDocument<TScene extends SceneLike, TStage extends Stage>(
  document: PraxisDocument<TScene, TStage>,
): PraxisDocument<TScene, TStage> {
  const { outline, ...core } = document;
  let migrated: PraxisDocument<TScene, TStage>;
  try {
    migrated = migrate(core) as PraxisDocument<TScene, TStage>;
  } catch (error) {
    throw validationFailure(error instanceof Error ? error.message : String(error));
  }
...
function assertStorableScene(scene: unknown, stageId: string): asserts scene is SceneLike {
  ...
  if (typeof value.id !== 'string') {
    throw validationFailure(
      `@praxis/storage: scene id must be a string, got ${JSON.stringify(value.id)}`,
    );
  }
  if (value.stageId !== stageId) {
    throw validationFailure(
      `@praxis/storage: scene ${JSON.stringify(value.id)} has stageId ` +
        `${JSON.stringify(value.stageId)} but belongs to document ${JSON.stringify(stageId)}`,
    );
  }
  if (typeof value.order !== 'number' || !Number.isFinite(value.order)) {
    throw validationFailure(
      `@praxis/storage: scene ${JSON.stringify(value.id)} order must be a finite number, got ` +
...
function validateDocument<TScene extends SceneLike, TStage extends Stage>(
  ...
  const document = body as unknown as PraxisDocument<TScene, TStage>;
  if (isFutureVersioned(document)) {
    throw futureVersion(
      `@praxis/storage: refusing to save document ${JSON.stringify(pathStageId)} — it was ` +
  ...
  if (normalized.stage.id !== pathStageId) {
    throw validationFailure(
      `@praxis/storage: stage ${JSON.stringify(normalized.stage.id)} does not belong to ` +
...
async function route<TScene extends SceneLike, TStage extends Stage>(
  ...
  if (principal === undefined) {
    throw new DocumentHttpError(
      401,
      'UNAUTHENTICATED',
      '@praxis/storage: authentication required',
    );
  }
  if (!(await (options.authorizeDocuments?.(principal, req) ?? true))) {
    throw new DocumentHttpError(
      403,
      'FORBIDDEN_DOCUMENTS',
      '@praxis/storage: document authorization required',
    );
  }
...
>(store: DocumentStore<TScene, TStage>, options: DocumentHttpHandlerOptions): RequestListener {
  if (typeof options?.authenticate !== 'function') {
    throw new Error('@praxis/storage: createDocumentHttpHandler requires authenticate');
  }
  ...
  return (req, res) => {
    void route(req, res, store, options).catch((error: unknown) => {
      ...
      if (!(error instanceof DocumentHttpError) || error.status >= 500) {
        console.error('@praxis/storage: Document HTTP handler internal error', error);
      }
