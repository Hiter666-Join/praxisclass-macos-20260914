...
function assertAddressableSegment(
  ...
  if (typeof value !== 'string' || value === '') {
    throw validationFailure(`@praxis/storage: ${label} must be a non-empty string`);
  }
  if (value === '.' || value === '..') {
...
function missingSessionError(sessionId: string): RuntimeHttpError {
  return new RuntimeHttpError(
    404,
    'SESSION_NOT_FOUND',
    `@praxis/storage: no session ${JSON.stringify(sessionId)}`,
  );
}
...
function assertNotFutureSession(session: RuntimeSession): void {
  ...
  if (!needsRuntimeMigration(session) && version !== RUNTIME_DSL_VERSION) {
    throw new RuntimeHttpError(
      409,
      'FUTURE_VERSION',
      `@praxis/storage: session ${JSON.stringify(session.id)} was written at runtime DSL ` +
...
function validatePayloadForKind(
  ...
  if (validator) {
    validationError(
      validator(payload),
      `@praxis/storage: invalid runtime record ${JSON.stringify(recordId)}`,
    );
  }
}
...
function forbiddenLearner(): RuntimeHttpError {
  return new RuntimeHttpError(
    403,
    'FORBIDDEN_LEARNER',
    '@praxis/storage: authenticated learner may not access the requested learner partition',
  );
}
...
function forbiddenAdmin(): RuntimeHttpError {
  return new RuntimeHttpError(
    403,
    'FORBIDDEN_ADMIN',
    '@praxis/storage: admin authorization required',
  );
}
...
async function rethrowClassifiedSessionWriteFailure(
  ...
  if (current.status !== 'active') {
    throw validationFailure(
      `@praxis/storage: session ${JSON.stringify(sessionId)} is no longer active; ` +
...
async function rethrowClassifiedMergeFailure(
  ...
  const match =
    error instanceof Error
      ? /^@praxis\/storage: session ("(?:[^"\\]|\\.)*") was written at runtime DSL version /.exec(
          error.message,
        )
      : null;
...
function mappedError(error: unknown): { status: number; body: ErrorBody } {
  ...
  return {
    ...
    body: {
      error: {
        code: 'INTERNAL_ERROR',
        message: '@praxis/storage: internal server error',
      },
    },
  };
}
...
async function route(
  ...
  if (
    ...
    throw new RuntimeHttpError(
      500,
      'INTERNAL_ERROR',
      '@praxis/storage: authenticate returned an invalid principal',
    );
  }
  ...
  if (method === 'POST' && parts.length === 2 && parts[1] === 'sessions') {
    ...
    validationError(
      ...
      `@praxis/storage: invalid runtime session ${JSON.stringify(init.id)}`,
    );
    ...
    if (existing !== undefined) {
      ...
      throw new RuntimeHttpError(
        ...
        `@praxis/storage: session ${JSON.stringify(init.id)} already exists`,
      );
    }
    try {
  ...
  if (parts[1] === 'sessions' && parts.length >= 3) {
    ...
    if (method === 'PATCH' && parts.length === 4 && parts[3] === 'status') {
      ...
      validationError(
        ...
        `@praxis/storage: invalid runtime session ${JSON.stringify(sessionId)}`,
      );
      try {
    ...
    if (method === 'POST' && parts.length === 4 && parts[3] === 'records') {
      ...
      validationError(
        ...
        `@praxis/storage: invalid runtime record ${JSON.stringify(init.id)}`,
      );
      ...
      if (session.status !== 'active') {
        throw validationFailure(
          `@praxis/storage: cannot append to session ${JSON.stringify(sessionId)} with ` +
  ...
  if (method === 'POST' && parts.length === 3 && parts[1] === 'learners' && parts[2] === 'merge') {
    ...
    if (
      ...
      throw validationFailure('@praxis/storage: learner keys must be non-empty strings');
    }
...
export function createRuntimeHttpHandler(
  ...
  if (typeof options?.authenticate !== 'function') {
    throw new Error('@praxis/storage: createRuntimeHttpHandler requires authenticate');
  }
  ...
  return (req, res) => {
    void route(req, res, store, options).catch((error: unknown) => {
      ...
      if (
        ...
        console.error('@praxis/storage: Runtime HTTP handler internal error', error);
      }
