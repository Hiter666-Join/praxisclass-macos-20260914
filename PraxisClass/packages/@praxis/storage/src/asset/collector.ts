...
export function assertSignedUrlTtlWithinGrace(ttlSeconds: number, graceMs: number): void {
  if (
    ...
    throw new Error(
      '@praxis/storage: the signed URL lifetime must stay far below the collection grace period',
    );
  }
}
...
function collectorFailure(): Error {
  return new Error('@praxis/storage: asset collection failed');
}

function collectorConfigurationFailure(): Error {
  return new Error(
    '@praxis/storage: the asset byte store cannot coordinate collection with the registry. A ' +
...
export class AssetCollector {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error('@praxis/storage: withTransaction is required for AssetCollector');
    }
    const graceMs = options.graceMs ?? DEFAULT_ASSET_COLLECTION_GRACE_MS;
    if (!Number.isSafeInteger(graceMs) || graceMs < 0) {
      throw new Error('@praxis/storage: graceMs must be a non-negative safe integer');
    }
    const batchSize = options.batchSize ?? DEFAULT_ASSET_COLLECTION_BATCH_SIZE;
    if (!Number.isSafeInteger(batchSize) || batchSize < 1) {
      throw new Error('@praxis/storage: batchSize must be a positive safe integer');
    }
