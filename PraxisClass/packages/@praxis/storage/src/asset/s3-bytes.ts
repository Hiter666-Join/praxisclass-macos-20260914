...
function missingSdk(error: unknown): Error {
  return new Error(
    `@praxis/storage: the S3 asset byte store requires the optional ${AWS_S3_CLIENT_PACKAGE} ` +
...
function missingPresigner(error: unknown): Error {
  return new Error(
    `@praxis/storage: signing S3 asset read URLs requires the optional ${AWS_S3_PRESIGNER_PACKAGE} ` +
      'dependency, which could not be resolved. Install it, or construct the store with ' +
      'an explicit `signer`.',
    { cause: error },
  );
...
export async function loadS3AssetByteStore(bucket: string): Promise<AssetByteStore> {
  try {
    ...
    throw new Error('@praxis/storage: S3 asset byte store initialization failed', {
      cause: error,
    });
  }
}
...
function s3Failure(operation: string): Error {
  return new Error(`@praxis/storage: S3 asset byte ${operation} failed`);
}
