 ...
 * transaction.
 */
import type { AssetMeta, AssetRef, BinaryBlob } from '@praxis/dsl';
...
function registryConfigurationFailure(problem: string): Error {
  return new Error(`@praxis/storage: ${problem}`);
}

function registryFailure(operation: string): Error {
  return new Error(`@praxis/storage: asset registry ${operation} failed`);
}
...
function encodeMeta(meta: AssetMeta): string {
  ...
  try {
    ...
    throw new Error('@praxis/storage: asset metadata is not JSON-serializable');
  }
}
...
export class PgAssetStore implements AssetStore {
  ...
  constructor(
    ...
    if (typeof options?.withTransaction !== 'function') {
      throw new Error(
        '@praxis/storage: withTransaction is required and must pin a fresh connection and transaction for every call',
      );
    }
    if (!options.byteStore) {
      throw new Error('@praxis/storage: byteStore is required for PgAssetStore');
    }
    if (
      options.quotaBytes !== undefined &&
      (!Number.isSafeInteger(options.quotaBytes) || options.quotaBytes < 0)
    ) {
      throw new Error('@praxis/storage: quotaBytes must be a non-negative safe integer');
    }
