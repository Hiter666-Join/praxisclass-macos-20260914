import type { AssetMeta, AssetRef, BinaryBlob, StorageProvider } from '@praxis/dsl';
...
function deadlineFailure(): HttpAssetStoreError {
  return new HttpAssetStoreError(
    0,
    'HTTP_REQUEST_FAILED',
    '@praxis/storage: asset HTTP request failed',
  );
}
...
function assertMetadata(meta: AssetMeta): void {
  if (typeof meta !== 'object' || meta === null || Array.isArray(meta)) {
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      '@praxis/storage: asset metadata must be a plain JSON object',
    );
  }
  if ('principal' in meta || 'contentHash' in meta) {
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      '@praxis/storage: asset metadata contains a prohibited member',
    );
  }
  try {
    ...
    throw new HttpAssetStoreError(
      0,
      'VALIDATION_FAILED',
      error instanceof Error ? error.message : '@praxis/storage: invalid asset metadata',
    );
  }
}
...
export class HttpAssetStore implements StorageProvider {
  ...
  constructor(options: HttpAssetStoreOptions) {
    ...
    if (typeof selectedFetch !== 'function') {
      throw new Error('@praxis/storage: HttpAssetStore requires a fetch implementation');
    }
  ...
  private async headers(
    ...
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'HTTP_REQUEST_FAILED',
        '@praxis/storage: asset request headers could not be constructed',
      );
    }
    if (multipart && headers['content-type'] !== undefined) {
      throw new HttpAssetStoreError(
        ...
        '@praxis/storage: the headers hook must not set Content-Type for multipart asset writes',
      );
    }
    return headers;
  }
  ...
  private async fetchResponse(
    ...
    if (this.closed) {
      throw new HttpAssetStoreError(
        0,
        'STORE_CLOSED',
        '@praxis/storage: HttpAssetStore is closed',
      );
    }
  ...
  private async httpError(response: Response): Promise<HttpAssetStoreError> {
    ...
    const message =
      typeof body?.error?.message === 'string'
        ? body.error.message
        : `@praxis/storage: asset HTTP request failed with status ${response.status}`;
  ...
  private async writeForm(
    ...
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'VALIDATION_FAILED',
        '@praxis/storage: asset bytes could not be read',
      );
    }
    if (!/^[\x20-\x7e]*$/.test(data.type)) {
      throw new HttpAssetStoreError(
        0,
        'VALIDATION_FAILED',
        '@praxis/storage: asset media type contains characters that cannot be carried in a header',
      );
    }
    ...
    if (includeMeta) {
      try {
        ...
        throw new HttpAssetStoreError(
          0,
          'VALIDATION_FAILED',
          '@praxis/storage: asset metadata could not be serialized',
        );
      }
    }
    let boundary: string;
    try {
      ...
      throw new HttpAssetStoreError(
        0,
        'HTTP_REQUEST_FAILED',
        '@praxis/storage: a safe multipart boundary could not be generated',
      );
    }
  ...
  async put(data: BinaryBlob, meta?: AssetMeta): Promise<AssetRef> {
    ...
    try {
      ...
      throw malformed(
        response.status,
        '@praxis/storage: asset allocation response was not valid JSON',
      );
    }
    const id =
    ...
    if (
      ...
      throw malformed(
        response.status,
        '@praxis/storage: asset allocation response must carry an id and revision',
      );
    }
    return id;
  }
  ...
  private async fetchBytes(
    ...
    if (redirectAnswer) {
      throw malformed(
        response.status,
        '@praxis/storage: asset byte GET answered with a redirect, which is never followed',
      );
    }
    if (!response.ok) {
    ...
    if (servedEssence?.toLowerCase() === ASSET_DESCRIPTOR_MEDIA_TYPE) {
      ...
      try {
        ...
        throw malformed(
          response.status,
          '@praxis/storage: asset egress descriptor could not be read',
        );
      }
      const signedUrl =
      ...
      if (
        ...
        throw malformed(
          response.status,
          '@praxis/storage: asset egress descriptor must carry a url and revision',
        );
      }
      if (!isAbsoluteHttpUrl(signedUrl)) {
        ...
        throw malformed(
          response.status,
          '@praxis/storage: asset egress descriptor must carry an absolute http(s) url',
        );
      }
      ...
      if (byteResponse.status === 404) {
        ...
        throw malformed(
          byteResponse.status,
          '@praxis/storage: asset signed URL answered 404 without confirming a missing object',
        );
      }
      if (!byteResponse.ok) {
        throw malformed(
          byteResponse.status,
          '@praxis/storage: asset signed URL did not serve bytes',
        );
      }
      ...
      if (mediaType === null || mediaType === '') {
        throw malformed(
          byteResponse.status,
          '@praxis/storage: asset byte response must carry content type and revision',
        );
      }
      ...
      try {
        ...
        throw malformed(
          byteResponse.status,
          '@praxis/storage: asset byte response could not be read',
        );
      }
    } else {
  ...
  private async get(
    ...
    const url = await this.urls.resolve(id, identity, async () => {
      ...
      try {
        ...
        throw malformed(status, '@praxis/storage: asset object URL could not be created');
      }
  ...
  async remove(id: AssetRef): Promise<void> {
    ...
    if (!response.ok) throw await this.httpError(response);
    if (response.status !== 204) {
      throw malformed(response.status, '@praxis/storage: asset removal response must be 204');
    }
    await this.invalidate(id);
  }
  ...
  async replace(id: AssetId, data: BinaryBlob, meta?: AssetMeta): Promise<void> {
    ...
    if (response.status !== 204 || revision === null || revision === '') {
      throw malformed(
        response.status,
        '@praxis/storage: asset replacement response must be 204 with a revision',
      );
    }
    await this.invalidate(id);
  }
