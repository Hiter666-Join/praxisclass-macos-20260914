...
The **contract keystone** of the PraxisClass SDK family. `@praxis/dsl` is *pure spec* — the
...
package can depend on `@praxis/dsl` without pulling in junk.

## Dependency arrows (acyclic)

```
@praxis/dsl       ->  (nothing)
@praxis/renderer  ->  @praxis/dsl
@praxis/importer  ->  @praxis/dsl
@praxis/exporter  ->  @praxis/dsl     (reserved, future)
```

`@praxis/dsl` is the only package everything else depends on, and it depends on
nothing.
...
```ts
import type { Slide, PPTElement, Action } from '@praxis/dsl';
import { isTextElement, DSL_VERSION, SYNC_ACTIONS } from '@praxis/dsl';
```
...
1. **JSON Schema artifacts (cross-language mirror)** — `Stage`, the default
   ...
   ```ts
   import stageSchema from '@praxis/dsl/schema/stage.schema.json' with { type: 'json' };
   import sceneSchema from '@praxis/dsl/schema/scene.schema.json' with { type: 'json' };
   import actionSchema from '@praxis/dsl/schema/action.schema.json' with { type: 'json' };
...
2. **Pure validators (in-process boundary)** — `validate*` are hand-written,
   ...
   ```ts
   import { validateStage, validateScene, validateAction } from '@praxis/dsl';
...
3. **Pure normalizers (repair boundary)** — where `validate*` *reports* on a
   ...
   ```ts
   import { normalizeElement, normalizeScene, ELEMENT_DEFAULTS } from '@praxis/dsl';
...
```ts
import { DSL_VERSION, migrate, dslVersionOf, needsMigration } from '@praxis/dsl';
...
outside the document, per learner, through a `RuntimeStore` (`@praxis/storage`).
...
Both consumers are now wired to `@praxis/dsl` and no longer vendor their own copy
of the slide types:

- **`@praxis/importer`**: imports all slide types from `@praxis/dsl`; vendored
...
- **`@praxis/renderer`**: imports all slide types from `@praxis/dsl`; vendored
  `types/slides.ts` deleted. `@praxis/dsl` is a regular dependency, kept external
  in the rollup build so consumers share one copy. The public
  `@praxis/renderer/types` surface now re-exports the DSL types.

### Roadmap

- [x] Wire `@praxis/importer` to import types from `@praxis/dsl` (vendored copy deleted).
- [x] Wire `@praxis/renderer` to import types from `@praxis/dsl` (vendored copy deleted).
...
- [ ] Reserve `@praxis/exporter` as the 4th family member.
...
```ts
import type { Action, InteractiveContent, PBLContent, Scene, SceneContent, WidgetConfigBase } from '@praxis/dsl';
...
```bash
pnpm --filter @praxis/dsl build         # -> dist/ (index.js, index.d.ts, …) + dist/schema/*.json
pnpm --filter @praxis/dsl build:schema  # regenerate only dist/schema/*.json
pnpm --filter @praxis/dsl typecheck
pnpm --filter @praxis/dsl test
```

## License

MIT, matching the rest of the family (`@praxis/dsl`, `@praxis/importer`,
`@praxis/renderer`) and the PraxisClass root, so the license policy is uniform
across the SDK.
