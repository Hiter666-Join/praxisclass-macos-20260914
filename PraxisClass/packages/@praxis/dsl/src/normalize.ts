...
function pblFail(field: string, expected: string, value: unknown): never {
  throw new Error(
    `@praxis/dsl: cannot normalize PBL project: \`${field}\` must be ${expected}, got ${JSON.stringify(value)}`,
  );
}
...
function fail(el: Raw, field: string, expected: string, value: unknown = el[field]): never {
  throw new Error(
    `@praxis/dsl: cannot normalize ${String(el.type)} element ${JSON.stringify(el.id)}: ` +
...
export function normalizeElement(el: unknown): PPTElement {
  if (!isObject(el))
    throw new Error(
      `@praxis/dsl: cannot normalize element: expected an object, got ${JSON.stringify(el)}`,
    );
  switch (el.type) {
    case 'text':
    ...
    default:
      throw new Error(
        `@praxis/dsl: cannot normalize element ${JSON.stringify(el.id)}: ` +
