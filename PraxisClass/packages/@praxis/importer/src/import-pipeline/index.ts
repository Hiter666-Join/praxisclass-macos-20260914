/**
 * High-level entry: parsed pptxtojson(-pro) JSON → PraxisClass canvas `Slide[]`.
 *
 ...
 * whole import either.
 */
import { normalizeSlideWith, type Slide } from '@praxis/dsl';
...
/**
 * Convert a pre-parsed pptxtojson(-pro) `Output` JSON into PraxisClass slides.
 *
...
const normalizeImportedSlide = normalizeSlideWith({
  ...
  onDropped: (_element, error) => {
    console.warn(
      `[@praxis/importer] dropping element that fails DSL normalization: ${
...
/**
 * Parse a .pptx file and convert it into PraxisClass canvas slides.
 *
