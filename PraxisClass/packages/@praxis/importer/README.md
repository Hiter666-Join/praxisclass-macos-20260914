...
**PraxisClass 扩展**：在 `parse()`（PPTX → 中间 JSON）之上新增了完整的 import pipeline —— `importPptx()` 直接吃 `.pptx` 文件、吐 PraxisClass 画布需要的 `Slide[]`，并可选地把所有 base64 图片 / blob 媒体转交给你提供的 OSS 上传函数。详见下方「🚀 进阶用法」。
...
# 🚀 进阶用法：PPTX → PraxisClass 画布 `Slide[]`

`parse()` 只完成「PPTX → 可读 JSON」这一步，元素的字段还是 PPT 语义（`type: 'text' | 'shape' | 'image' …`、单位 `pt`、媒体是 base64 / `blob:` URL）。

如果你需要的最终产物是 **PraxisClass 画布直接可渲染的 `Slide[]`**（`PPTTextElement` / `PPTShapeElement` / `PPTImageElement` 等、单位 `px`、媒体替换为 OSS URL），用 `importPptx()`。
...
| `CanvasSlide` | PraxisClass `Slide` 类型 | 用于消费方做类型注解 |
...
```ts
import { importPptx } from '@praxis/importer';
...
```ts
import { importPptx, type OssUpload } from '@praxis/importer';
...
```ts
import { parse, parsedToSlides } from '@praxis/importer';
...
`praxis-importer` 源码依赖 `pdfjs-dist`，其动态 `require()` 模式会被 Turbopack 拒绝。PraxisClass 的做法：

1. `pnpm run build` 把整个包（含 importPptx）打成 `dist/`。
2. `scripts/sync-praxis-importer.mjs` 把 `dist/` 复制到 `public/vendor/praxis-importer/`。
3. 在客户端组件里用**静态 URL 动态 import**，bundler 完全看不到：

```ts
import type * as PptxtojsonPro from '@praxis/importer';

const mod = (await import(
  /* webpackIgnore: true */
  /* turbopackIgnore: true */
  /* @vite-ignore */
  '/vendor/praxis-importer/index.js'
...
`public/vendor/praxis-importer/` 是 **gitignored 的构建产物**，不进仓库，由 `postinstall`
现生成（`pnpm --filter @praxis/importer build` + `node scripts/sync-praxis-importer.mjs`）。
因此部署流水线**必须执行 `postinstall`**（或显式跑这两步），否则运行时
`/vendor/praxis-importer/index.js` 会 404，PPTX 导入功能失效。

两道防护已就位：

- **构建期断言**：根 `build` 脚本前置 `node scripts/assert-vendor-praxis-importer.mjs`，
