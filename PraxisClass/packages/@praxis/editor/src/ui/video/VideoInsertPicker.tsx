...
export function VideoInsertPicker({ labels, onInsert }: VideoInsertPickerProps) {
  ...
  return (
    <div className="praxis-editing-ui-video-insert-picker">
      <button
        type="button"
        className="praxis-editing-ui-video-dropzone"
      ...
      <input
        ...
        type="file"
        accept="video/*"
        className="praxis-editing-ui-visually-hidden"
        onChange={(event) => void insertFile(event.target.files?.[0])}
      />
      <div className="praxis-editing-ui-video-or">{labels.videoOr}</div>
      <div className="praxis-editing-ui-video-url-row">
        <input
          type="url"
          value={url}
