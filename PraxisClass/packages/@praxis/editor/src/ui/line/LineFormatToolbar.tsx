...
import type { LinePoint, LineStyleType, PPTLineElement } from '@praxis/dsl';
...
export function LineFormatToolbar({
  ...
  return (
    <div
      className={['praxis-editing-ui-root', 'praxis-editing-ui-line-toolbar', className]
        .filter(Boolean)
        .join(' ')}
      role="toolbar"
    ...
    >
      <select
        className="praxis-editing-ui-select praxis-editing-ui-line-select"
      ...
      <button
        ref={colorButtonRef}
        type="button"
        className="praxis-editing-ui-icon-button praxis-editing-ui-color-button"
      ...
      >
        <span
          className="praxis-editing-ui-color-button-preview"
          aria-hidden="true"
          style={{ backgroundColor: element.color }}
        />
      </button>
      <select
        className="praxis-editing-ui-select praxis-editing-ui-line-width-select"
      ...
      {(['start', 'end'] as const).map((position, index) => (
        <select
          key={position}
          className="praxis-editing-ui-select praxis-editing-ui-line-marker-select"
      ...
      {onBringToFront || onSendToBack || onDelete ? (
        <>
          <div className="praxis-editing-ui-divider" aria-hidden="true" />
          {onBringToFront ? (
            <button
              type="button"
              className="praxis-editing-ui-icon-button"
          ...
          {onDelete ? (
            <button
              type="button"
              className="praxis-editing-ui-icon-button praxis-editing-ui-delete-button"
      ...
      {isColorPickerOpen && typeof document !== 'undefined'
        ? createPortal(
            <div
              ref={colorPopoverRef}
              className="praxis-editing-ui-root praxis-editing-ui-color-popover praxis-editing-ui-color-popover-overlay"
              role="dialog"
              aria-label={labels.color}
