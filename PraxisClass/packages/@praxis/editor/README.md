...
Composable slide-editing package for PraxisClass.

- `@praxis/editor/core`: document operations, transactions, and undo/redo history.
- `@praxis/editor/react`: editable slide interaction surface and rich-text editors.
- `@praxis/editor/ui`: editor toolbars, insertion controls, and context menus.

## Dependencies

```text
@praxis/editor
├─> @praxis/renderer   (reuses the read-only slide renderer)
└─> @praxis/dsl        (document and element data contracts)
```

`@praxis/renderer` does not depend on `@praxis/editor`, so the package boundary remains
...
`onTransaction` sink. `@praxis/editor` owns built-in element adapters, insertion defaults,
...
```bash
pnpm add @praxis/editor @praxis/dsl @praxis/renderer \
...
`node_modules/@praxis/renderer/dist/**/*.{js,cjs}` as described in the
[`@praxis/renderer` setup](https://www.npmjs.com/package/@praxis/renderer#tailwind-4-setup).
...
import type { SlideContent } from '@praxis/dsl';
import { applyEditorTransaction, createEditorHistory, type EditorTransaction } from '@praxis/editor/core';
import { EMPTY_SELECTION, type Selection } from '@praxis/editor/react';
import { EditableSlideCanvasWithUI, type EditorInsertItem } from '@praxis/editor/ui';
