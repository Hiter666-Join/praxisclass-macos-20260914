...
### Record real test runs in the platform dashboard

After an explicit learner submission finishes running the frozen test cases, post ONE batch
to the host. Do not post from page initialization, animation, hints, or example-output rendering.
Python algorithm exercises use suite `case_a_algo`; other code exercises use `code_practice`.
Use stable case IDs across course versions. Reuse the same attempt ID when retrying delivery;
mint a new ID only for a new learner submission. Run hidden cases as well, but show only their
IDs and pass/fail, never their input or expected answer. Report actual execution results only.
...
window.parent.postMessage({
  ...
  results: testResults.map(result => ({
    ...
    failureReason: result.passed ? undefined : result.failureReason
  }))
}, '*');
```

The host supplies the classroom and anonymous learner identity. Never send identities,
cookies, credentials, submitted code, or hidden-case answers in this message.
This is a teaching feedback channel, not a trusted examination grading service.
