...
Help the user choose how PraxisClass should run before starting anything, then establish the resulting `<PRAXIS_BASE_URL>`.
...
After startup, set `<PRAXIS_BASE_URL>` to the URL printed by the selected startup mode and verify:

```text
GET <PRAXIS_BASE_URL>/api/health
```

If the skill config provides a custom `url`, use that operator-supplied value instead.
