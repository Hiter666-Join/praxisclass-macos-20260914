/**
 * PraxisClass Agent — pi StreamFn adapter (promoted from PoC).
 *
 * Bridges the pi agent loop's LLM call to PraxisClass's existing AI-SDK-based
 ...
 * call through PraxisClass's resolved Vercel `LanguageModel`, then map the AI SDK
...
/** Build a pi `StreamFn` that calls PraxisClass's connector instead of pi-ai providers. */
...
export function toModelMessages(
  ...
  for (const m of messages) {
    if (m.role === 'user') {
      const content =
        typeof m.content === 'string'
          ...
          : m.content.map((c) => c.type === 'text'
              ...
              : { type: 'image' as const, image: c.data, mediaType: c.mimeType });
      ...
      // Preserve preview evidence at the transport boundary. `details` remains
      // UI metadata; tools must put model-relevant structured data in content.
      const output = hasImage && !m.isError
        ? { type: 'content', value: m.content.map((c) => c.type === 'text'
            ? { type: 'text', text: c.text }
            : { type: 'image-data', data: c.data, mediaType: c.mimeType }) }
        : { type: m.isError ? 'error-text' : 'text', value: text };
      out.push({
        role: 'tool',
        content: [
          {
