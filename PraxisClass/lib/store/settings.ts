 ...
 * Settings Store
 *
 * Independent teacher/student settings, persisted under separate keys through
 * the `@praxis/storage` KVStore in the `account` scope. Role changes select a
 * profile; they never copy credentials or model selections between profiles.
 */

import { create } from 'zustand';
import { useSettingsMode } from './settings-mode';
import { persist } from 'zustand/middleware';
import type { ProviderId } from '@/lib/ai/providers';
import type { DifySettings, PlatformMode, ProvidersConfig } from '@/lib/types/settings';
...
import { serviceConfigurationIssue } from '@/lib/settings/service-activation';
...
export interface SettingsState {
  ...
  // Knowledge base (Dify) connection override; blank fields use the server env
  dify: DifySettings;
  ...
  // Which platform surface the shell renders. UI only — never an authorization
  // input; the server decides that from the teacher cookie.
  mode: PlatformMode;
  ...
  setDify: (partial: Partial<DifySettings>) => void;
...
const getDefaultAudioConfig = () => ({
  ...
  ttsProvidersConfig: {
    ...
    'xfyun-tts': { apiKey: '', baseUrl: '', enabled: true },
...
function ensureValidProviderSelections(state: Partial<SettingsState>): void {
  // Retired built-ins must not reappear from older teacher/student settings.
  for (const [config, id] of [
    [state.providersConfig, 'lemonade'],
    [state.ttsProvidersConfig, 'lemonade-tts'],
    [state.asrProvidersConfig, 'lemonade-asr'],
    [state.imageProvidersConfig, 'lemonade'],
  ] as const) {
    if (config) delete (config as Record<string, unknown>)[id];
  }
  if ((state.providerId as string) === 'lemonade') {
    state.providerId = '' as ProviderId;
    state.modelId = '';
  }
  if ((state.ttsProviderId as string) === 'lemonade-tts') {
    state.ttsEnabled = false;
    state.ttsVoice = 'default';
  }
  if ((state.asrProviderId as string) === 'lemonade-asr') {
    state.asrEnabled = false;
    state.asrLanguage = 'zh';
  }
  if ((state.imageProviderId as string) === 'lemonade') {
    state.imageModelId = '';
...
function createSettingsProfile(profileMode: PlatformMode) {
const recovery: { rehydrate?: () => void | Promise<void> } = {};
const profile = create<SettingsState>()(
  persist(
    (set, get) => {
      const defaultAudioConfig = getDefaultAudioConfig();
      const defaultPDFConfig = getDefaultPDFConfig();
      ...
      return {
        ...
        // Knowledge base (Dify) — blank means "use the server env configuration"
        dify: { baseUrl: '', datasetId: '', apiKey: '' },
        ...
        setMode: (mode) => useSettingsMode.getState().setMode(mode),
        ...
        setImageGenerationEnabled: (enabled) => {
          if (enabled) {
            const state = get();
            if (serviceConfigurationIssue(state, 'image', state.imageProviderId) || !state.imageModelId) return;
          }
        ...
        setVideoGenerationEnabled: (enabled) => {
          if (enabled) {
            const state = get();
            if (serviceConfigurationIssue(state, 'video', state.videoProviderId) || !state.videoModelId) return;
          }
        ...
        setDify: (partial) => set((state) => ({ dify: { ...state.dify, ...partial } })),

        // Fetch server-configured providers and merge into local state
        fetchServerProviders: async () => {
          try {
            const res = await fetch(`/api/server-providers?scope=${profileMode}`);
            if (!res.ok) return;
            ...
            set((state) => {
              ...
              for (const pid of Object.keys(newProvidersConfig)) {
                ...
                if (newProvidersConfig[key]) {
                  newProvidersConfig[key] = {
                    ...newProvidersConfig[key],
                    ...(newProvidersConfig[key].serverModels !== undefined && !data.providers[pid]?.models
              ...
              for (const [pid, info] of Object.entries(data.providers)) {
                ...
                if (newProvidersConfig[key]) {
                  ...
                  const filteredModels = info.models
                    ? info.models.map((id) => {
              ...
              for (const pid of Object.keys(newImageConfig)) {
                ...
                if (newImageConfig[key]) {
                  newImageConfig[key] = {
                    ...newImageConfig[key],
                    ...(newImageConfig[key].isServerConfigured && newImageConfig[key].replaceBuiltInModels && !data.image[pid]?.models
              ...
              for (const pid of Object.keys(newVideoConfig)) {
                ...
                if (newVideoConfig[key]) {
                  newVideoConfig[key] = {
                    ...newVideoConfig[key],
                    ...(newVideoConfig[key].isServerConfigured && newVideoConfig[key].replaceBuiltInModels && !data.video?.[pid]?.models
                      ? { customModels: [], replaceBuiltInModels: false }
                      : {}),
                    isServerConfigured: false,
                    serverDisabled: false,
    ...
    {
      // Preserve existing settings for the teacher; students have a separate record.
      name: profileMode === 'teacher' ? 'settings-storage' : 'settings-storage-student',
      ...
      migrate: (persistedState: unknown, version: number) => {
        ...
        if (!state.dify) {
          state.dify = { baseUrl: '', datasetId: '', apiKey: '' };
        }
      ...
      merge: (persistedState, currentState) => {
        ...
        const merged = { ...currentState, ...persisted, mode: profileMode };
...
recovery.rehydrate = () => profile.persist.rehydrate();
return profile;
}

const profiles = { teacher: createSettingsProfile('teacher'), student: createSettingsProfile('student') };

// Each profile owns its in-flight saves and hydration; switching never copies state.
function useProfileSettings(_target: unknown, _thisArg: unknown, args: unknown[]) {
    const mode = useSettingsMode((state) => state.mode);
    return Reflect.apply(profiles[mode], undefined, args);
}

export const useSettingsStore = new Proxy<typeof profiles.teacher>(profiles.teacher, {
  apply: useProfileSettings,
  get(_target, property) {
    if (property === 'subscribe') {
      return (listener: (state: SettingsState, previous: SettingsState) => void) => {
        let previous = profiles[useSettingsMode.getState().mode].getState();
        const onChange = (next: SettingsState, prev: SettingsState) => {
          previous = next;
          listener(next, prev);
        };
        let unsubscribe = profiles[useSettingsMode.getState().mode].subscribe(onChange);
        const unsubscribeMode = useSettingsMode.subscribe(({ mode }) => {
          unsubscribe();
          const next = profiles[mode].getState();
          listener(next, previous);
          previous = next;
          unsubscribe = profiles[mode].subscribe(onChange);
        });
        return () => { unsubscribe(); unsubscribeMode(); };
      };
    }
    return Reflect.get(profiles[useSettingsMode.getState().mode], property);
  },
});

export async function resetCurrentSettingsProfile(): Promise<void> {
  const mode = useSettingsMode.getState().mode;
  const profile = profiles[mode];
  if (!profile.persist.hasHydrated()) await profile.persist.rehydrate();
  const initial = Object.fromEntries(Object.entries(profile.getInitialState()).filter(([, value]) => typeof value !== 'function'));
  const clean = { ...JSON.parse(JSON.stringify(initial)), mode, providerId: '', modelId: '', imageProviderId: '', imageModelId: '', videoProviderId: '', videoModelId: '', ttsEnabled: false, asrEnabled: false };
  profile.setState(clean);
  const { storage, name } = profile.persist.getOptions();
  if (!storage || !name) throw new Error('Settings storage unavailable');
  await storage.setItem(name, { state: clean, version: SETTINGS_PERSIST_VERSION });
}
