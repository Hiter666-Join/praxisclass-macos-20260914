...
function publishSaveStatus(): void {
  useStageSaveStatus.setState({
    stageId: pendingStageId,
    dirty: pendingChanges.size > 0,
    saving: !!flushInFlight && flushInFlight.stageId === pendingStageId,
    failed: consecutiveFlushFailures > 0,
  });
}

export function hasPendingStageSave(stageId: string): boolean {
  return pendingStageId === stageId && pendingChanges.size > 0;
}
  ...
  loadFromStorage: async (stageId: string, loadToken?: StageSceneLoadToken) => {
    try {
      ...
      if (data) {
        ...
        const generationComplete = outlinesRecord?.producer === 'server-job'
          ? persistedComplete
          :
          persistedComplete ||
          // ZIP v1 imported only stage/scenes. Preserve an explicit incomplete
          // outline record; only upgrade the older outline-less archive shape.
          (!outlinesRecord && migrated.length > 0) ||
          isDeckComplete({
            outlines,
            scenes: migrated,
            failedOutlines,
