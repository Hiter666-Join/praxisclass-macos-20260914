...
// pdfjs-dist (transitively pulled by `praxis-importer/src`) uses dynamic
...
import type * as PraxisImport from '@praxis/importer';

const log = createLogger('ImportPptx');

export type PptxUpload = NonNullable<PraxisImport.ImportPptxOptions['upload']>;
 ...
 * the bundled `praxis-importer` dist that we load by URL to bypass
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        // `scripts/sync-praxis-importer.mjs` copies the prebuilt dist into
        // `public/vendor/` after every `pnpm install`.
        const url = '/vendor/praxis-importer/index.js';
