...
import { restoreCourseSources, removeImportedSources } from '@/lib/export/source-materials';
...
export function useImportClassroom(onSuccess?: (importedStageId: string) => void) {
  ...
  const handleFileChange = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (!(name.endsWith('.praxis.zip') || name.endsWith('.maic.zip'))) {
        toast.error(t('import.error.invalidZip'));
        return;
      }
      ...
      try {
        ...
        // 5. Write course data
        const sourceMaterials = await restoreCourseSources(zip, manifest.sourceMaterials, importedSourceIds);
        ...
        const document: AppDocument = {
          outline: {
            outlines: [],
            generationComplete: manifest.scenes.length > 0 && manifest.generationComplete !== false,
            ...(sourceMaterials.length ? { sourceMaterials } : {}),
            createdAt: now,
            updatedAt: now,
          },
          stage: {
        ...
        if (!importCommitted && importedStageId) {
          ...
          await cleanup('source materials', () => removeImportedSources(importedSourceIds));
        }
        setImporting(false);
        setPhase('idle');
      }
