...
export async function persistQuizRetry(
  ...
  readAttempt: typeof loadQuizAttemptState = loadQuizAttemptState,
): Promise<string> {
  ...
  const loaded = await readAttempt(input);
  if (!loaded.state || loaded.state.status !== 'active' || loaded.state.phase !== 'draft') {
    throw new QuizRetryProgressedError(loaded.state?.sessionId ?? input.attemptId);
  }
  return loaded.state.sessionId;
}
