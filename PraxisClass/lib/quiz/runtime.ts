...
import { RuntimeAppendConflictError, type RuntimeStore } from '@praxis/storage';
...
async function withAttemptLock<T>(attemptId: string, work: () => Promise<T>): Promise<T> {
  if (typeof navigator !== 'undefined' && navigator.locks) {
    return navigator.locks.request(`praxis:quiz-attempt:${attemptId}`, work);
  }
  return work();
}
...
async function migrateLegacyQuizState(
  ...
  // Legacy scene-only snapshots belong to the existing student partition.
  // Rehearsal must neither import nor clear them.
  if (isDemoLearner(learnerKey)) return;
            ...
            if (payload.phase === 'reviewed') {
              mirrorMany(
                (payload.results ?? [])
                  ...
                  .map((result) => ({
                    suite: `quiz_${input.stageId}`,
                    caseId: result.questionId,
                    stageId: input.stageId,
                    sceneId: input.sceneId,
                    learnerKey,
                    attemptId: sessionId,
