...
function validateAgentRuntime(): void {
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    warn(
      'PRAXIS_AGENT_RUNTIME_ENABLED is set but DATABASE_URL is not — the agent runtime is enabled but unusable: its probe reports disabled, its routes answer 404, and no runner starts. Set DATABASE_URL or disable the flag.',
    );
  }
}
