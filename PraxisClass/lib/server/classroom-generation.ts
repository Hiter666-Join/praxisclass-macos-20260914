...
import { GenerationQualityError, repairInteractiveContent } from '@/lib/server/generation-content-quality';
...
export async function generateClassroom(
  ...
  const { outlines } = await reviewGenerationPlan(outlinesResult.data.outlines, requirements, aiCall, { documentText: pdfText, researchContext });
  log.info(
  ...
  const failWithDraft = async (error: GenerationQualityError): Promise<never> => {
    const partial = store.getState().scenes;
    if (partial.length) {
      const draft = await persistClassroom({ id: stageId, stage, scenes: partial }, options.baseUrl);
      error.message += `。已生成的 ${partial.length} 页保留在 ${draft.url}`;
    }
    throw error;
  };
  ...
  for (const [index, outline] of outlines.entries()) {
    ...
    const vocationalContext = buildVocationalGenerationContext(requirements, safeOutline, outlines);
    const goalCall = withGenerationRequirementCall(contentCall.aiCall, requirements);
    const pageCall: AICallFn = (system, user, ...rest) => goalCall(system, withVocationalGenerationContext(user, vocationalContext), ...rest);
    const content = await (async () => {
      try {
        return await withGenerationRetry(
          () =>
            generateSceneContent(safeOutline, pageCall, {
              agents,
              languageDirective,
              ...
              ...(safeOutline.type === 'pbl'
                ...
                : {}),
            }).then(async generated => generated && 'html' in generated
              ? repairInteractiveContent(generated, safeOutline, pageCall)
              : generated),
          {
        ...
        if (error instanceof GenerationQualityError) return failWithDraft(error);
        return containPBLGenerationError(error, safeOutline.title);
      }
    })();
    if (!content) {
      return failWithDraft(new GenerationQualityError([`第 ${index + 1} 页「${safeOutline.title}」生成失败，课程尚未完成`]));
    }

    const actionsAiCall = await getSceneActionsAiCall();
    const actionsGoalCall = withGenerationRequirementCall(actionsAiCall, requirements);
    const actions = await withGenerationRetry(
      () =>
        generateSceneActions(safeOutline, content, (system, user, ...rest) => actionsGoalCall(system, withVocationalGenerationContext(user, vocationalContext), ...rest), {
          agents,
          languageDirective,
        }),
      {
    ...
    if (!sceneId) {
      return failWithDraft(new GenerationQualityError([`第 ${index + 1} 页「${safeOutline.title}」组装失败，课程尚未完成`]));
    }
