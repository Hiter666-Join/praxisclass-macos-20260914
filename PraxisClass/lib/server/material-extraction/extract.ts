...
import { getRuntimeServiceSettings, usesPersonalProviderConfig } from '@/lib/server/provider-request-context';
...
function extractorCandidates(
  ...
  if (getRuntimeServiceSettings()) {
    return supported.filter(provider => provider.id === 'plain-text' || configured.has(provider.id));
  }
...
export async function extractClaimedSessionMaterial(
  ...
  if (isMedia) {
    if (getRuntimeServiceSettings() && !getRuntimeServiceSettings()?.asr) {
      throw new MaterialExtractionError('请在本端设置中启用语音识别后解析音视频资料。', false);
    }
    const mediaInput = {
      buffer: raw.bytes,
    ...
    try {
      artifact = await provider.extract({
        ...
        config: {
          ...
          accessKeyId: getRuntimeServiceSettings()?.pdf?.accessKeyId,
          accessKeySecret: getRuntimeServiceSettings()?.pdf?.accessKeySecret,
