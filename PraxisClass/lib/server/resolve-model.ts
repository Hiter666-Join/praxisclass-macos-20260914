...
import { getRuntimeServiceSettings, runWithProviderRequest, usesPersonalProviderConfig, withPersonalProviderConfig } from './provider-request-context';
...
export async function resolveModel(params: {
  ...
  const runtime = getRuntimeServiceSettings();
  if (runtime) {
    const selected = runtime.llm;
    params = { stage: params.stage, modelString: `${selected.providerId}:${selected.modelId}`,
      apiKey: selected.apiKey, baseUrl: selected.baseUrl, providerType: selected.providerType,
      thinkingConfig: selected.thinkingConfig };
  }
  if (params.apiKey && !usesPersonalProviderConfig()) {
    return withPersonalProviderConfig(() => resolveModel(params));
  }
  ...
  // vendor default.
  const stageRoute = usesPersonalProviderConfig() ? undefined : getStageRoute(params.stage);
  const stageModel = stageRoute?.model;
  const modelString = stageModel || params.modelString || (usesPersonalProviderConfig() ? undefined : process.env.DEFAULT_MODEL);
  if (!modelString) {
    throw new Error(
...
export async function resolveModelFromHeaders(
  ...
  return runWithProviderRequest(req, () => resolveModel({
