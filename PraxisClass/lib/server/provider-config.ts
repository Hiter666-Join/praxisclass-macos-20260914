...
import { getRuntimeServiceSettings, usesPersonalProviderConfig } from './provider-request-context';
import type { ServiceSelection } from '@/lib/types/service-settings';
import { encodeXfyunCredentials, hasXfyunCredentials } from '@/lib/audio/xfyun-config';
...
function loadEnvSection(
  ...
  if (yamlSection) {
    for (const [id, entry] of Object.entries(yamlSection)) {
      if (['lemonade', 'lemonade-tts', 'lemonade-asr'].includes(id)) continue;
      if (
...
export function resolveServerMediaExtractorConfig(): {
  ...
  if (getRuntimeServiceSettings()) return { providerId: 'local-media', allowEnvFallback: false };
  ...
  return {
    ...
    allowEnvFallback: !usesPersonalProviderConfig(),
  };
}
...
function buildConfig(yamlData: YamlData): ServerConfig {
  ...
  // Support three console fields in env, or one atomic APPID:APIKey:APISecret
  // in the existing YAML apiKey field. Never combine partial credential sources.
  const xfyunAppId = process.env.TTS_XFYUN_APP_ID?.trim();
  const xfyunSecret = process.env.TTS_XFYUN_API_SECRET?.trim();
  if (xfyunAppId || xfyunSecret) {
    tts['xfyun-tts'] = {
      ...tts['xfyun-tts'],
      apiKey: encodeXfyunCredentials({
        appId: xfyunAppId || '',
        apiKey: process.env.TTS_XFYUN_API_KEY || '',
        apiSecret: xfyunSecret || '',
      }),
    };
  }
  if (tts['xfyun-tts'] && !hasXfyunCredentials(tts['xfyun-tts'].apiKey)) {
    delete tts['xfyun-tts'];
  }
  ...
  const providers = applyBedrockProviderConfig(
    loadEnvSection(LLM_ENV_MAP, yamlData.providers, {
      keylessProviders: new Set(['ollama', BEDROCK_PROVIDER_ID]),
    }),
    yamlData.providers,
  );
  ...
  return {
    ...
    asr: loadEnvSection(ASR_ENV_MAP, yamlData.asr, {
      keylessProviders: new Set(['funasr-asr']),
    }),
...
function getConfig(): ServerConfig {
  if (usesPersonalProviderConfig()) {
    const settings = getRuntimeServiceSettings();
    const entry = (selected?: ServiceSelection): Record<string, ServerProviderEntry> => selected
      ? { [selected.providerId]: { apiKey: selected.apiKey, baseUrl: selected.baseUrl,
          models: selected.modelId ? [selected.modelId] : undefined,
          accessKeyId: selected.accessKeyId, accessKeySecret: selected.accessKeySecret } } : {};
    return { providers: entry(settings?.llm), tts: entry(settings?.tts), asr: entry(settings?.asr),
      pdf: entry(settings?.pdf), image: entry(settings?.image), video: entry(settings?.video), webSearch: entry(settings?.webSearch),
      disabled: { tts: new Set(), asr: new Set(), image: new Set(), video: new Set(), webSearch: new Set() } };
  }
  const cached = _configs.get('');
  if (cached) return cached;
  const config = buildConfig(loadYamlFile(DEFAULT_FILENAME));
...
export function isServerConfiguredProvider(section: ProviderSection, providerId: string): boolean {
  if (usesPersonalProviderConfig()) return false;
...
export function getServerProviders(): Record<string, { models?: string[] }> {
  ...
  for (const [id, entry] of Object.entries(cfg.providers)) {
    result[id] = {};
    if (entry.models) result[id].models = entry.models;
  }
  return result;
}
...
export function resolveQwenVoiceCloneModel(): string {
  if (usesPersonalProviderConfig()) return DEFAULT_QWEN_TTS_VOICE_CLONE_MODEL;
