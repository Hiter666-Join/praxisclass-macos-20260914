...
import { UserSkillError, validateUserSkillInput, type UserSkillFields } from '@praxis/storage';

export const openClawSkillDir = join(process.cwd(), 'skills', 'praxis');
...
export async function buildSkillDirZip(dir: string, root: string): Promise<Buffer | null> {
  ...
  try {
    ...
    for (const file of await walk(dir)) {
      bundle.file(`${root}/${relative(dir, file).split(sep).join('/')}`, await readFile(file));
    }
...
export function buildOpenClawSkillZip(): Promise<Buffer | null> {
  return buildSkillDirZip(openClawSkillDir, 'praxis');
}
