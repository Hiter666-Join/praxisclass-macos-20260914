...
export async function synthesizeSceneNarration(input: SceneTtsInput): Promise<SceneTtsSummary> {
  const selected = getRuntimeServiceSettings()?.tts;
  ...
  const voice =
    bound?.providerId === providerId && bound.voiceId
      ? bound.voiceId
      : selected?.voice || DEFAULT_TTS_VOICES[providerId as keyof typeof DEFAULT_TTS_VOICES] || '';
  const modelId =
    resolveTTSModel(
      providerId,
      selected?.modelId || DEFAULT_TTS_MODELS[providerId as keyof typeof DEFAULT_TTS_MODELS] || '',
      voice,
    ) || '';
  let generated = 0;
  let skipped = 0;
      ...
      const audio = await generateTTS(
        {
          ...
          voice,
          speed: selected?.speed ?? speech.speed,
          ...(selected?.providerOptions ? { providerOptions: selected.providerOptions } : {}),
          signal: input.signal,
        },
        speech.text,
      );
