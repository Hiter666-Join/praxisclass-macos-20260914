...
import { loadRuntimeSettings } from './service-settings';
import { loadRuntimeTeachingMemory } from '@/lib/platform/memory/teaching-context';
import { withRuntimeTeachingMemoryMessages } from './teaching-memory';
import { bindImportedCourseMaterials } from './imported-course-materials';
...
import { assertCurrentStageMutationActive, runStageMutation } from './mutation-fence';
...
import { completeCourseRepairs, courseCompletionFailure } from './course-completion';
import { CourseBriefState, buildCourseBriefTools, withCurrentCourseBrief, COURSE_BRIEF_ENTRY, COURSE_BRIEF_PROMPT } from './course-brief';
import { InteractionChecks, TEST_INTERACTIVE_SCENE_TOOL_NAME, INTERACTION_CHECKPOINT_TYPE, COURSE_CHECKPOINT_TYPE } from './interaction-tools';
import {
...
const DURABLE_USER_MESSAGE_SEQ = 'praxisDurableUserMessageSeq';
...
export async function runSession(ctx: RunContext, meta: ClaimedAgentSession): Promise<void> {
  ...
  try {
    ...
    const serviceSettings = await loadRuntimeSettings(id, meta.ownerId);
    if (!serviceSettings) throw new Error('此任务尚未绑定设置。请在当前端设置中启用服务，然后重新发送以继续断点。');
    await withRuntimeServiceSettings(serviceSettings, async () => {
    const teachingMemory = await loadRuntimeTeachingMemory(serviceSettings.role);
    log.info(`session ${id}: teaching memory ${teachingMemory.status}`);
    ...
    const courseBrief = new CourseBriefState(meta.prompt, async receipt => {
      await flushAll();
      await writeRequiredSessionEntry(async () => {
        await entrySession!.appendCustomEntry(COURSE_BRIEF_ENTRY, receipt);
      }, markLeaseLost);
      abort.signal.throwIfAborted();
    });
    courseBrief.addMessages(loggedMessages.map(message => ({ seq: message.seq, text: toFollowUp(message).text })));
    courseBrief.restore(await entrySession!.getBranch());
    ...
    const resolveFollowUpElementContext = async (
      ...
      if (serviceSettings.role === 'teacher') {
        const courseIds = new Set([
          ...(message.courseRefs ?? []).map((ref) => ref.stageId),
          ...(message.elementRefs ?? []).map((ref) => ref.stageId),
        ]);
        for (const stageId of courseIds) {
          if ((await probeStageAccess(meta.ownerId, stageId)).kind !== 'owned') continue;
          const course = await ownerScopedStore.loadDocument(stageId);
          await bindImportedCourseMaterials(id, meta.ownerId, course?.outline as AppDocumentOutline | undefined);
        }
      }
    ...
    const interactionChecks = new InteractionChecks(
      { store: ownerScopedStore, stageAccess, abortSignal: abort.signal, courseBrief }, undefined,
      async receipt => {
        await flushAll();
        await writeRequiredSessionEntry(async () => {
          await entrySession!.appendCustomEntry(INTERACTION_CHECKPOINT_TYPE, receipt);
        }, markLeaseLost);
      },
    );
    const changedCourses = interactionChecks.restore(await entrySession!.getBranch());
    const checkpoint = (info: CheckpointInfo) => {
      if (info.stageId) changedCourses.add(info.stageId);
      interactionChecks.markChanged(info);
      if (info.stageId) enqueue(async () => {
        await entrySession!.appendCustomEntry(COURSE_CHECKPOINT_TYPE, info);
      }, true);
      emit(LIFECYCLE.checkpoint, info);
    };
    ...
    const dslTools = buildDslCourseToolset({
      ...
      getUserRequirement: () => courseBrief.requirementText(),
      getCourseBrief: () => courseBrief.snapshot(),
      interactionChecks,
    });
    const curriculumTools = buildCurriculumTools({
      ...
      sessionId: id,
      stageAccess,
      onStageLink: (course) => { changedCourses.add(course.stageId); emit(LIFECYCLE.stageLink, course); },
    ...
    if (serviceSettings.role === 'teacher' && meta.stageId && (await stageAccess(meta.stageId)).kind === 'owned') {
      const attachedCourse = await ownerScopedStore.loadDocument(meta.stageId);
      await bindImportedCourseMaterials(id, meta.ownerId, attachedCourse?.outline as AppDocumentOutline | undefined);
    }
    ...
    const briefTools = buildCourseBriefTools(courseBrief);
    const tools = withCurrentCourseBrief(assembleRunnerTools(
      briefTools,
      [askUserTool],
      webSearchTools,
    ...
    const agent = buildAgent({
      ...
      systemPrompt: [
        ...
        teachingMemory.instructions,
        COURSE_BRIEF_PROMPT,
      ].filter(Boolean).join('\n\n'),
      transformContext: async (messages) =>
        withRuntimeTeachingMemoryMessages(messages, teachingMemory),
      model: driver.piModel,
      tools,
      allowedToolNames: new Set([
        ...MINIMAL_AGENT_TOOL_NAMES,
        ...briefTools.map(tool => tool.name),
        ...(search ? ['web_search'] : []),
        'create_skill',
      ...
      afterToolCall: (toolContext) => {
        toolCalls += 1;
        if ((toolContext.toolCall.name === 'list_scenes' && toolContext.isError) || toolContext.toolCall.name === TEST_INTERACTIVE_SCENE_TOOL_NAME) {
          const stageId = (toolContext.toolCall.arguments as { stageId?: unknown }).stageId;
          if (typeof stageId === 'string') changedCourses.add(stageId);
        }
    ...
    const drainMessages = async (): Promise<number> => {
      ...
      if (all.some(message => message.seq > deliveredThrough && !acceptedMessageSeqs.has(message.seq))) {
        const latestSettings = await loadRuntimeSettings(id, meta.ownerId);
        // A changed model/tool selection starts the next claim with a fresh toolset.
        // Do not steer that message into an already-running, differently configured agent.
        if (JSON.stringify(latestSettings) !== JSON.stringify(serviceSettings)) return 0;
      }
      courseBrief.addMessages(all.map(message => ({ seq: message.seq, text: toFollowUp(message).text })));
      const current = await store.getSession(id);
    ...
    try {
      ...
      const canFinishCourse = () => {
        try { courseBrief.assertWritable(); return true; } catch { return false; }
      };
      const completionReports = await completeCourseRepairs({
        inspect: async () => {
          const reports = [];
          for (const stageId of changedCourses) {
            const doc = await ownerScopedStore.loadDocument(stageId);
            if (doc) {
              const report = inspectCourseQuality(doc, activeSkill?.constraints ?? null);
              // Fix syntax/structure first; only executable artifacts enter Chrome.
              const behavior = report.issues.length ? { issues: [], unavailable: [], checked: 0 } : await interactionChecks.inspect(doc);
              reports.push({ ...report, issues: [...report.issues, ...behavior.issues], unavailable: behavior.unavailable,
                structureIssues: report.issues, interactionIssues: behavior.issues, interactionChecked: behavior.checked });
            }
          }
          return reports;
        },
        prompt: text => agent.prompt(text),
        stopped: () => !canFinishCourse() || abort.signal.aborted || questionEmitted || askUserLatch.isCommitted() ||
          Boolean(terminalLoopError(agent.state.messages, agent.state.errorMessage)),
      });
      if (canFinishCourse() && !abort.signal.aborted && !questionEmitted && !terminalLoopError(agent.state.messages, agent.state.errorMessage)) {
        for (const report of completionReports) {
          const { stageId } = report;
          const doc = await ownerScopedStore.loadDocument(stageId);
          const outline = doc?.outline as AppDocumentOutline | undefined;
          if (doc && outline) {
            const structure = report.structureIssues?.length ? 'failed' : 'passed';
            const interaction = structure === 'failed' ? 'not-run' : report.unavailable?.length ? 'unavailable' : report.interactionIssues?.length ? 'failed' : report.interactionChecked ? 'passed' : 'not-required';
            await runStageMutation(abort.signal, () => ownerScopedStore.saveDocument({
              ...doc, outline: { ...outline,
                ...(outline.producerRef === id ? { generationComplete: structure === 'passed' } : {}),
                currentBrief: courseBrief.snapshot(),
                validation: { checkedAt: Date.now(), structure, interaction,
                  issues: [...report.issues, ...(report.unavailable ?? [])], teacherReview: 'pending' },
                updatedAt: Date.now() },
            }));
            checkpoint({ tool: 'list_scenes', stageId,
              detail: `课程草稿已保存；结构检查${structure === 'passed' ? '通过' : '待修正'}；互动${interaction === 'passed' ? '所选用例通过' : interaction === 'unavailable' ? '执行条件缺失，未验证' : interaction === 'failed' ? '待修正' : interaction === 'not-run' ? '尚未检查' : '无本次需检查的互动页'}；教学内容待教师审阅，学习成效以实际学生成果为准。`,
              skillViolations: [...report.issues, ...(report.unavailable ?? [])] });
          }
        }
      }
      ...
      const error = !settledCancelled ? loopError || courseCompletionFailure(completionReports) : undefined;
      const status = settledCancelled ? 'cancelled' : error ? 'failed' : 'succeeded';
      emit(LIFECYCLE.sessionEnd, { status, toolCalls, ...(error ? { error } : {}),
        ...(!settledCancelled && completionReports.some(report => report.unavailable?.length)
          ? { note: '课程草稿已保存；互动执行条件缺失，相关行为仍未验证。' } : {}) });
      await flushAll();
      const settled = await store.finishSession(id, WORKER_ID, {
        status,
        ...(error ? { error } : {}),
