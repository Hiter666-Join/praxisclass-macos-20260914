 ...
 *
 * The tool follows the active settings snapshot. When search is enabled and
 * configured, the agent decides when to call it; otherwise it is not registered.
 */
...
export interface WebSearchCapability {
  ...
  apiKey: string;
  baseUrl?: string;
  baiduSubSources?: Parameters<typeof searchWeb>[0]['baiduSubSources'];
...
export function resolveWebSearchCapability(): WebSearchCapability | null {
  ...
  const selected = getRuntimeServiceSettings()?.webSearch;
  const config = resolveClassroomWebSearchConfig({ baiduSubSources: selected?.baiduSubSources });
