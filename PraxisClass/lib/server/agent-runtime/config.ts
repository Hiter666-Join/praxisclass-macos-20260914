...
export const agentRuntimeConfig = {
  ...
  scanIntervalMs: numberFromEnv(process.env.PRAXIS_AGENT_RUNTIME_SCAN_INTERVAL_MS, 1000),
  /** How often a running session's lease heartbeat is refreshed. */
  heartbeatIntervalMs: numberFromEnv(process.env.PRAXIS_AGENT_RUNTIME_HEARTBEAT_MS, 2000),
  /**
   ...
   */
  leaseTtlMs: numberFromEnv(process.env.PRAXIS_AGENT_RUNTIME_LEASE_TTL_MS, 10_000),
  /** Maximum sessions one application instance runs concurrently. */
  maxConcurrent: numberFromEnv(process.env.PRAXIS_AGENT_RUNTIME_MAX_CONCURRENT, 2),
  /** Maximum consecutive unattended starts or resumptions. */
  maxAttempts: numberFromEnv(process.env.PRAXIS_AGENT_RUNTIME_MAX_ATTEMPTS, 5),
  /**
   * Native conversation compaction is opt-in at this layer: it runs only when
   * PRAXIS_AGENT_COMPACTION_ENABLED=true. This is a deliberate inversion of
   ...
   */
  compaction: {
    enabled: process.env.PRAXIS_AGENT_COMPACTION_ENABLED === 'true',
    ...(process.env.PRAXIS_AGENT_COMPACTION_RESERVE_TOKENS
      ? {
          reserveTokens: numberFromEnv(process.env.PRAXIS_AGENT_COMPACTION_RESERVE_TOKENS, 0),
        }
      : {}),
    ...(process.env.PRAXIS_AGENT_COMPACTION_KEEP_RECENT_TOKENS
      ? {
  ...
  skillsDir: process.env.PRAXIS_AGENT_SKILLS_DIR ?? `${process.cwd()}/skills/agent-runtime`,
  /** Audio/video upload safety ceiling; defaults to the same 50 MiB cap as documents/images. */
  maxUploadBytes: numberFromEnv(process.env.PRAXIS_AGENT_MAX_UPLOAD_BYTES, 50 * 1024 * 1024),
