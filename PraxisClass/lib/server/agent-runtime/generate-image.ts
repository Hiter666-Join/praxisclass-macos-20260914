...
import { usesPersonalProviderConfig } from '@/lib/server/provider-request-context';
...
function selectProvider(
  ...
  const requested = usesPersonalProviderConfig() ? undefined : process.env.DEFAULT_IMAGE_PROVIDER?.trim();
...
export function buildGenerateImageTool(
  ...
  return {
    ...
    async execute(toolCallId, params: Static<typeof GenerateImageParams>, signal) {
      ...
      const requestedDefault = usesPersonalProviderConfig() ? undefined : process.env.DEFAULT_IMAGE_PROVIDER?.trim();
      if (!providerId) {
        if (requestedDefault) {
          log.warn(
