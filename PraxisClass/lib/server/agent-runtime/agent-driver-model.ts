...
export const AGENT_DRIVER_STAGE = 'praxis-agent-driver' as const;
...
export async function resolveAgentDriverModel(): Promise<{
  ...
  if (getRuntimeServiceSettings()) {
    ...
    return {
      connection,
      // Transport is provided by the shared AI SDK adapter, not Pi's provider registry.
      piModel: buildPiDriverModel(connection, 'openai-completions'),
      wireMaxOutputTokens,
