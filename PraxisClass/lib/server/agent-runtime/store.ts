...
import { extractObservedUrls } from '@praxis/storage';
import type { Pool } from 'pg';

import { getServerPersistenceProvider } from '@/lib/persistence/server-provider';
import { notifyDurableAgentEvent } from './event-notify-bus';
import { ensureRuntimeSettingsSchema, saveRuntimeSettings } from './service-settings';
...
async function createAgentSessionStore(connectionString: string): Promise<PgAgentSessionStore> {
  ...
  await ensureRuntimeSettingsSchema(pool);
  ...
  const store: PgAgentSessionStore = new PgAgentSessionStore(pool, {
    ...
    onSessionCreated: async (transaction, meta): Promise<void> => {
      ...
      if (settings) await saveRuntimeSettings(transaction, meta.id, meta.ownerId, settings);
    ...
    onUserMessagePosted: async (transaction, input): Promise<void> => {
      const settings = getRuntimeServiceSettings();
      if (settings) await saveRuntimeSettings(transaction, input.session.id, input.session.ownerId, settings);
      await store.registerSessionUrls(
        input.session.id,
