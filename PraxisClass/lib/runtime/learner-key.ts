 ...
 */
import { BrowserKVStore, type KVStore } from '@praxis/storage';
...
const LEARNER_KEY_LOCK = 'praxis:learner-key';
 ...
 */
export function getDeviceLearnerKey(kv?: KVStore): Promise<string> {
...
/** Teacher rehearsal has its own runtime partition; student keys retain their history. */
export async function getLearnerKey(kv?: KVStore): Promise<string> {
  const demo = typeof window !== 'undefined' && useSettingsMode.getState().mode === 'teacher';
  const key = await getDeviceLearnerKey(kv);
  return demo ? demoLearnerKey(key) : key;
}
