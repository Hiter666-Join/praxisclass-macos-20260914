...
import { hasXfyunCredentials } from './xfyun-config';
 ...
 * local provider (e.g. VoxCPM) must be shown only once the operator
...
export function isTTSProviderConfigured(
  ...
  if (!def && !isCustomTTSProvider(providerId)) return false;
  if (config.isServerConfigured) return true;
  if (providerId === 'xfyun-tts') return hasXfyunCredentials(config.apiKey);
  ...
  // Keyless built-in (VoxCPM): require an EXPLICIT base URL.
