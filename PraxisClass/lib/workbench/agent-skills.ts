...
interface RegistrySnapshot {
  /** False for library-only deployments; personal mutations require the runner. */
...
function loadAgentSkills(force = false): Promise<AgentSkillInfo[]> {
  ...
  skillsRequest = fetch('/api/agent/skills')
    .then(async (res) => {
      ...
      return {
        list: (await res.json()) as AgentSkillInfo[],
        runtimeReady: res.headers?.get('X-Agent-Runtime-Ready') !== 'false',
      };
    })
    .then(({ list, runtimeReady }) => {
      ...
      skillsCache = list;
      publish({ skills: list, loading: false, error: null, runtimeReady });
      return list;
    })
    .catch((error: unknown) => {
      ...
      publish({ skills: snapshot.skills, loading: false, error: copyKey, runtimeReady: false });
      throw error;
    })
    .finally(() => {
      skillsRequest = null;
