...
// Next may load this module in more than one route bundle or reload it in dev.
// Reuse the carrier so a new module cannot detach an in-flight caller's store.
// AsyncLocalStorage still isolates the values belonging to each caller.
const globals = globalThis as typeof globalThis & {
  __thinkingContext?: AsyncLocalStorage<ThinkingConfig | undefined>;
};
export const thinkingContext = (globals.__thinkingContext ??= new AsyncLocalStorage<
