 ...
 *
 * Catalog checked against official API documentation on 2026-09-10.
 * Sources:
...
import { getThinkingMode, pickThinkingBudget, pickThinkingEffort } from './thinking-config';
...
export const PROVIDERS: Record<ProviderId, ProviderConfig> = {
  ...
  minimax: {
    ...
    name: 'MiniMax',
    type: 'anthropic',
    defaultBaseUrl: 'https://api.minimax.cn/anthropic/v1',
    alternateBaseUrls: [
      { label: 'settings.baseUrlRegion.china', url: 'https://api.minimax.cn/anthropic/v1' },
  ...
  // Spark versions have separate endpoints and/or APIPassword grants. Keep
  // independent provider slots so selecting a model never reuses another key.
  ...
  'xfyun-x2-flash': {
    id: 'xfyun-x2-flash',
    name: 'iFLYTEK Spark X2-Flash',
    type: 'openai',
    defaultBaseUrl: 'https://spark-api-open.xf-yun.com/agent/v1',
    supportsModelDiscovery: false,
    requiresApiKey: true,
    models: [
      {
...
function getCompatThinkingBodyParams(
  ...
  switch (capability.requestAdapter) {
    ...
    case 'xfyun':
      return mode ? { thinking: { type: mode } } : undefined;
      ...
      if (capability.control === 'effort') {
        if (capability.toggleable === false) {
          return {
            ...
            reasoning_effort: pickThinkingEffort(capability, config) ?? capability.defaultEffort,
          };
        }
