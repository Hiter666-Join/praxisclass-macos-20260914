...
import { DocumentVersionError, type DocumentSummary } from '@praxis/storage';
...
function documentSnapshot(
  ...
  return {
    ...
    outline: {
      // Editor snapshots contain page state only. Retain the server producer
      // and uploaded originals so ordinary viewing/editing cannot erase them.
      ...(existingOutline?.producer ? { producer: existingOutline.producer } : {}),
      ...(existingOutline?.producerRef ? { producerRef: existingOutline.producerRef } : {}),
      ...(existingOutline?.sourceMaterials ? { sourceMaterials: existingOutline.sourceMaterials } : {}),
      ...(existingOutline?.sourceContext ? { sourceContext: existingOutline.sourceContext } : {}),
      ...outline,
...
type ThumbnailSlide = import('@praxis/dsl').Slide;
            ...
            for (const el of mediaElements) {
              const videoBinding =
                el.type === 'video'
                  ? resolveVideoMediaForElement(
                      taskEntries,
                      el as import('@praxis/dsl').PPTVideoElement,
                      stageId,
                      documentElements,
                    )
                  : undefined;
              ...
              const selected =
                el.type === 'video'
                  ...
                  : resolveMediaTaskForElement(
                      taskEntries,
                      el as import('@praxis/dsl').PPTElement,
                      stageId,
                    );
...
// `@praxis/storage` DocumentStore. With server persistence on (`listStages`
