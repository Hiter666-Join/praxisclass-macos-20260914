...
(function () {
  ...
  function ensureRoot() {
    ...
    root.setAttribute('data-praxis-element-picker-overlay', '');
  ...
  function isOverlay(element) {
    return !!(element && element.closest && element.closest('[data-praxis-element-picker-overlay]'));
  }
  ...
  function draw() {
    ...
    Array.prototype.slice.call(root.querySelectorAll('[data-praxis-picker-pin]')).forEach(function (node) { node.remove(); });
    selectors.forEach(function (selector, index) {
      var element = null;
      try { element = document.querySelector(selector); } catch (e) {}
      if (!selectable(element)) return;
      var badge = document.createElement('div');
      badge.setAttribute('data-praxis-picker-pin', '');
