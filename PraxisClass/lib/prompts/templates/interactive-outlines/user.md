...
## Task-card annotation

When the requirement describes a job/work-position task or contains knowledge-point lines in the form `- 名称（来源：…｜适用：…）`, append exactly one fenced `taskcard` block to the end of every scene `description`:

```taskcard
{ "工作情境": "...", "规范要点": ["..."], "来源": ["..."], "评价标准": ["..."] }
```

Each block must contain valid JSON, with at most one block per scene. Copy every `来源` entry verbatim from the requirement's `来源：` fragments and never invent a source; if no source fragments exist, use `"来源": []`. Keep the rest of each scene description unchanged.
