...
app-only outline modes. Generation templates live in `@praxis/generation`.
...
- Package-owned generation templates follow the same placeholder conventions;
  edit them under `packages/@praxis/generation/templates/`.
...
- Tests in `tests/prompts/templates.test.ts` and
  `packages/@praxis/generation/test/` assert that fully rendered app and
