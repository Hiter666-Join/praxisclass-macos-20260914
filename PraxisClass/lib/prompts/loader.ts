...
import { loadVocationalSkillBody } from '@/lib/server/vocational-skill';
import {
...
export function loadPrompt(promptId: PromptId): LoadedPrompt | null {
  ...
  try {
    ...
    if (promptId === 'task-engine-outlines') {
      systemPrompt += '\n\n## Integrated native vocational requirements\n' +
        'For vocational jobs, apply the following shared Skill in addition to the JSON contract above. ' +
        'Its native minimum of three procedural-skill stages and common widget fields is mandatory. ' +
        'Other practice widgets supplement these stages.\n\n' + loadVocationalSkillBody();
    }
