...
export interface AppDocumentOutline {
   ...
   */
  requirement?: string;
  currentBrief?: CourseBrief;
  validation?: {
    checkedAt: number;
    structure: 'passed' | 'failed';
    interaction: 'passed' | 'unavailable' | 'failed' | 'not-required' | 'not-run';
    issues: string[];
    teacherReview: 'pending';
  };
  /** Source facts reviewed for this course; never shared across sessions. */
  sourceContext?: string;
  /** Original preparation files restored into this server's owner material library. */
  sourceMaterials?: CourseSourceMaterial[];
  /** Page production is finished; teaching review and interaction evidence are separate. */
...
export type AppDocument = PraxisDocument<AppScene, AppStage>;
