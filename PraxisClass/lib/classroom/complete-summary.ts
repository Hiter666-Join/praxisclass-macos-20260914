...
type QuizAnswerLoader = (input: { stageId: string; sceneId: string }) => Promise<{
  state?: { phase: 'draft' | 'submitted' | 'reviewed'; answers: Record<string, string | string[]> };
}>;
...
export async function readSceneQuizAnswers(
  ...
  return state?.phase === 'reviewed' ? state.answers : undefined;
}
