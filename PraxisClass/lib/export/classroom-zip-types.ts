...
import type { AgentVoiceConfig, Slide, VoiceDesign } from '@praxis/dsl';

export const CLASSROOM_ZIP_FORMAT_VERSION = 1;
export const CLASSROOM_ZIP_EXTENSION = '.praxis.zip';
...
export interface ClassroomManifest {
  ...
  /** Absent in older archives, whose nonempty scene list represented the finished deck. */
  generationComplete?: boolean;
  /** Original preparation documents; no session, credentials, or personal memory. */
  sourceMaterials?: ManifestSourceMaterial[];
}

export interface ManifestSourceMaterial {
  path: string;
  name: string;
  mime: string;
  bytes: number;
