...
import { archiveCourseSources } from './source-materials';
...
export async function buildClassroomExportZip(
  ...
  try {
    ...
    // 9. Assemble manifest
    const sourceMaterials = await archiveCourseSources(
      zip, freshDocument.document?.outline as AppDocumentOutline | undefined,
    );
    const manifest: ClassroomManifest = {
      ...
      generationComplete:
        (freshDocument.document?.outline as AppDocumentOutline | undefined)?.generationComplete ??
        manifestScenes.length > 0,
    };
