import { BrowserKVStore, type KVStore, type RuntimeStore } from '@praxis/storage';
...
export async function hydratePBLProjectFromRuntime(
  ...
  return withRuntimeStorageSharedLockUntilSettled(async () => {
    ...
    const store = args.store ?? getRuntimeStore();
    const transactionKey = `${args.stageId}:${args.sceneId}:${learnerKey}`;
    const sourceLearner = (args.project as PBLProjectV2 & { learnerKey?: string }).learnerKey;
    const project =
      sourceLearner === learnerKey || (!sourceLearner && !isDemoLearner(learnerKey))
        ? args.project
        : stripToDesignTemplate(args.project);

    const result = await withPBLRuntimeTransaction(store, transactionKey, () =>
      withDrainedProjectRuntime(
        {
          stageId: args.stageId,
          sceneId: args.sceneId,
        ...
        async () => {
          ...
          const designTemplate = stripToDesignTemplate(project);
          const folded = foldPBLRuntime({ designTemplate, records });
          const documentState = extractLearnerState(project);
          ...
          const hasDocumentLearnerState =
            !isDemoLearner(learnerKey) && documentContainsLearnerState(project);
          const cutoverStarted = hasWriteCutoverSnapshot(records);
          ...
          return {
            project: isDemoLearner(learnerKey)
              ? applyLearnerState(designTemplate, folded.learnerState)
              : preserveDocumentTransients(
                  applyLearnerState(designTemplate, folded.learnerState),
                  project,
                ),
            source: 'fold' as const,
    ...
    return { ...result, project: { ...result.project, learnerKey } };
  }, PBL_HYDRATION_DRAIN_BARRIER_TIMEOUT_MS);
}
  ...
  return Promise.all(
    scenes.map(async (scene) => {
      ...
      try {
        ...
        const learnerKey = options.learnerKey ?? (await getLearnerKey(options.kv));
        const sourceLearner = (resolved.projectV2 as PBLProjectV2 & { learnerKey?: string })
          .learnerKey;
        if (isDemoLearner(learnerKey) || (sourceLearner && sourceLearner !== learnerKey))
          throw error;
