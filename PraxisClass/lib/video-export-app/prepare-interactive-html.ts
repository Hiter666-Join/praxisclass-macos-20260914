...
function staticCaptureInjection(): string {
  ...
  return `
<meta data-praxis-static-csp http-equiv="Content-Security-Policy" content="default-src 'none'; script-src 'unsafe-inline' 'unsafe-eval' data: blob:; style-src 'unsafe-inline' data:; img-src data: blob:; font-src data:; media-src data: blob:; worker-src 'none'; connect-src 'none'; frame-src 'none'; object-src 'none'; base-uri 'none'">
<script data-praxis-static-capture>
(function () {
  var FLAG = ${flag};
  var frozen = false;
  ...
  function freeze() {
    ...
    style.setAttribute('data-praxis-static-frozen', '');
    ...
    document.documentElement.setAttribute('data-praxis-static-state', 'frozen');
  }

  window.__praxisFreezeInteractive = freeze;
...
declare global {
  interface Window {
    __praxisFreezeInteractive?: () => void;
  }
}
