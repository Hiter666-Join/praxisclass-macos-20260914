...
export interface EmitHyperframesOptions {
  ...
  height?: number;
  /** Composition id used for the root `data-composition-id` and the timeline key. Default `praxis`. */
  compositionId?: string;
  /** Relative path the emitted HTML loads GSAP from. Default `assets/vendor/gsap.min.js`. */
  gsapVendorPath?: string;
  /** Manifest filename. Default `praxis-video-manifest.json`. */
  manifestPath?: string;
...
const DEFAULT_MANIFEST = 'praxis-video-manifest.json';
const DEFAULT_LOCALE = 'en-US';
const PRODUCT_NAME = 'PraxisClass';
...
function interactiveStaticBridgeScript(labels: InteractiveFallbackLabels): string {
  ...
  return `
function initializePraxisInteractiveStaticFrames() {
  var hosts = Array.from(document.querySelectorAll('[data-interactive-static-host]'));
  window.__praxisVideoDiagnostics = window.__praxisVideoDiagnostics || [];
  window.__praxisVideoManifest = window.__praxisVideoManifest || { runtimeDiagnostics: [] };
  var labels = ${localized};
  var diagnosticCodes = new Set(${diagnosticCodes});
  var runtimeReport = document.querySelector('[data-praxis-runtime-diagnostics]');
  function record(sceneId, code, message) {
    ...
    window.__praxisVideoDiagnostics.push(diagnostic);
    window.__praxisVideoManifest.runtimeDiagnostics = window.__praxisVideoDiagnostics.slice();
    if (runtimeReport) runtimeReport.textContent = JSON.stringify(window.__praxisVideoDiagnostics);
...
function renderReadme(project: {
  ...
  const scriptFontSummary = project.quizScriptFonts.length
    ...
    : '';
  return `# ${project.stageName} — ${PRODUCT_NAME} video export
...
- Runtime interactive diagnostics are exposed live through \`window.__praxisVideoManifest.runtimeDiagnostics\` and the machine-readable DOM report in \`index.html\`.
...
export function emitHyperframes(
  ...
  const compositionId = options.compositionId ?? 'praxis';
...
<title>${escapeHtml(ir.stage.name)} — ${PRODUCT_NAME} video</title>
<style>
  ${INTER_FONT_FACE_CSS}${
    hasQuizQuestionList
...
${effectHtml.join('\n')}
${subtitles.html}
<script type="application/json" data-praxis-runtime-diagnostics>[]</script>
</div>
...
${statements.join('\n')}
window.__praxisVideoManifest = { runtimeDiagnostics: [], manifestPath: ${JSON.stringify(manifestPath)} };
window.__timelines = window.__timelines || {};
${
  hasInteractiveHtml
    ? `${interactiveStaticBridgeScript(labels.interactive)}
window.__praxisInteractiveReady = initializePraxisInteractiveStaticFrames();
window.__praxisInteractiveReady.then(function () {
