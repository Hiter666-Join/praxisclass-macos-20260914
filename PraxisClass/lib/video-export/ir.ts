 ...
 *
 * Pure: depends only on `@praxis/dsl` (the `SceneType` set) and `zod`.
 */
import { z } from 'zod';
import { SCENE_TYPES } from '@praxis/dsl';

/** Manifest `schema` tag — stable across versions; the shape is versioned by {@link VIDEO_TIMELINE_VERSION}. */
export const VIDEO_TIMELINE_SCHEMA = 'praxis.videoTimeline';
...
export const VIDEO_TIMELINE_COMPILER = 'praxis-video-timeline';
