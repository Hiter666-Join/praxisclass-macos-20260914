...
// VideoManifest / SlideContent / QuizContent / …) now lives in `@praxis/dsl` and
...
export { isSlideContent, isQuizContent } from '@praxis/dsl';

// `@praxis/dsl` inlines the question-type union on `QuizQuestion.type` rather than
// exporting a named alias; derive it here so editor quiz code can keep importing
// `QuizQuestionType` from `@/lib/types/stage`.
export type QuizQuestionType = import('@praxis/dsl').QuizQuestion['type'];
...
export type { SceneContent as SceneContentBase } from '@praxis/dsl';
...
export type { Scene as SceneShape } from '@praxis/dsl';
...
export type AppScene = DslScene<Action, SceneContent> & {
   ...
   * annotation only — not part of the `@praxis/dsl` Scene contract. Absent on
