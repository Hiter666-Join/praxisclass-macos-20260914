 ...
 *    so a keyless local provider (VoxCPM) is NOT force-shown;
...
// Simulate a provider ID retained in settings saved before its removal.
const retiredProviderId = 'lemonade-tts' as TTSProviderId;
...
describe('isTTSProviderConfigured', () => {
  ...
  it('does not activate the retired Lemonade provider from stale configuration', () => {
    expect(isTTSProviderConfigured(retiredProviderId, { baseUrl: 'http://localhost:13305/v1' })).toBe(false);
    expect(isTTSProviderConfigured(retiredProviderId, { isServerConfigured: true })).toBe(false);
  });
...
describe('isTTSProviderEnabled', () => {
  ...
  it('an unconfigured provider is never enabled', () => {
    ...
    expect(isTTSProviderEnabled(retiredProviderId, { enabled: true })).toBe(false);
  });
