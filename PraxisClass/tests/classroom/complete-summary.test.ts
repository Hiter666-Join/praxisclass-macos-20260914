...
describe('summarizeScenes', () => {
  ...
  it('does not report zero marks for an unattempted quiz or an unfinished attempt', async () => {
    const scene = quizScene('q1', 0, [choiceQ('qa', ['a'])]);
    for (const state of [
      undefined,
      { phase: 'draft' as const, answers: { qa: 'b' } },
      { phase: 'submitted' as const, answers: { qa: 'b' } },
    ]) {
      const summary = await summarizeScenes([scene], () =>
        readSceneQuizAnswers(scene, async () => ({ state })),
      );
      expect(summary.quiz).toBeNull();
    }
  });

  it('retains a real zero score after the quiz has been reviewed', async () => {
    const scene = quizScene('q1', 0, [choiceQ('qa', ['a'])]);
    const summary = await summarizeScenes([scene], () =>
      readSceneQuizAnswers(scene, async () => ({
        state: { phase: 'reviewed', answers: { qa: 'b' } },
      })),
    );
    expect(summary.quiz).toEqual({ correct: 0, total: 1, pct: 0 });
  });
