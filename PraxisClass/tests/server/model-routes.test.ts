...
describe('model-routes', () => {
  ...
  it('routes the Praxis editor agent stage (praxis-agent)', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({ 'praxis-agent': 'anthropic:claude-opus-4' });
    const { getStageModel } = await import('@/lib/server/model-routes');
    expect(getStageModel('praxis-agent')).toBe('anthropic:claude-opus-4');
  });
  ...
  it('parses an api and a contextWindow on an object route value', async () => {
    ...
    expect(getStageRoute('praxis-agent-driver')).toEqual({
      model: 'openai:gpt-5.6-luna',
      api: 'openai-completions',
      contextWindow: 32_000,
    });
  ...
  it('accepts dialect as an alias for api', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', dialect: 'openai-responses' },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({
      model: 'openai:gpt-5.6-luna',
  ...
  it('prefers api over dialect and warns on conflict', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      'Both api and dialect are set for stage "praxis-agent-driver"; api wins.',
    );
  });
  ...
  it('uses dialect when api is invalid and warns that dialect won', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      'Invalid api for stage "praxis-agent-driver" in MODEL_ROUTES; using dialect "openai-completions" instead.',
    );
  });
  ...
  it('ignores a non-string api with a warning but keeps the model', async () => {
    ...
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', api: 123 },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({ model: 'openai:gpt-5.6-luna' });
    expect(warn).toHaveBeenCalled();
  });
  ...
  it('ignores a non-string dialect with a warning but keeps the model', async () => {
    ...
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', dialect: ['openai-completions'] },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({ model: 'openai:gpt-5.6-luna' });
    expect(warn).toHaveBeenCalled();
  ...
  it('floors a fractional positive contextWindow', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', contextWindow: 32_000.9 },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({
      model: 'openai:gpt-5.6-luna',
  ...
  it('drops a contextWindow that floors below 1 with a warning but keeps the model', async () => {
    ...
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', contextWindow: 0.5 },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({ model: 'openai:gpt-5.6-luna' });
    expect(warn).toHaveBeenCalled();
  ...
  it('drops a non-positive contextWindow with a warning but keeps the model', async () => {
    ...
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', contextWindow: 0 },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({ model: 'openai:gpt-5.6-luna' });
    expect(warn).toHaveBeenCalled();
  ...
  it('drops a non-numeric contextWindow with a warning but keeps the model', async () => {
    ...
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', contextWindow: '32000' },
    });
    const { getStageRoute } = await import('@/lib/server/model-routes');
    expect(getStageRoute('praxis-agent-driver')).toEqual({ model: 'openai:gpt-5.6-luna' });
    expect(warn).toHaveBeenCalled();
  ...
  it('routes the agent driver stage (praxis-agent-driver)', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': {
        model: 'openai:gpt-5.6-luna',
        api: 'openai-completions',
    ...
    const { getStageModel } = await import('@/lib/server/model-routes');
    expect(getStageModel('praxis-agent-driver')).toBe('openai:gpt-5.6-luna');
  });
