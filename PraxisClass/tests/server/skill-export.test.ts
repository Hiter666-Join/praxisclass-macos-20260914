...
describe('skill export zips', () => {
  it('packages the shipped Praxis skill verbatim under praxis/', async () => {
    ...
    const entries = Object.values(loaded.files)
      ...
      .map((file) => file.name);
    expect(new Set(entries)).toEqual(new Set(onDisk.map((path) => `praxis/${path}`)));
    for (const path of onDisk) {
      expect(await loaded.file(`praxis/${path}`)!.async('string')).toBe(
  ...
  it('rejects traversal ids', () => {
    ...
    for (const value of ['../praxis', 'a/b', '..', '']) expect(isSafeSkillId(value)).toBe(false);
  });
