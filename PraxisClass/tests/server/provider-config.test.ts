...
describe('provider-config', () => {
  ...
  describe('iFLYTEK credentials', () => {
    it('keeps all Spark version credentials separate and out of client metadata', async () => {
      const slots = [
        ['XFYUN_X2_FLASH', 'xfyun-x2-flash'],
        ['XFYUN_X2', 'xfyun-x2'],
        ['XFYUN_X1_5', 'xfyun-x1-5'],
        ['XFYUN_ULTRA', 'xfyun-ultra'],
        ['XFYUN_PRO', 'xfyun-pro'],
        ['XFYUN_PRO_128K', 'xfyun-pro-128k'],
        ['XFYUN_LITE', 'xfyun-lite'],
      ];
      for (const [prefix] of slots) vi.stubEnv(`${prefix}_API_KEY`, `test-secret-${prefix}`);
      const { resolveApiKey, getServerProviders } = await import('@/lib/server/provider-config');
      const metadata = getServerProviders();
      for (const [prefix, id] of slots) {
        expect(resolveApiKey(id, 'client-fallback')).toBe(`test-secret-${prefix}`);
        expect(metadata).toHaveProperty(id);
      }
      expect(JSON.stringify(metadata)).not.toContain('test-secret-');
    });

    it('loads a complete env triple atomically and hides credentials from provider metadata', async () => {
      vi.stubEnv('TTS_XFYUN_APP_ID', 'app');
      vi.stubEnv('TTS_XFYUN_API_KEY', 'key');
      vi.stubEnv('TTS_XFYUN_API_SECRET', 'secret');
      const { resolveTTSApiKey, getServerTTSProviders } =
        await import('@/lib/server/provider-config');
      expect(resolveTTSApiKey('xfyun-tts', 'client:other:credentials')).toBe('app:key:secret');
      const metadata = getServerTTSProviders();
      expect(metadata).toHaveProperty('xfyun-tts');
      expect(JSON.stringify(metadata)).not.toContain('secret');
    });

    it('accepts an atomic YAML credential triple', async () => {
      yamlOverride = 'tts:\n  xfyun-tts:\n    apiKey: app:key:secret\n';
      const { resolveTTSApiKey } = await import('@/lib/server/provider-config');
      expect(resolveTTSApiKey('xfyun-tts')).toBe('app:key:secret');
    });

    it('does not enable or merge incomplete environment credentials with YAML', async () => {
      yamlOverride = 'tts:\n  xfyun-tts:\n    apiKey: yaml:key:secret\n';
      vi.stubEnv('TTS_XFYUN_APP_ID', 'other-app');
      const { isServerConfiguredProvider, resolveTTSApiKey } =
        await import('@/lib/server/provider-config');
      expect(isServerConfiguredProvider('tts', 'xfyun-tts')).toBe(false);
      expect(resolveTTSApiKey('xfyun-tts', 'client:key:secret')).toBe('client:key:secret');
    });

    it('honors the server disable switch', async () => {
      vi.stubEnv('TTS_XFYUN_API_KEY', 'app:key:secret');
      vi.stubEnv('TTS_XFYUN_ENABLED', 'false');
      const { isServerTTSProviderDisabled } = await import('@/lib/server/provider-config');
      expect(isServerTTSProviderDisabled('xfyun-tts')).toBe(true);
    });
  });
...
`;
  ...
  describe('image and video provider metadata', () => {
    ...
    it('ignores retired Lemonade environment and YAML configuration', async () => {
      vi.stubEnv('IMAGE_LEMONADE_BASE_URL', 'http://localhost:13305/v1');
      vi.stubEnv('LEMONADE_API_KEY', 'test-retired');
      yamlOverride = `
providers:
  lemonade:
    apiKey: test-retired
...
`;
      ...
      expect(isServerConfiguredProvider('image', 'lemonade')).toBe(false);
      expect(getServerImageProviders().lemonade).toBeUndefined();
      expect(resolveImageApiKey('lemonade')).toBe('');
      const { getServerProviders, getServerTTSProviders, getServerASRProviders } =
        await import('@/lib/server/provider-config');
      expect(getServerProviders()).not.toHaveProperty('lemonade');
      expect(getServerTTSProviders()).not.toHaveProperty('lemonade-tts');
      expect(getServerASRProviders()).not.toHaveProperty('lemonade-asr');
    });
  });
