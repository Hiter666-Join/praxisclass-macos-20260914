...
describe('readUsageRecords', () => {
  it('keeps live usage in the selected platform data directory', async () => {
    vi.stubEnv('PLATFORM_DATA_DIR', tmpDir);
    vi.stubEnv('VITEST', '');
    ...
    expect(records).toHaveLength(1);
    expect(records[0].source).toBe(llmInput.source);
    expect(await readUsageRecords({ baseDir: path.join(tmpDir, 'usage') })).toEqual(records);
  });
