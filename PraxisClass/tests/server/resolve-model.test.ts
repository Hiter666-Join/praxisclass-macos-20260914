...
describe('resolveModel — per-stage resolution order', () => {
  ...
  it('honors personal connection settings without applying deployment stage routes', async () => {
  ...
  it('carries the agent driver thinking toggle onto its resolved connection', async () => {
    ...
    const { resolveModel } = await import('@/lib/server/resolve-model');
    const r = await resolveModel({ stage: 'praxis-agent-driver' });
