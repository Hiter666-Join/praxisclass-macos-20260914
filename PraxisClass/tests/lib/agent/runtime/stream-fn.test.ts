...
it('preserves user images and tool preview images through the real model message boundary', () => {
  const image = { type: 'image', data: 'aGVsbG8=', mimeType: 'image/png' };
  const messages = toModelMessages([
    { role: 'user', content: [{ type: 'text', text: 'Inspect' }, image], timestamp: 0 },
    { role: 'toolResult', toolCallId: 'preview-1', toolName: 'render_scene_preview', content: [image], isError: false, timestamp: 1 },
  ] as PiMessage[]);
  expect(messages[0]).toMatchObject({ role: 'user', content: [{ type: 'text', text: 'Inspect' }, { type: 'image', image: 'aGVsbG8=', mediaType: 'image/png' }] });
  expect(messages[1]).toMatchObject({ role: 'tool', content: [{ output: { type: 'content', value: [{ type: 'image-data', data: 'aGVsbG8=', mediaType: 'image/png' }] } }] });
});
