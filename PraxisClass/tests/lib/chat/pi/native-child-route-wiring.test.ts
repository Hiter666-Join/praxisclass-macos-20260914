...
const envNames = [
  ...
  'PRAXIS_ENABLE_PI_NATIVE_CHILD_RUNTIME',
  'PRAXIS_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT',
  'TAVILY_API_KEY',
  'TAVILY_BASE_URL',
...
describe('PR2 Native Child route production wiring', () => {
  beforeEach(() => {
    ...
    process.env.PRAXIS_ENABLE_PI_NATIVE_CHILD_RUNTIME = 'true';
    process.env.PRAXIS_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT = 'true';
  ...
  it('keeps the production route on Legacy when the Native runtime flag is absent', async () => {
    delete process.env.PRAXIS_ENABLE_PI_NATIVE_CHILD_RUNTIME;
    delete process.env.PRAXIS_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT;
    const directorResponses = [
      [toolCall('read-1', 'read_scene', { sceneId: 'scene-current' }), finish('tool-calls')],
      [
        toolCall('delegate-1', 'call_agent', {
