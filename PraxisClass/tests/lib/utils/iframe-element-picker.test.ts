...
describe('iframe element picker shim', () => {
  it('stays dormant, blocks armed page clicks, emits picks, syncs pins, and exits', () => {
    ...
    expect(document.querySelector('[data-praxis-element-picker-overlay]')).toBeNull();
    ...
    flushFrame();
    expect(document.querySelector('[data-praxis-element-picker-overlay]')).not.toBeNull();
    ...
    flushFrame();
    expect(document.querySelector('[data-praxis-picker-pin]')?.textContent).toBe('1');
