...
describe('resolveClassroomExit', () => {
  ...
  it.each(['', 'returnTo=home'])('returns students to their course page for %s', (query) => {
    expect(resolveClassroomExit({ searchParams: params(query), platformMode: 'student' })).toEqual({
      kind: 'push',
      href: '/student',
    });
  });

  it('preserves an explicit workspace return during student co-creation', () => {
    expect(
      resolveClassroomExit({ searchParams: params('from=workspace'), platformMode: 'student' }),
...
describe('exitClassroom', () => {
  ...
  it('returns students to their platform without entering course creation', () => {
    const router = { push: vi.fn() };
    exitClassroom(router, params(), 'student');
...
describe('classroom navigation metadata', () => {
  ...
  it('keeps student exit copy aligned with its destination', () => {
    expect(classroomExitLabelKey(params(), 'student')).toBe('platform.backToPlatform');
    expect(classroomExitLabelKey(params('from=workspace'), 'student')).toBe(
      'workbench.common.backToWorkspace',
    );
  });
});
