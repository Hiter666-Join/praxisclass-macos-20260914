...
describe('classroom exit wiring', () => {
  ...
  ])('%s uses the shared exit helper and has no hardcoded home push', (path) => {
    const text = source(path);
    expect(text).toContain('exitClassroom(router, searchParams, platformMode)');
    expect(text).toContain('classroomExitLabelKey(searchParams, platformMode)');
