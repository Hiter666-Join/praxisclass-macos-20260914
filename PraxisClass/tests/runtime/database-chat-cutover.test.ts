...
import { DSL_VERSION } from '@praxis/dsl';
import { BrowserRuntimeStore, type KVStore, type RuntimeStore } from '@praxis/storage';
...
describe('database runtime chat integration', () => {
  ...
  it('loads a divergent destination without marking or deleting its legacy source', async () => {
    ...
    const { getDocumentStore } = await import('@/lib/document-store');
    const { loadStageData } = await import('@/lib/utils/stage-storage');
    const kv = new (await import('@praxis/storage')).BrowserKVStore();
  ...
  it('acquires both the stage-wide and legacy partition Web Lock names', async () => {
    ...
    expect(requested).toEqual([
      'praxis:chat-storage:all',
      `praxis:chat-storage:${encodeURIComponent('stage-compatible-lock')}`,
      `praxis:chat-storage:${encodeURIComponent(`stage-compatible-lock\0${learnerKey}`)}`,
    ]);
  });
  ...
  it('clears document and runtime databases without Web Locks', async () => {
    ...
    localStorage.setItem('praxis:device:document-migration:stage-clear-document', '{}');
    localStorage.setItem('praxis:device:editor-current-scene:stage-clear-document', '{}');
    await getAssetPool().put(new Blob(['private generated media'], { type: 'text/plain' }));
    expect((await indexedDB.databases()).map((entry) => entry.name)).toContain('praxis-asset-pool');
    ...
    expect(localStorage.getItem('praxis:device:document-migration:stage-clear-document')).toBeNull();
    expect(
      localStorage.getItem('praxis:device:editor-current-scene:stage-clear-document'),
    ).toBeNull();
