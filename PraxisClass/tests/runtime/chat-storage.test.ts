...
import { HttpRuntimeStoreError } from '@praxis/storage/runtime/http';
...
describe('chat RuntimeStore cutover', () => {
  it('persists optional spotlight inputs and does not append them again on an unchanged save', async () => {
    ...
    const input = session({
      messages: [{
        ...
        parts: [{
          ...
          input: { elementId: 'element-1', dimOpacity: undefined },
        }],
      }],
    });
    const options = { store, learnerKey: LEARNER_KEY, legacyStore };

    await saveChatSessions(STAGE_ID, [input], options);
    const records = await runtimeChatRecords(store);
    const loaded = await loadChatSessions(STAGE_ID, options);
    expect(loaded[0].messages[0].parts[0]).toEqual({
      type: 'dynamic-tool', toolName: 'spotlight', toolCallId: 'spotlight-1',
      state: 'input-available', input: { elementId: 'element-1' },
    });
    expect(input.messages[0].parts[0]).toHaveProperty('input.dimOpacity');

    await saveChatSessions(STAGE_ID, [input], options);
    expect(await runtimeChatRecords(store)).toHaveLength(records.length);
  });
  ...
  it.each([
    [
      ...
      `@praxis/storage: cannot append to session "chat:stage-chat:anon%3Achat-test:session-1" with status 'completed' — records may only be appended to an active session`,
    ],
    [
      'post-race reclassification',
      `@praxis/storage: session "chat:stage-chat:anon%3Achat-test:session-1" is no longer active; its current status is 'completed'`,
    ],
