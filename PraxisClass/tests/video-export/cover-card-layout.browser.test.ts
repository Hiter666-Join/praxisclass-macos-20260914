...
const SCRIPT_FONT_MATRIX = [
  {
    ...
    family: 'PraxisClass Noto Sans Cyrillic',
    text: 'Привет Ёж Ԁ',
    expectedFontFiles: [
  ...
  },
  {
    ...
    family: 'PraxisClass Noto Sans Arabic',
    text: 'العربية',
  ...
  try {
    ...
    const runtime = await page.evaluate(() => {
      ...
      return {
        ...
        praxisTimeline: typeof globals.__timelines?.praxis?.duration === 'function',
      };
    });
...
describe('Quiz/PBL cover cards in a real browser', () => {
    ...
    check(`falls back when a selected Quiz font load is ${fontLoadResult}`, async (page) => {
      ...
      expect(surface.requiredFontLoads).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ family: 'PraxisClass Noto Sans Cyrillic' }),
        ]),
      );
      await page.setViewportSize(frame);
      await page.goto(HARNESS_URL, { waitUntil: 'load' });
  ...
  check('fails closed when visible script text is missing from the vendored cmap', async (page) => {
    ...
    for (const { character, family } of [
      { character: '\u1d2b', family: 'PraxisClass Noto Sans Cyrillic' },
      { character: '\u1c89', family: 'PraxisClass Noto Sans Cyrillic' },
      { character: '\u0897', family: 'PraxisClass Noto Sans Arabic' },
    ]) {
      const surface = createQuizQuestionListMeasurementSurface({
        content: {
  ...
  check('runs the production Quiz measurer and emitted timeline end to end', async (page) => {
    ...
    expect(
      await page.evaluate(() => document.fonts.check('16px "PraxisClass Noto Sans SC"', '中文')),
    ).toBe(true);
    const cjkCoverage = await page.evaluate(async () => {
      await Promise.all([
        document.fonts.load('40px "PraxisClass Noto Sans SC"', '漢字あア'),
        document.fonts.load('40px "PraxisClass Noto Sans KR"', '한글'),
      ]);
      ...
      const covered = (text: string, family: string): boolean =>
        signature(text, `"${family}", monospace`) !== signature(text, 'monospace');
      return {
        han: covered('漢字', 'PraxisClass Noto Sans SC'),
        kana: covered('あア', 'PraxisClass Noto Sans SC'),
        hangul: covered('한글', 'PraxisClass Noto Sans KR'),
      };
    });
