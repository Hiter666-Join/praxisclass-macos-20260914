    ...
    try {
      ...
      await page.waitForFunction(() => {
        ...
        const timelines = (window as typeof window & { __timelines?: Record<string, unknown> })
          .__timelines;
        return (
          host?.getAttribute('data-interactive-static-state') === 'frozen' && !!timelines?.praxis
        );
      });
      ...
      const before = await child.evaluate(() => ({
        ...
        state: document.documentElement.getAttribute('data-praxis-static-state'),
      ...
      const result = await page.evaluate(() => {
        ...
        const diagnostics = (
          window as typeof window & {
            __praxisVideoDiagnostics?: Array<{ code: string; message: string }>;
          }
        ).__praxisVideoDiagnostics;
        const manifest = (
          window as typeof window & {
            __praxisVideoManifest?: { runtimeDiagnostics?: Array<{ code: string }> };
          }
        ).__praxisVideoManifest;
        const runtimeReport = document.querySelector<HTMLElement>(
          '[data-praxis-runtime-diagnostics]',
        );
        return {
