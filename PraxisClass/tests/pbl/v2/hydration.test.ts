...
import type { RuntimePayload, RuntimeRecord, RuntimeRecordInit, RuntimeSession } from '@praxis/dsl';
import { BrowserRuntimeStore, type RuntimeSessionInit, type RuntimeStore } from '@praxis/storage';
...
import type { KVScope, KVStore } from '@praxis/storage';
...
describe('PBL runtime hydration', () => {
  it('does not backfill student document answers or outboxes into a teacher rehearsal', async () => {
    ...
    const hydrated = await hydratePBLProjectFromRuntime({
      ...
      learnerKey: `demo:${LEARNER_KEY}`,
    });
    expect(hydrated.source).toBe('fold');
    expect(extractLearnerState(hydrated.project)).toEqual(
      extractLearnerState(stripToDesignTemplate(project)),
    );
    expect(
      hydrated.project.threads.some((thread) =>
        thread.messages.some((message) => message.id === 'student-private'),
      ),
    ).toBe(false);
    expect(await store.listSessions(STAGE_ID, LEARNER_KEY)).toHaveLength(0);
  });
