...
import { DocumentVersionError } from '@praxis/storage';
...
describe('saveStageDataIncremental', () => {
  it('retains Pro provenance and imported originals when an editor snapshot saves page state', async () => {
    const metadata = { producer: 'server-job', producerRef: 'session-1', sourceMaterials: [{ materialId: 'source-1', originalName: 'a.pdf', mime: 'application/pdf', bytes: 5 }] };
    loadDocument.mockReturnValue({ stage, scenes, outline: { ...data.outline, ...metadata } });
    await saveStageDataIncremental('stage-1', [{ kind: 'outline' }], data, 0);
    expect(saveDocument.mock.calls[0][0].outline).toMatchObject(metadata);
  });
