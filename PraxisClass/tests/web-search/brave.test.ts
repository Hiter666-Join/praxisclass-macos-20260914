...
import { BraveSearchError, parseBraveSearchHtml, searchWithBrave } from '@/lib/web-search/brave';
...
describe('searchWithBrave', () => {
  ...
  it.each([undefined, 'test-brave-key'])(
    'does not expose upstream error pages (key: %s)',
    async (apiKey) => {
      proxyFetchMock.mockResolvedValueOnce(
        new Response('<html><script>private challenge payload</script></html>', { status: 429 }),
      );
      const error = await searchWithBrave({ query: 'Spring Boot', apiKey }).catch((err) => err);
      expect(error).toBeInstanceOf(BraveSearchError);
      expect(error.status).toBe(429);
      expect(error.message).toContain('稍后重试');
      expect(error.message).not.toMatch(/html|script|challenge/);
    },
  );
