...
import { skillCatalog, getSkillGuide } from '@/lib/workbench/skill-catalog';
...
vi.mock('@/lib/workbench/agent-skills', () => ({
  useAgentSkills: () => ({
    ...
    runtimeReady: mocks.registry.runtimeReady,
    reload: mocks.reload,
  }),
...
afterEach(() => {
  ...
  mocks.registry.runtimeReady = true;
...
describe('the skills list renders from the API shape', () => {
  it('covers every shipped skill without adding skills absent from the runtime registry', () => {
    const installed = readdirSync(resolve(process.cwd(), 'skills/agent-runtime'), {
      withFileTypes: true,
    })
      .filter((entry) => entry.isDirectory())
      .map((entry) => entry.name)
      .sort();
    expect(Object.keys(skillCatalog).sort()).toEqual(installed);
    for (const name of installed) {
      expect(getSkillGuide(name, 'zh-CN')?.benefit).toBeTruthy();
      expect(getSkillGuide(name, 'zh-CN')?.scenario).toBeTruthy();
    }
    mocks.registry.skills = [builtinSkill()];
    const host = mount();
    expect(host.querySelectorAll('[data-testid^="skill-settings-row-"]')).toHaveLength(1);
    expect(host.querySelector('a[href^="/workspace"]')).toBeNull();
  });

  it('filters by teaching category and can recover from an empty search', () => {
    mocks.registry.skills = [
      builtinSkill(),
      builtinSkill({ id: 'fact-check', name: 'fact-check', title: 'Fact check' }),
    ];
    const host = mount();
    const category = Array.from(host.querySelectorAll('button')).find(
      (button) => button.textContent === 'Research & reliability',
    )!;
    act(() => category.click());
    expect(host.querySelector('[data-testid="skill-settings-row-stage-design"]')).toBeNull();
    expect(host.querySelector('[data-testid="skill-settings-row-fact-check"]')).not.toBeNull();
    ...
    expect(host.querySelectorAll('[data-testid^="skill-settings-row-"]')).toHaveLength(0);
    const clear = Array.from(host.querySelectorAll('button')).find(
      (button) => button.textContent === 'Clear filters',
    )!;
    act(() => clear.click());
    expect(host.querySelectorAll('[data-testid^="skill-settings-row-"]')).toHaveLength(2);
  });

  it('shows a library-only explanation and no invocation or personal mutations when runtime is unavailable', () => {
    mocks.registry.runtimeReady = false;
    mocks.registry.skills = [builtinSkill()];
    const host = mount();
    expect(
      host.querySelector('[data-testid="skill-settings-library-status"]')?.textContent,
    ).toContain('Personal skill management is currently unavailable');
    expect(host.querySelector('[data-testid="skill-settings-use-stage-design"]')).toBeNull();
    expect(
      host.querySelector<HTMLButtonElement>('[data-testid="skill-settings-upload"]')?.disabled,
    ).toBe(true);
...
describe('the settings surface mounts the section', () => {
  ...
  it('renders SkillSettings when the skills section is active', () => {
    ...
    expect(dialog).toContain("id: 'skills', labelKey: 'settings.skills.nav'");
