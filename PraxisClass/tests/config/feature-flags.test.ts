...
const FLAG = 'NEXT_PUBLIC_PRAXIS_EDITOR_ENABLED';

describe('agent runtime configuration predicate', () => {
  const ENV_KEYS = ['PRAXIS_AGENT_RUNTIME_ENABLED', 'DATABASE_URL'] as const;
  ...
  it.each([
    ...
    (_case, runtimeFlag, databaseUrl, enabled, configured) => {
      if (runtimeFlag !== undefined) process.env.PRAXIS_AGENT_RUNTIME_ENABLED = runtimeFlag;
...
describe('isPraxisEditorEnabled', () => {
  ...
  it('returns false when the env var is unset', () => {
    delete process.env[FLAG];
    expect(isPraxisEditorEnabled()).toBe(false);
  });

  it("returns true for 'true'", () => {
    process.env[FLAG] = 'true';
    expect(isPraxisEditorEnabled()).toBe(true);
  });
...
describe('isPlaybackRendererEnabled', () => {
  const flag = 'NEXT_PUBLIC_PRAXIS_PLAYBACK_RENDERER_ENABLED';
  let original: string | undefined;
...
describe('isEditorRendererEnabled', () => {
  const flag = 'NEXT_PUBLIC_PRAXIS_EDITOR_RENDERER_ENABLED';
  let original: string | undefined;
...
describe.each([
  ['PRAXIS_ENABLE_PI_NATIVE_CHILD_RUNTIME', isPiNativeChildRuntimeEnabled],
  ['PRAXIS_ENABLE_PI_NATIVE_CHILD_SPOTLIGHT', isPiNativeChildSpotlightEnabled],
])('%s', (flag, readFlag) => {
  let original: string | undefined;
...
describe('isVocationalTaskEngineEnabled', () => {
  const flag = 'PRAXIS_ENABLE_VOCATIONAL';
  let original: string | undefined;
