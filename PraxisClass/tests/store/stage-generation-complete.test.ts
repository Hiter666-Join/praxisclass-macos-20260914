...
describe('generationComplete', () => {
  it('restores Pro ownership and leaves completion to the producer', async () => {
    loadStageDataMock.mockResolvedValue({
      ...makeStoredLoad('stage-1', 'partial'),
      outline: { outlines: [], producer: 'server-job', producerRef: 'producer-session', generationComplete: false, createdAt: 1, updatedAt: 1 },
    });
    await useStageStore.getState().loadFromStorage('stage-1');
    expect(useStageStore.getState()).toMatchObject({ outlineProducer: 'server-job', outlineProducerRef: 'producer-session', generationComplete: false });
  });
  it('restores the completion page for an older outline-less imported course', async () => {
    loadStageDataMock.mockResolvedValue(makeStoredLoad('stage-1', 'imported'));
    await useStageStore.getState().loadFromStorage('stage-1');
    expect(useStageStore.getState().generationComplete).toBe(true);
    expect(useStageStore.getState().scenes).toHaveLength(1);
  });
  it('keeps an explicitly incomplete imported course incomplete', async () => {
    loadStageDataMock.mockResolvedValue({
      ...makeStoredLoad('stage-1', 'partial'),
      outline: { outlines: [], generationComplete: false, createdAt: 1, updatedAt: 1 },
    });
    await useStageStore.getState().loadFromStorage('stage-1');
