...
import { saveClassroomEdits, useClassroomDrafts, getInteractiveDraft } from '@/lib/edit/classroom-save';
...
beforeEach(() => {
  ...
  useClassroomDrafts.setState({ drafts: {} });
...
describe('incremental stage flush', () => {
  it('manual save rejects partial persistence and permits retry without losing the edit', async () => {
    useStageStore.getState().updateScene('scene-1', { title: 'unsaved correction' });
    incrementalSave.mockResolvedValueOnce({ failedChanges: [{ kind: 'scene', sceneId: 'scene-1' }] });
    await expect(saveClassroomEdits()).rejects.toThrow('尚未全部保存');
    expect(useStageSaveStatus.getState()).toMatchObject({ dirty: true, failed: true });
    await saveClassroomEdits();
    expect(useStageSaveStatus.getState()).toMatchObject({ dirty: false, saving: false, failed: false });
  });
  it('saves interactive drafts from another page and retains a newer draft during an in-flight save', async () => {
    const page: Scene = { id: 'html-page', stageId: 'stage-1', order: 3, title: 'Practice', type: 'interactive', content: { type: 'interactive', html: '<p>old</p>' } };
    useStageStore.setState({ scenes: [...useStageStore.getState().scenes, page], isOwner: true, readOnly: false });
    useClassroomDrafts.getState().setDraft('stage-1', page.id, { html: '<p>corrected</p>', url: '' });
    useStageStore.getState().setCurrentSceneId('scene-2');
    let release!: () => void;
    incrementalSave.mockImplementationOnce(() => new Promise<void>(resolve => { release = resolve; }));
    const saving = saveClassroomEdits();
    await vi.waitFor(() => expect(incrementalSave).toHaveBeenCalled());
    useClassroomDrafts.getState().setDraft('stage-1', page.id, { html: '<p>newest</p>', url: '' });
    release();
    await expect(saving).rejects.toThrow('新的修改');
    expect(getInteractiveDraft('stage-1', page.id)?.html).toBe('<p>newest</p>');
    await saveClassroomEdits();
    expect(getInteractiveDraft('stage-1', page.id)).toBeUndefined();
    expect(useStageStore.getState().scenes.find(item => item.id === page.id)?.content).toMatchObject({ html: '<p>newest</p>' });
    expect(useStageStore.getState().currentSceneId).toBe('scene-2');
  });
