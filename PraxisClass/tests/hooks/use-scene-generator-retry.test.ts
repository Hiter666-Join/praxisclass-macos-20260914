...
type StageState = ReturnType<typeof import('@/lib/store/stage').useStageStore.getState>;
...
vi.mock('@/lib/media/media-orchestrator', () => ({
  generateMediaForOutlines: vi.fn().mockResolvedValue(undefined),
}));
...
describe('browser scene generation retry wrappers', () => {
  beforeEach(() => {
    ...
    mocks.stageState = {
      ...
      setGenerationStatus: (status) => { mocks.stageState.generationStatus = status; },
      setCurrentGeneratingOrder: vi.fn(),
      setGeneratingOutlines: (outlines) => { mocks.stageState.generatingOutlines = outlines; },
      addFailedOutline: (failed) => { mocks.stageState.failedOutlines = [failed]; },
      retryFailedOutline: () => { mocks.stageState.failedOutlines = []; },
      addScene: (scene) => { mocks.stageState.scenes.push(scene); },
      markGenerationCompleteIfDone: () => {
        mocks.stageState.generationComplete = mocks.stageState.scenes.length === 1;
      },
    } as StageState;
  });

  it.each(['content', 'actions', 'abort'] as const)(
    'returns a failed %s retry to paused and keeps its outline retryable', async (phase) => {
      const { useSceneGenerator } = await import('@/lib/hooks/use-scene-generator');
      const generator = useSceneGenerator();
      mockFetch.mockResolvedValue(jsonResponse(401, { error: 'unavailable' }));
      await generator.generateRemaining({ stageInfo: { name: 'Retry Course' } });

      if (phase === 'actions') {
        mockFetch.mockResolvedValueOnce(jsonResponse(200, { success: true, content: { elements: [] } }));
      } else if (phase === 'abort') {
        mockFetch.mockRejectedValueOnce(Object.assign(new Error('aborted'), { name: 'AbortError' }));
      }
      await generator.retrySingleOutline(outline.id);

      expect(mocks.stageState.generationStatus).toBe('paused');
      expect(mocks.stageState.failedOutlines).toEqual([outline]);
      expect(mocks.stageState.generatingOutlines).toEqual([outline]);
      expect(mocks.stageState.generationComplete).toBe(false);
    },
  );

  it('finishes generation when retry materializes the last missing scene', async () => {
    const { useSceneGenerator } = await import('@/lib/hooks/use-scene-generator');
    const generator = useSceneGenerator();
    mockFetch.mockResolvedValue(jsonResponse(401, { error: 'unavailable' }));
    await generator.generateRemaining({ stageInfo: { name: 'Retry Course' } });
    const scene = { id: 'scene-1', stageId: 'stage-1', order: outline.order, actions: [] } as unknown as Scene;
    mockFetch
    ...
    expect(mocks.stageState.scenes).toEqual([scene]);
  });
