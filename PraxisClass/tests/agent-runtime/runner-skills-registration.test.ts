...
import type { ClaimedAgentSession } from '@praxis/storage';
...
const fixture = vi.hoisted(() => {
  /* eslint-disable @typescript-eslint/no-require-imports -- vi.hoisted runs before
     module imports, and PRAXIS_AGENT_SKILLS_DIR must be set before config.ts loads. */
  ...
  process.env.PRAXIS_AGENT_SKILLS_DIR = root;
  return { root };
});
...
describe('skills runner registration', () => {
  ...
  it('reuses the fixture temp dir so the loader reads real files', () => {
    expect(fixture.root).toBe(process.env.PRAXIS_AGENT_SKILLS_DIR);
  });
});
