...
describe('generation and deck tools', () => {
  it('persists the complete native plan and carries its page goals and source facts into generation', async () => {
    const current = state(document([]));
    const prompts: string[] = [];
    const aiCall = vi.fn(async (_system: string, user: string) => {
      prompts.push(user);
      return prompts.length === 1 ? '<html><body><button>Check</button></body></html>' : '[]';
    });
    const tools = buildGenerationTools(deps(current.store, { aiCall, getUserRequirement: () => '完整原始要求：必须保留 null 验收记录' }));
    const plan = [
      { id: 'custom-practice', order: 1, title: '接口练习', type: 'interactive', description: 'null 验收步骤', keyPoints: ['必须记录输入和实际结果'], widgetType: 'code', widgetOutline: { language: 'java', task: '检查空值' } },
      { id: 'custom-quiz', order: 2, title: '诊断', type: 'quiz', description: '判断结果', keyPoints: [] },
    ];
    const saved = await find(tools, 'review_course_plan').execute('plan', { stageId: 'stage-test', requirement: '模型压缩后的要求', outlines: plan, sourceContext: '教师材料第 2 页：接口返回空数组' } as never);
    expect(saved).not.toMatchObject({ isError: true });
    expect(aiCall).not.toHaveBeenCalled();
    expect(current.get()?.outline).toMatchObject({ outlines: plan, generationComplete: false, requirement: '完整原始要求：必须保留 null 验收记录' });
    await find(tools, 'generate_scene').execute('page', { stageId: 'stage-test', order: 1, title: '接口练习', type: 'interactive', brief: '实现此页' } as never);
    expect(current.get()?.scenes[0]).toMatchObject({ outlineId: 'custom-practice', content: { widgetType: 'code' } });
    expect(prompts[0]).toContain('必须记录输入和实际结果');
    expect(prompts[0]).toContain('null 验收步骤');
    expect(prompts[0]).toContain('教师材料第 2 页：接口返回空数组');
    const report = await find(tools, 'list_scenes').execute('check', { stageId: 'stage-test' } as never);
    expect(report).toMatchObject({ isError: true });
    expect(report.content[0]).toMatchObject({ text: expect.stringContaining('第 2 页 诊断 尚未生成') });
  });

  it('returns actionable invalid plan errors without replacing a saved plan', async () => {
    const current = state(document([slide('a', 1)]));
    const before = structuredClone(current.get()?.outline);
    const review = find(buildGenerationTools(deps(current.store)), 'review_course_plan');
    const response = await review.execute('invalid', { stageId: 'stage-test', requirement: '练习', outlines: [null] } as never);
    expect(response).toMatchObject({ isError: true });
    expect(current.get()?.outline).toEqual(before);
  });

  it('checks actual question quantity against the persisted plan', async () => {
    const scene = { ...slide('quiz', 1), type: 'quiz', content: { type: 'quiz', questions: [{ id: 'a' }, { id: 'b' }, { id: 'c' }] } } as unknown as Scene;
    const current = state(document([scene]));
    const tools = buildGenerationTools(deps(current.store));
    await find(tools, 'review_course_plan').execute('plan', { stageId: 'stage-test', requirement: '只要两道单选题', outlines: [{ id: 'quiz', order: 1, title: '诊断', type: 'quiz', description: '分母检查', keyPoints: [], quizConfig: { questionCount: 2, difficulty: 'easy', questionTypes: ['single'] } }] } as never);
    const report = await find(tools, 'list_scenes').execute('check', { stageId: 'stage-test' } as never);
    expect(report).toMatchObject({ isError: true });
    expect(report.content[0]).toMatchObject({ text: expect.stringContaining('应有 2 题，实际 3 题') });
  });

  it('returns a broken page draft to the native agent, with no hidden repair or narration calls', async () => {
    const current = state(document([]));
    const aiCall = vi.fn<(...args: unknown[]) => Promise<string>>(async () => '<!DOCTYPE html><html><body><h1>Java 验收记录</h1><script>const x = ;</script></body></html>');
    const tools = buildGenerationTools(deps(current.store, { aiCall, getUserRequirement: () => '原始要求：Java null 验收记录' }));
    const response = await find(tools, 'generate_scene').execute('bad', { stageId: 'stage-test', order: 1, title: '验收', type: 'interactive', brief: '本页检查', widgetType: 'code' } as never);
    expect(response).toMatchObject({ isError: true });
    expect(aiCall).toHaveBeenCalledOnce();
    expect(aiCall.mock.calls[0]?.[1]).toContain('原始要求：Java null 验收记录');
    expect(current.get()?.scenes[0]?.content).toMatchObject({ html: expect.stringContaining('Java 验收记录') });
    const report = await find(tools, 'list_scenes').execute('check', { stageId: 'stage-test' } as never);
    expect(report.content[0]).toMatchObject({ type: 'text', text: expect.stringContaining('Unexpected token') });
    expect(report.content[0]).toMatchObject({ text: expect.stringContaining('验收') });
  });
  ...
  it('validates widget params through the tool schema', () => {
    ...
    // The native vocational widget is now available through the Pro tool too.
  ...
  it('generates native practice with the same course context as the preparation entry', async () => {
    vi.stubEnv('PRAXIS_ENABLE_VOCATIONAL', 'true');
    try {
      const current = state(document([slide('job-brief', 1, '服务上线岗位任务')]));
      const prompts: string[] = [];
      const generate = find(buildGenerationTools(deps(current.store, {
        aiCall: vi.fn(async (_system: string, user: string) => {
          prompts.push(user);
          return prompts.length === 1 ? '<!doctype html><html><body><button>检查依赖</button></body></html>' : '[]';
        }),
      })), 'generate_scene');
      const result = await generate.execute('vocational', { stageId: 'stage-test', order: 2, title: '发布前依赖检查', type: 'interactive', brief: '验证依赖并决定是否继续上线', widgetType: 'procedural-skill',
        widgetOutline: { procedureType: 'inspection', task: '依赖检查', tools: ['构建日志'], steps: ['核对版本', '确认构建'], successCriteria: ['版本一致'], errorConsequences: ['构建失败时停止上线'] } } as never);
      expect(result).not.toMatchObject({ isError: true });
      expect(current.get()?.scenes[1]?.content).toMatchObject({ widgetType: 'procedural-skill' });
      expect(prompts[0]).toContain('current outline');
      expect(prompts[0]).not.toContain('培养方案建议');
      expect(prompts[0]).toContain('服务上线岗位任务');
      expect(prompts[1]).toContain('服务上线岗位任务');
