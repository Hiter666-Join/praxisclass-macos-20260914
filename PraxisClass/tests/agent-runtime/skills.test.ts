...
describe('pi-native skills', () => {
  ...
  it('activates a skill only after its SKILL.md is read successfully', async () => {
    const root = mkdtempSync(join(tmpdir(), 'praxis-native-skill-'));
    try {
...
describe('shipped skill constraints', () => {
  ...
  it('ships the K-12 core-literacy skill as a PraxisClass-native classroom flow', async () => {
...
describe('shipped course-design skills', () => {
  ...
  it('ships build-personal-skill with evidence, paging, calibration, and creation workflow', async () => {
    ...
    expect(personal!.title).toBe('教师经验复用');
    expect(personal!.description).toContain('saved profile and long-term memory');
    expect(personal!.content).toContain('references/apply-teaching-memory.md');
