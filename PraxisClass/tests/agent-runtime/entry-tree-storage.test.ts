import { Session, SessionError, type SessionTreeEntry } from '@earendil-works/pi-agent-core';
import {
  PgAgentSessionStore,
...
describe.skipIf(!contractUrl)('AgentSessionEntryStorage with PostgreSQL 16', () => {
  ...
  it('restores browser acceptance from PostgreSQL after a worker change and compaction', async () => {
    const meta = await store.createSession({
      id: 'browser-check-resume',
    ...
    const session = new Session(
      AgentSessionEntryStorage.fromHandle(
        meta,
        await store.openEntryTree(meta.id, 'worker-a', claim!.attempt),
      ),
    );
    const doc = {
      stage: { id: 'course-a' },
      scenes: [
        {
          ...
          order: 1,
          type: 'interactive',
          content: { type: 'interactive', html: '<button id="run">运行</button>' },
        },
      ],
    } as unknown as CourseDocument;
    const deps = {
      store: { loadDocument: async () => doc } as unknown as CourseStore,
      stageAccess: async () => ({ kind: 'owned' as const }),
    };
    const checks = new InteractionChecks(
      deps,
      async () => ({ status: 'passed', cases: [], blockedRequests: [] }),
      async (receipt) => {
        await session.appendCustomEntry(INTERACTION_CHECKPOINT_TYPE, receipt);
      },
    );
    await checks.execute('course-a', 'scene-a', [
      {
        name: '入口可见',
        requirement: '运行入口',
        steps: [{ action: 'expect', selector: '#run', property: 'visible', expected: true }],
      },
    ]);
    const tail = await session.appendMessage({ role: 'user', content: '继续', timestamp: 1 });
    await session.appendCompaction('保留进度', tail, 100);
    await store.finishSession(meta.id, 'worker-a', { status: 'failed' });
    await store.requeueForRetry(meta.id);
    const next = await store.claimNextSession('worker-b', 202, {
      leaseTtlMs: 10_000,
      maxAttempts: 5,
      sessionId: meta.id,
    });
    const reopened = new Session(
      AgentSessionEntryStorage.fromHandle(
        meta,
        await store.openEntryTree(meta.id, 'worker-b', next!.attempt),
      ),
    );
    const resumed = new InteractionChecks(deps, async () => {
      throw new Error('unchanged passed content must reuse its checkpoint');
    });
    resumed.restore(await reopened.getBranch());
    expect((await resumed.execute('course-a', 'scene-a')).cached).toBe(true);
    expect(JSON.stringify(await reopened.buildContext())).not.toContain('<button');
  });
