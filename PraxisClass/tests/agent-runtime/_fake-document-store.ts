...
export interface FakeDocumentStore {
  ...
  docs: Map<string, PraxisDocument<AppScene, AppStage>>;
  ...
  saveCalls: PraxisDocument<AppScene, AppStage>[];
...
export function createFakeDocumentStore(): FakeDocumentStore {
  const docs = new Map<string, PraxisDocument<AppScene, AppStage>>();
  const stageRevs = new Map<string, number>();
  const sceneRevs = new Map<string, Map<string, number>>();
  const saveCalls: PraxisDocument<AppScene, AppStage>[] = [];
  ...
  const store = {
    forOwner: () => store,
    async saveDocument(doc: PraxisDocument<AppScene, AppStage>) {
    ...
    async putStage(stageId: string, stage: AppStage) {
      const doc = docs.get(stageId);
      if (!doc) throw new Error('@praxis/storage: document not found');
