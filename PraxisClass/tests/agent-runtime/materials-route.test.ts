...
import type { AgentSessionMaterial } from '@praxis/storage';
...
describe('POST /api/materials', () => {
  ...
  it('keeps anonymous ownership while using a Windows-safe material directory', async () => {
    mocks.resolveRequestOwnerId.mockReturnValue('anon:demo-owner');
    const response = await post(Buffer.from('lesson'));
    expect(response.status).toBe(201);
    const { materialId } = await response.json();
    const ossKey = `materials/anon%3Ademo-owner/${materialId}`;
    expect(mocks.registerOwnerMaterial).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ ownerId: 'anon:demo-owner', ossKey }),
      expect.anything(),
    );
    expect(mocks.byteStore.put).toHaveBeenCalledWith(
      ossKey,
