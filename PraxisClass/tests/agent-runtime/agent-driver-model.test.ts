...
describe('agent driver model route', () => {
  ...
  it.each(['gpt-5.6-terra', 'gpt-5.6-luna'])(
    ...
    async (modelId) => {
      ...
      expect(mocks.resolveModel).toHaveBeenCalledWith({ stage: 'praxis-agent-driver' });
  ...
  it('uses the provider catalog window when the model is known', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'openai:gpt-5.6-luna', api: 'openai-completions' },
    });
    mocks.resolveModel.mockResolvedValue({
      model: {},
  ...
  it('falls back to a conservative real window for an unknown model', async () => {
    process.env.MODEL_ROUTES = JSON.stringify({
      'praxis-agent-driver': { model: 'custom:some-model', api: 'openai-completions' },
    });
    mocks.resolveModel.mockResolvedValue({
      model: {},
      modelInfo: null,
