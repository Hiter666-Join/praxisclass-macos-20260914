...
describe('GET agent skills', () => {
  ...
  it('lists only the filesystem library without resolving an owner when the runtime is off', async () => {
    mocks.isAgentRuntimeEnabled.mockReturnValueOnce(false);
    mocks.listSkills.mockResolvedValueOnce([
      {
        id: 'stage-design',
        name: 'stage-design',
        description: 'Build a lesson',
        source: 'builtin',
        constraints: null,
      },
    ]);
    const response = await GET(request());
    expect(response.status).toBe(200);
    expect(response.headers.get('X-Agent-Runtime-Ready')).toBe('false');
    expect(mocks.listSkills).toHaveBeenCalledWith();
    expect(mocks.withRequestOwnerId).not.toHaveBeenCalled();
    expect(await response.json()).toMatchObject([{ id: 'stage-design', source: 'builtin' }]);
  });

  it('keeps personal skill uploads unavailable when the runtime is off', async () => {
    mocks.isAgentRuntimeEnabled.mockReturnValueOnce(false);
    expect((await POST(uploadRequest(new File(['test'], 'SKILL.md')))).status).toBe(404);
