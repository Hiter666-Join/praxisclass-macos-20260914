...
// This lifecycle test stops at driver setup; runtime settings and memory must
// not introduce a real database dependency before the first event is emitted.
vi.mock('@/lib/server/agent-runtime/service-settings', () => ({
  loadRuntimeSettings: vi.fn(async () => ({ role: 'student', llm: {} })),
}));
vi.mock('@/lib/platform/memory/teaching-context', () => ({
  loadRuntimeTeachingMemory: vi.fn(async () => ({ status: 'disabled', instructions: '' })),
}));
