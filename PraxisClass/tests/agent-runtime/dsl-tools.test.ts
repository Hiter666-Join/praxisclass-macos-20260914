...
import type { PPTTextElement } from '@praxis/dsl';
import { ensureDocumentSchema } from '@praxis/storage/document/pg';
import {
...
describe('read_stage', () => {
  it('reads raw HTML at a reported line and uses unescaped anchors with imported scene ids', async () => {
    const scene = interactiveScene();
    scene.id = 'JRSF5lfRg1HVc-zQjk7W5';
    (scene.content as InteractiveContent).html = '<style>/* unrelated layout */</style>\n<script>\nconst match = /\\s+/;\n</script>';
    const current = state(course([scene]));
    const tools = dslTools(current.store);
    const read = await tool(tools, 'read_stage').execute('raw', { path: `/scenes/${scene.id}/content/html`, line: 3 });
    expect(textOf(read)).toContain('const match = /\\s+/;');
    expect(textOf(read)).not.toContain('unrelated layout');
    expect(textOf(read)).not.toContain('\\\\s');
    const patched = await tool(tools, 'patch_stage').execute('fix', { target: `/scenes/${scene.id}`, intent: 'Change the exact raw fragment', ops: [{ op: 'str_replace', path: '/content/html', oldText: 'const match = /\\s+/;', newText: 'const match = /\\w+/;' }] });
    expect(patched).not.toMatchObject({ isError: true });
    expect((current.get().scenes[0].content as InteractiveContent).html).toContain('/\\w+/');
    expect(await tool(tools, 'read_stage').execute('ambiguous', { path: '/scenes/3/content/html', line: 1, offset: 1 })).toMatchObject({ isError: true });
  });
...
describe('patch_stage', () => {
  it('returns a saved script failure to the agent and accepts its local correction', async () => {
    const current = state(course([interactiveScene()]));
    const patch = tool(dslTools(current.store), 'patch_stage');
    const invalidHtml = '<p>Keep the learning goal</p>\n<script>\nconst passed = ;\n</script>';
    const failed = await patch.execute('bad-edit', {
      target: '/scenes/scene_widget', intent: 'Update check behavior',
      ops: [{ op: 'set', path: '/content/html', value: invalidHtml }],
    });
    expect(failed).toMatchObject({ isError: true, details: { draftSaved: true, sceneId: 'scene_widget' } });
    expect(textOf(failed)).toContain('HTML line 3');
    expect(textOf(failed)).toContain('const passed = ;');
    expect((current.get().scenes[0].content as InteractiveContent).html).toBe(invalidHtml);

    const fixed = await patch.execute('local-fix', {
      target: '/scenes/scene_widget', intent: 'Correct the reported initializer',
      ops: [{ op: 'str_replace', path: '/content/html', oldText: 'const passed = ;', newText: 'const passed = false;' }],
    });
    expect(fixed).not.toMatchObject({ isError: true });
    expect((current.get().scenes[0].content as InteractiveContent).html)
      .toBe(invalidHtml.replace('const passed = ;', 'const passed = false;'));
    expect(current.putScene).toHaveBeenCalledTimes(2);
  });

  it('returns the requested quiz count mismatch in model-visible tool text', async () => {
    const document = course([quizScene()]);
    (document.outline as { outlines: Array<Record<string, unknown>> }).outlines[0].quizConfig = {
      questionCount: 2, difficulty: 'easy', questionTypes: ['single'],
    };
    const current = state(document);
    const result = await tool(dslTools(current.store), 'patch_stage').execute('still-incomplete', {
      target: '/scenes/scene_quiz', intent: 'Refine the question',
      ops: [{ op: 'set', path: '/content/questions/0/question', value: 'Choose one answer' }],
    });
    expect(result).toMatchObject({ isError: true, details: { draftSaved: true } });
    expect(textOf(result)).toContain('应有 2 题，实际 1 题');
  });
...
describe('DSL course-tool wiring', () => {
  ...
  it('registers generation and DSL tools while excluding legacy editors', () => {
    ...
    expect(tool(tools, 'test_interactive_scene')).toMatchObject({ executionMode: 'sequential' });
    expect(buildCourseAllowlist()).toContain('test_interactive_scene');
