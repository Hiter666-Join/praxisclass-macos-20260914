...
describe('web_search abort handling', () => {
  ...
  it('passes the resolved source selection to the provider', async () => {
    const subSources = { webSearch: false, baike: false, scholar: true };
    mocked.mockReturnValue({ providerId: 'baidu', apiKey: 'test', baiduSubSources: subSources });
    searchWebMock.mockResolvedValue({ sources: [] });
    await buildWebSearchTool(resolveWebSearchCapability()!).execute(
      'call_options',
      { query: 'q' } as never,
      undefined,
    );
    expect(searchWebMock).toHaveBeenCalledWith(
      expect.objectContaining({ baiduSubSources: subSources }),
    );
  });
