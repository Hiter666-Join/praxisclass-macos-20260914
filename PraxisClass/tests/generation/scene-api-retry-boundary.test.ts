...
vi.mock('@praxis/generation', async (importOriginal) => ({
  ...(await importOriginal<typeof import('@praxis/generation')>()),
...
describe('scene API retry boundary', () => {
  ...
  it('preserves the provider status carried by a PBLGenerationError', async () => {
    ...
    const { PBLGenerationError } = await import('@praxis/generation');
