...
const reviewPlanMock = vi.hoisted(() => vi.fn(async (outlines) => ({ outlines, coverage: [], changedPages: 0, elapsedMs: 0 })));
const VOCATIONAL_FLAG = 'PRAXIS_ENABLE_VOCATIONAL';
...
// These cases exercise streaming/normalization. The goal checker has its own
// contract tests; keep the fixture's deliberately incomplete legacy outlines.
vi.mock('@/lib/server/generation-plan-review', () => ({ reviewGenerationPlan: reviewPlanMock }));
...
describe('task-engine outline route', () => {
  ...
  test('uses the task-engine prompt and preserves allowed mixed task-engine scene types', async () => {
    ...
    expect(reviewPlanMock).toHaveBeenCalledOnce();
  ...
  test('retains diagnosis before adaptive PBL and preserves their native configs', async () => {
    vi.resetModules();
    streamLLMMock.mockReset();
    process.env[VOCATIONAL_FLAG] = 'true';
    resolveModelFromRequestMock.mockResolvedValue({
      model: { provider: 'test.chat', modelId: 'test' },
      modelInfo: { outputWindow: 4096, capabilities: {} },
    ...
    const scenes = [
      ...
      },
      {
        ...
        widgetType: 'game',
        widgetOutline: { challenge: 'discard stale widget data' },
      },
      {
        id: 'project',
        type: 'pbl',
        title: '个性化接口验收',
        description: '根据真实诊断结果调整练习支持',
        keyPoints: ['测试设计'],
        order: 10,
        pblConfig: {
          projectTopic: '接口验收',
          projectDescription: '提交接口验收报告',
          targetSkills: ['测试设计'],
          issueCount: 3,
        },
        widgetType: 'procedural-skill',
        widgetOutline: { task: 'discard stale widget data' },
      },
    ];
    streamLLMMock.mockReturnValue({
      textStream: (async function* () {
    ...
    expect(done.outlines[1].quizConfig).toEqual(scenes[1].quizConfig);
    expect(done.outlines[2].pblConfig).toMatchObject(scenes[2].pblConfig!);
    for (const outline of done.outlines) {
      expect(outline.widgetType).toBeUndefined();
      expect(outline.widgetOutline).toBeUndefined();
    }
    expect(
      events.filter((event) => event.type === 'outline').map((event) => event.data.type),
    ).toEqual(['slide', 'quiz', 'pbl']);
  });
