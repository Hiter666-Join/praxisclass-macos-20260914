...
describe('thinking config normalization', () => {
  ...
  it.each(['gemini-3.7-flash', 'gemini-3.8-flash'])('normalizes unsupported minimal thinking for %s', (modelId) => {
    const thinking = getThinking('google', modelId);
    expect(thinking?.levelValues).toEqual(['low', 'medium', 'high']);
    expect(normalizeThinkingConfig(thinking, { mode: 'enabled', level: 'minimal' })).toEqual({
      mode: 'enabled', level: 'high',
    });
    expect(normalizeThinkingConfig(thinking, { mode: 'disabled' })).toEqual({
      mode: 'enabled', level: 'low',
    });
  });
