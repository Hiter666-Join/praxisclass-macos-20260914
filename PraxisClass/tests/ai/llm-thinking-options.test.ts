...
describe('LLM thinking provider options', () => {
  ...
  it.each(['gpt-6-astra', 'gpt-5.6'])('sends %s max reasoning effort through OpenAI provider options', async (modelId) => {
    await callLLM(
      {
        model: {
  ...
  it.each(['claude-fable-5', 'claude-fable-5-1'])('coerces disabled thinking to the lowest supported effort for %s', async (modelId) => {
    await callLLM(
      {
        model: {
          provider: 'anthropic.messages',
