...
import { useSettingsStore, resetCurrentSettingsProfile } from '@/lib/store/settings';
...
export function GeneralSettings() {
  ...
  return (
    <div className="flex flex-col gap-8">
      <section className="space-y-3 rounded-xl border p-4" aria-label="基础配置">
        <h3 className="text-sm font-semibold">{mode === 'teacher' ? '教师端' : '学生端'}基础配置</h3>
        <p className="text-sm text-muted-foreground">恢复源码默认设置：清空本端供应商凭据与模型选择，关闭扩展生成和语音功能。课程、测试记录和另一端设置保留。</p>
        {!confirmReset ? <Button variant="outline" onClick={() => setConfirmReset(true)}>恢复本端基础配置</Button> : <div className="flex gap-2">
          <Button disabled={resetting} onClick={async () => {
            setResetting(true);
            try { await resetCurrentSettingsProfile(); toast.success('本端已恢复基础配置'); setConfirmReset(false); }
            catch { toast.error('重置未完成，请重试'); }
            finally { setResetting(false); }
          }}>{resetting ? '正在重置…' : '确认重置本端设置'}</Button>
          <Button variant="outline" disabled={resetting} onClick={() => setConfirmReset(false)}>取消</Button>
        </div>}
      </section>
