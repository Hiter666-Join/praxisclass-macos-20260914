...
export function ProviderConfigPanel({
  ...
  const isXfyun = isBuiltIn && provider.id.startsWith('xfyun-');
  const isServerConfigured = providersConfig[provider.id]?.isServerConfigured;
  // Legacy managed metadata is cleared by the role-scoped provider refresh.
  const modelsLocked = providersConfig[provider.id]?.serverModels !== undefined;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      {!isServerConfigured && (
        <>
          ...
          <div className="space-y-2">
            <Label>{isXfyun ? 'APIPassword' : t('settings.apiSecret')}</Label>
            <div className="flex gap-2">
              <div className="relative flex-1">
                <Input
                  ...
                  autoCorrect="off"
                  spellCheck={false}
                  placeholder={isXfyun ? 'APIPassword' : 'sk-...'}
                  value={apiKey}
            ...
            {isXfyun && (
              <p className="text-xs text-muted-foreground">{t('settings.xfyunLLMCredentialsHint')}</p>
            )}
            {testMessage && (
              <div
                className={cn(
      ...
      <div className="space-y-3">
        {provider.id === 'azure' && (
          <div className="rounded-lg border border-border bg-muted/70 p-3 text-sm text-muted-foreground">
