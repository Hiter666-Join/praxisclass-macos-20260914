...
import { ProviderScopeNote } from './provider-scope-note';
...
import {
  encodeXfyunCredentials,
  splitXfyunCredentials,
  XFYUN_TTS_ENDPOINT,
} from '@/lib/audio/xfyun-config';
import {
...
export function TTSSettings({ selectedProviderId }: TTSSettingsProps) {
  ...
  const isXfyun = selectedProviderId === 'xfyun-tts';
  const [xfyunTestVoice, setXfyunTestVoice] = useState('xiaoyan');
  const xfyunCredentials = splitXfyunCredentials(providerConfig?.apiKey);
  const xfyunVoices = [
    ...(isXfyun ? ttsProvider?.voices || [] : []),
    ...(providerConfig?.customVoices || []),
  ...
  return (
    <div className={cn('space-y-6', isVoxCPM ? 'max-w-5xl' : 'max-w-3xl')}>
        ...
        (isXfyun ? (
          <div className="space-y-3">
            <div className="grid gap-4 sm:grid-cols-3">
              {(['appId', 'apiKey', 'apiSecret'] as const).map((field) => {
                const label = { appId: 'APPID', apiKey: 'APIKey', apiSecret: 'APISecret' }[field];
                return (
                  <div key={field} className="space-y-2">
                    <Label htmlFor={`tts-xfyun-${field}`} className="text-sm">
                      {label}
                    </Label>
                    <Input
                      id={`tts-xfyun-${field}`}
                      name={`tts-xfyun-${field}`}
            ...
            <Button
              ...
              variant="ghost"
              size="sm"
              aria-label={t('settings.xfyunToggleCredentials')}
              aria-pressed={showApiKey}
              onClick={() => setShowApiKey(!showApiKey)}
              className="gap-2"
            >
              {showApiKey ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
              {t('settings.xfyunToggleCredentials')}
            </Button>
            <p className="text-xs text-muted-foreground">{t('settings.xfyunCredentialsHint')}</p>
            <p className="break-all text-xs text-muted-foreground">
              {t('settings.requestUrl')}: {XFYUN_TTS_ENDPOINT}
            </p>
          </div>
        ) : isVoxCPM ? (
      ...
      <div className="space-y-2">
        ...
        {isXfyun && (
          <div className="space-y-2">
            <Label htmlFor="tts-xfyun-test-voice" className="text-sm">
              {t('settings.xfyunTestVoice')}
            </Label>
            <Select value={xfyunTestVoice} onValueChange={setXfyunTestVoice}>
              <SelectTrigger id="tts-xfyun-test-voice">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {xfyunVoices.map((voice) => (
            ...
            <p className="text-xs text-muted-foreground">{t('settings.xfyunVoiceHint')}</p>
          </div>
        )}
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={
              testingTTS ||
              !testText.trim() ||
              (isXfyun && (!providerConfigured || providerServerDisabled)) ||
              (requiresApiKey &&
        ...
        <div className="space-y-3">
          ...
          {(providerConfig?.customVoices as Array<{ id: string; name: string }> | undefined)
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (isXfyun && xfyunTestVoice === voice.id) setXfyunTestVoice('xiaoyan');
                    }}
            ...
            <p className="text-sm text-muted-foreground/50 italic">
              {isXfyun ? t('settings.xfyunVoiceHint') : t('settings.noVoicesAdded')}
            </p>
          )}
          <AddVoiceRow
            idPlaceholder={isXfyun ? 'vcn (x4_xiaoyan)' : undefined}
            existingIds={(
              (providerConfig?.customVoices as Array<{ id: string; name: string }> | undefined) ||
              []
            )
              .map((v) => v.id)
              .concat(isXfyun ? (ttsProvider?.voices || []).map((v) => v.id) : [])}
            onAdd={(voiceId, voiceName) => {
              ...
              if (isXfyun) setXfyunTestVoice(voiceId);
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr_auto] gap-2 items-center">
      <Input
        ...
        placeholder={idPlaceholder || t('settings.voiceIdPlaceholder')}
      />
      <Input
        value={voiceName}
