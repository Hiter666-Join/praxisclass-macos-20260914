...
export function GenerationToolbar({
  ...
  const webControl = webSearchAvailable ? (
    <Popover>
      <PopoverTrigger asChild>
  ...
  if (composer) {
    return (
      <div className="home-focus-composer">
        <div data-pro-morph="composer" className="home-focus-input-card">
          {composer.content}
          <div className="home-focus-actions">
            <div className="home-focus-material">{materialControl}</div>
        ...
        <div className="home-focus-dock">
          <Popover>
            <PopoverTrigger asChild>
              <button type="button" className="home-focus-tool">
                <Puzzle className="size-4" />
                <span>{t('home.focus.extensions')}</span>
            ...
            <PopoverContent
              ...
              sideOffset={10}
              collisionPadding={12}
              className="w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl p-4"
            >
              <h2 className="text-sm font-semibold">{t('home.focus.extensions')}</h2>
              <p className="mt-1 text-xs leading-5 text-muted-foreground">
                {t('home.focus.extensionsHint')}
              </p>
              <div className="mt-3 divide-y divide-border/60">
                <div className="flex min-h-14 items-center justify-between gap-3 py-3">
                  <span className="text-sm">{t('home.focus.webTools')}</span>
                  {webControl}
                </div>
                <div className="flex min-h-14 flex-wrap items-center justify-between gap-3 py-3">
                  <span className="text-sm">{t('home.focus.mediaTools')}</span>
                  <MediaPopover onSettingsOpen={onSettingsOpen} />
                </div>
                {composer.tools && (
                  <div className="flex min-h-14 items-center justify-between gap-3 pt-3">
                    <span className="text-sm">{t('home.focus.practiceTools')}</span>
                    {composer.tools}
                  </div>
                )}
              </div>
          ...
          {composer.context && <div className="home-focus-context">{composer.context}</div>}
        </div>
      </div>
    );
  }
          ...
          <PopoverTrigger asChild>
            <button
              ...
              className={cn(
                ...
                showModelName
                  ? 'h-9 border-transparent bg-transparent text-foreground hover:bg-muted'
            ...
            >
              ...
              {showModelName && (
                <span className="max-w-[88px] truncate text-xs sm:max-w-[180px] sm:text-[13px]">
                  {currentModelLabel}
                </span>
              )}
              ...
              {showModelName && <ChevronDown className="size-3.5 shrink-0 text-muted-foreground" />}
            </button>
          </PopoverTrigger>
        </TooltipTrigger>
        <TooltipContent>
