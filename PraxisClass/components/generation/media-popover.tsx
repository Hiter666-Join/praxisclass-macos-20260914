...
import { activateService, currentServiceProvider, disableService } from '@/lib/settings/service-activation';
...
export function MediaPopover({ onSettingsOpen }: MediaPopoverProps) {
  ...
  const toggleService = (section: TabId, enabled: boolean) => {
    const state = useSettingsStore.getState();
    if (!enabled) { disableService(state, section); return; }
    try {
      activateService(state, section, currentServiceProvider(state, section),
        section === 'image' ? state.imageModelId : section === 'video' ? state.videoModelId : undefined);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : '请先完成服务配置。');
      setOpen(false);
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <button
          type="button"
          aria-label={t('home.focus.mediaTools')}
          className={cn(
      ...
      <PopoverContent
        ...
        side="bottom"
        collisionPadding={12}
        className="max-h-[min(32rem,80dvh)] w-80 max-w-[calc(100vw-2rem)] overflow-y-auto p-0"
      >
        ...
        <div className="p-3 pt-2.5">
          {activeTab === 'image' && (
            <TabPanel
              ...
              onToggle={enabled => toggleService('image', enabled)}
            >
              <GroupedSelect
                groups={imageGroups}
                ...
                onSelect={(gid, iid) => {
                  ...
                  setImageModelId(iid);
                  if (imageGenerationEnabled) toggleService('image', true);
                }}
              />
            </TabPanel>
          )}
          ...
          {activeTab === 'video' && (
            <TabPanel
              ...
              onToggle={enabled => toggleService('video', enabled)}
            >
              <GroupedSelect
                groups={videoGroups}
                ...
                onSelect={(gid, iid) => {
                  ...
                  setVideoModelId(iid);
                  if (videoGenerationEnabled) toggleService('video', true);
                }}
              />
            </TabPanel>
          )}
          ...
          {activeTab === 'tts' && (
            <TabPanel
              ...
              enabled={ttsEnabled}
              onToggle={enabled => toggleService('tts', enabled)}
            />
          )}
          ...
          {activeTab === 'asr' && (
            <TabPanel
              ...
              enabled={asrEnabled}
              onToggle={enabled => toggleService('asr', enabled)}
            >
              <GroupedSelect
                groups={asrGroups}
                ...
                onSelect={(gid, iid) => {
                  ...
                  setASRLanguage(iid);
                  if (asrEnabled) toggleService('asr', true);
                }}
              />
            </TabPanel>
          )}
