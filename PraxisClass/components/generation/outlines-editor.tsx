...
import { changeOutlineType } from '@praxis/generation';
import { countBlockingOutlines, validateOutline } from '@/lib/edit/content-validation';
import { parseTaskcard } from '@/components/generation/taskcard-view';

type SceneType = SceneOutline['type'];

/** Hides the ```taskcard block from the editable description; `fence` is re-appended verbatim on save. */
function splitTaskcardFence(description: string): { rest: string; fence: string | null } {
  const { rest, fence } = parseTaskcard(description);
  return { rest, fence };
}
...
export function OutlinesEditor({
  ...
  return (
    <motion.div
      ...
      initial={{ rotate: 0 }}
      animate={{ rotate: 0 }}
      className="workspace-panel relative overflow-hidden"
    >
      {/* Header */}
      <div className="relative flex items-start gap-3 px-6 pt-6 pb-4 md:px-10 md:pt-8 md:pb-6">
        ...
        {onCollapse && (
          <button
            ...
            className={cn(
              'mt-1 inline-flex min-h-10 shrink-0 items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium',
      ...
      {/* Scene list */}
      <div className="relative px-3 pb-2 md:max-h-[64vh] md:overflow-y-auto md:px-6">
        {outlines.length === 0 ? (
      ...
      {/* Footer */}
      <div className="relative flex flex-col gap-4 border-t border-border bg-card px-6 py-4 md:flex-row md:items-center md:justify-between md:px-10 md:py-5">
        <label
          className={cn(
...
function SceneRow({
  ...
  const { rest: descriptionRest, fence: descriptionFence } = splitTaskcardFence(
    outline.description,
  );

  // Auto-resize textareas to content for the typography-first feel.
  useAutoResize(titleRef, outline.title);
  useAutoResize(descRef, descriptionRest);
  ...
  return (
    <motion.li
      ...
      className={cn(
        ...
        'hover:bg-muted/40 focus-within:bg-muted/60',
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex shrink-0 flex-col items-center gap-1 pt-1 sm:flex-row sm:gap-0.5">
          <button
            type="button"
            draggable={!disabled}
            ...
            disabled={disabled}
            className={cn(
              'flex size-8 shrink-0 cursor-grab items-center justify-center rounded-md',
        ...
        <div className="min-w-0 flex-1 space-y-1.5">
          ...
          <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-start sm:justify-between">
            ...
            <textarea
              ...
              aria-label={t('generation.sceneTitlePlaceholder')}
              aria-invalid={!disabled && !outline.title.trim()}
              disabled={disabled}
              rows={1}
              spellCheck={false}
              className={cn(
                'w-full min-w-0 resize-none rounded-md border-none bg-transparent p-1 text-base font-semibold leading-7 tracking-tight sm:w-auto sm:flex-1',
                'placeholder:font-normal placeholder:text-muted-foreground',
                'focus:outline-none focus-visible:ring-2 focus-visible:ring-ring md:text-lg',
          ...
          <textarea
            ...
            onChange={(event) =>
              onUpdate({
                description: descriptionFence
                  ? `${event.target.value}\n\n${descriptionFence}`
                  : event.target.value,
              })
            }
            placeholder={t('generation.sceneDescriptionPlaceholder')}
            aria-label={t('generation.sceneDescriptionPlaceholder')}
            disabled={disabled}
            rows={1}
            className={cn(
              'block w-full resize-none rounded-md border-none bg-transparent p-1 text-sm leading-7 text-muted-foreground',
              'placeholder:text-muted-foreground',
              'focus:outline-none focus-visible:ring-2 focus-visible:ring-ring focus:text-foreground',
              disabled && 'cursor-default',
            )}
          />
                ...
                >
                  ...
                  {!disabled && (
                    <button
                      ...
                      className="ml-0.5 inline-flex size-6 shrink-0 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted-foreground/20 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring"
                    >
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild disabled={disabled}>
        <button
          type="button"
          className={cn(
            'inline-flex min-h-9 items-center gap-1 px-2.5 py-1 text-xs font-medium transition-colors focus-visible:ring-2 focus-visible:ring-ring',
...
function useAutoResize(ref: React.RefObject<HTMLTextAreaElement | null>, value: string) {
  useEffect(() => {
    ...
    resize();
    // The same text needs more lines after a narrow viewport or sidebar change.
    // Observe width only so our own height update does not schedule a loop.
