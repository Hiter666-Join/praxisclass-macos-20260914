 ...
 *   persisted to IndexedDB by the PraxisClass stage layer. The Modal's
    ...
    try {
      ...
      const runStream = async (
        ...
        const res = await fetch(endpoint, {
          method: 'POST',
          headers,
          body: JSON.stringify({ ...body, thinkingConfig: modelConfig.thinkingConfig }),
        });
...
function SubmissionModal({
  ...
  const handleFile = async (file: File) => {
    ...
    abortRef.current = ac;
    // Image / screenshot: store via PraxisClass's existing object storage
    ...
    // PDF: parse to text via PraxisClass's existing /api/parse-pdf (reused, not
