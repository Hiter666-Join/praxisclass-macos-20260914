...
export function PBLV2Chat({
  ...
  const { t } = useI18n();
  const isStudent = useSettingsStore((state) => state.mode) === 'student';
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const response = await fetch('/api/platform/memory?scope=learner', {
          headers: { 'x-learner-key': learnerKey },
          signal: AbortSignal.timeout(5000),
        });
        if (response.ok) {
          const stored = (await response.json()) as { profile?: string; memory?: string };
          const userBio = [stored.profile, stored.memory]
            .filter(Boolean)
            .join('\n\n')
            .slice(0, 4096);
          if (userBio) body.userBio = userBio;
        }
      } catch {
        // Learner memory is optional context; a failure must not block the greeting.
      }
      await run({ endpoint: '/api/pbl/v2/open-task', body, initialProject });
    })();
  }, [
    project.uiPhase,
    project.createdAt,
 ...
 *  decoupled from the PraxisClass classroom teacher avatar; changing one
