...
export function QuizView({ questions, sceneId, stageId }: QuizViewProps) {
  ...
  const handleRetry = useCallback(async () => {
    ...
    let retryAttemptId = attemptId;
    await runQuizPersistenceTransition(
      async () => {
        retryAttemptId = await persistQuizRetry({ stageId, sceneId, attemptId }, runtimeWriter);
      },
      viewLifetime,
      () => {
        setRuntimeGate({ status: 'ready', attemptId: retryAttemptId });
