...
function PBLV2Container({
  ...
  // live inside whatever element is natively fullscreened — PraxisClass fullscreens
  ...
  // is on. NOTE: this never reads/writes PraxisClass code — it only chooses this
  ...
  // True while the browser is in native (OS) fullscreen — PraxisClass enters it
  // on the stage via `stageRef.requestFullscreen()`. In that mode PraxisClass's
    ...
    <div ref={hostRef} className="h-full w-full">
      ...
      {(runtimeProject.uiPhase === 'workspace' || runtimeProject.uiPhase === 'completed') &&
        ...
        createPortal(
          ...
          // launch), so PraxisClass's native fullscreen still shows the workspace.
  ...
  return (
    <>
      ...
      <motion.div
        ...
        transition={transition}
      >
        {/* Native (OS) fullscreen takes over the top-right slot: PraxisClass's
            own exit bar is hidden for PBL, so this is the only click target
            to leave true fullscreen (Esc still works, handled by PraxisClass).
            `document.exitFullscreen()` triggers PraxisClass's own fullscreenchange
            cleanup (keyboard unlock etc.) — we never touch PraxisClass code. The
            violet, borderless-ish treatment mirrors PraxisClass's native control
