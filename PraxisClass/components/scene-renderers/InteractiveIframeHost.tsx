...
import { createTrainingInteractiveBridge } from '@/lib/training/interactive-bridge';
import {
...
function PooledIframe({ sceneId, entry, visible }: PooledIframeProps) {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const bridge = stageId ? createTrainingInteractiveBridge({
      stageId, sceneId,
      post: (data) => iframeRef.current?.contentWindow?.postMessage(data, '*'),
      isCurrent: () => mounted && visible && useStageStore.getState().stage?.id === stageId,
    }) : null;
    const onMessage = (e: MessageEvent) => {
      if (e.source !== iframeRef.current?.contentWindow) return;
      if (e.data?.type === 'praxis:training' && visible) {
        void bridge?.(e.data);
        return;
      }
      if (e.data?.type === 'praxis:code-test-results' && visible) {
        const stageId = useStageStore.getState().stage?.id;
        if (!stageId) return;
        void getLearnerKey()
          .then((learnerKey) => {
            if (useStageStore.getState().stage?.id !== stageId) return;
            mirrorMany(parseInteractiveResults(e.data, { stageId, sceneId, learnerKey }));
          })
          .catch(() => {});
        return;
      }
    ...
    if (visible) iframeRef.current?.contentWindow?.postMessage({ type: 'praxis:training-request-ready' }, '*');
  ...
  }, [sceneId, entry.srcDoc, visible, t]);
  ...
  // Reading mode changes only the iframe's CSS viewport. Its source, DOM and
  // in-progress work stay mounted, and fixed-layout lessons can return to fit.
  const showReadingControl = !!rect && (rect.width < 640 || readingMode);
  const contentSlot =
    rect && showReadingControl
      ? { ...rect, top: rect.top + 40, height: Math.max(0, rect.height - 40) }
      : rect;
  const viewport = contentSlot
    ? readingMode
      ? { box: contentSlot, scale: 1 }
      : fitGenUiViewport(contentSlot)
    : null;
  ...
  const iframeStyle: CSSProperties = {
    ...
    width: readingMode ? contentSlot?.width : GENUI_LOGICAL_WIDTH,
    height: readingMode ? contentSlot?.height : GENUI_LOGICAL_HEIGHT,
    border: 0,
  ...
  return (
    <>
      {showReadingControl && shown && rect && (
        <div
          className="fixed z-[2] flex h-10 items-center justify-end bg-card px-2"
        ...
        >
          ...
          >
            {t(readingMode ? 'platform.design.originalLayout' : 'platform.design.readingMode')}
          </Button>
        </div>
      )}
      <div style={wrapStyle}>
        <iframe
          ...
          aria-label={t('platform.design.interactiveLesson')}
