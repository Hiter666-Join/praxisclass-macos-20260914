    ...
    <div className="flex items-center gap-2" onClick={onExpand}>
      <div
        className={cn(
          ...
          bubble.role === 'user'
            ? 'bg-accent/90 border-primary/30'
            : bubble.role === 'agent'
              ? 'bg-(--classroom-tone-lecture-soft)/90 border-(--classroom-tone-lecture)/30'
        ...
        <div
          className={cn(
            ...
            bubble.role === 'user'
              ? 'border-primary/60'
              : bubble.role === 'agent'
                ? 'border-(--classroom-tone-lecture)/60'
                : 'border-border',
          )}
        >
      ...
      {onPlayPause && (
        <div
          ...
          className={cn(
            ...
            'hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]',
            bubble.role === 'user'
              ? 'bg-accent/90 border-primary/30 hover:bg-accent'
              : bubble.role === 'agent'
                ? 'bg-(--classroom-tone-lecture-soft)/90 border-(--classroom-tone-lecture)/30 hover:bg-(--classroom-tone-lecture-soft)'
                : 'bg-card/85 border-border hover:bg-muted',
          )}
        >
          {isPaused ? (
    ...
    <div
      ...
      className={cn(
        ...
        bubble.role === 'user'
          ...
          : bubble.role === 'agent'
            ? 'bg-(--classroom-tone-lecture-soft)/80 border-(--classroom-tone-lecture)/30'
            : 'bg-card/85 border-border',
      )}
    >
      <div className="flex items-center gap-3 px-4 pt-3 pb-2">
        ...
        {onCollapse && (
          <div
            ...
            className="absolute top-2 right-2 p-1.5 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted transition-colors duration-200 cursor-pointer z-10"
          >
      ...
      {bubble.role !== 'user' &&
        ...
        (() => {
          const barsColor =
            bubble.role === 'agent' ? 'var(--classroom-tone-lecture)' : 'var(--primary)';
          ...
          if (buttonState === 'play') {
            return (
              <div
                ...
                className="absolute right-2.5 bottom-2.5 z-20 p-1.5 rounded-full bg-card/60 backdrop-blur-sm group-hover/bubble:bg-accent transition-all duration-300 cursor-pointer"
              >
                <Play className="w-3.5 h-3.5 text-muted-foreground group-hover/bubble:text-primary ml-0.5" />
              </div>
            );
          }
          ...
          if (buttonState === 'restart') {
            return (
              <div
                ...
                className="absolute right-2.5 bottom-2.5 z-20 p-1.5 rounded-full bg-card/60 backdrop-blur-sm group-hover/bubble:bg-accent transition-all duration-300 cursor-pointer"
              >
                <Repeat className="w-3.5 h-3.5 text-muted-foreground group-hover/bubble:text-primary" />
              </div>
            );
          }
          ...
          return (
            ...
            >
              {isPaused ? (
                <Play className="w-3.5 h-3.5 text-amber-500 dark:text-amber-400 group-hover/bubble:text-primary ml-0.5" />
              ) : (
                <>
                  ...
                  {/* Pause icon on hover */}
                  <Pause className="w-3.5 h-3.5 text-primary hidden group-hover/bubble:block" />
                </>
              )}
            </div>
          );
