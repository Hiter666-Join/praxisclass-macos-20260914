...
export function AgentBar() {
  ...
  const renderAgentRow = (agent: AgentConfig, agentIndex: number, isTeacher: boolean) => {
    ...
    return (
      <div
        ...
        className={cn(
          'grid w-full grid-cols-[1.25rem_1.75rem_minmax(0,1fr)] items-center gap-x-2 gap-y-1.5 rounded-lg px-2.5 py-2.5 transition-colors sm:flex sm:gap-2 sm:py-1.5',
      ...
      >
        <Checkbox
          checked={isSelected}
          disabled={isTeacher}
          aria-label={getAgentName(agent)}
          onClick={(event) => event.stopPropagation()}
          onCheckedChange={() => {
            if (!isTeacher) toggleAgent(agent.id);
          }}
          className={cn('row-span-2', isTeacher && 'opacity-50')}
        />
        <div
          className="row-span-2 size-7 rounded-full overflow-hidden shrink-0 ring-1 ring-border/40"
        ...
        <div className="col-start-3 flex shrink-0 items-center gap-2">
          <span className="shrink-0 text-[11px] text-muted-foreground sm:w-[52px] sm:text-right">
            {getAgentRole(agent)}
          </span>
          <AgentVoicePill
            agent={agent}
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full max-w-full sm:w-96"
      onKeyDown={(event) => {
    ...
    >
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <span className="min-w-0 flex-1 truncate text-left text-xs font-medium text-muted-foreground transition-colors">
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-1 z-50 w-full sm:w-96"
          >
