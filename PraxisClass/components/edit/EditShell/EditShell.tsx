...
interface EditShellProps {
  ...
  /** Use the compact save bar inside the workbench panel. */
 ...
 * CanvasArea. Its command bar owns the explicit save and exit controls; the
 * compact hosted variant keeps those controls available in the workbench.
 *
 *   ┌──────────────────────────────────────────────┐  (Stage Header above)
 *   ├──────────────────────────────────────────────┤
...
function Frame({
  ...
  return (
    <StageGrid
      ...
      topSlot={
        hideCommandBar ? <div className="flex flex-wrap justify-end gap-2 border-b p-2">{trailing}</div> : (
      ...
      centerSlot={
        ...
        >
          ...
          {usesClassroomAspect(sceneType) ? (
            <ContainBox
              fit="contain"
              className="overflow-hidden rounded-xl bg-white ring-1 ring-border dark:bg-zinc-900 shadow-sm"
            >
              <div
            ...
            <div
              data-maic-edit-canvas="true"
              data-maic-stage-card="true"
              className="relative h-full w-full overflow-hidden rounded-xl bg-white ring-1 ring-border dark:bg-zinc-900 shadow-sm"
            >
              {children}
            </div>
          )}
