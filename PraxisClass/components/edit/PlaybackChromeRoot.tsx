...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot(
    ...
    const handleChatAreaChange = useCallback(
      (collapsed: boolean) => {
    ...
    // Completion card → run the course again from its first page. Goes through
    // the same gate as any other jump, so an open topic still gets its prompt.
    ...
    useEffect(() => {
      const onKeyDown = (event: KeyboardEvent) => {
        ...
        switch (event.key) {
          ...
          case ' ':
          case 'Spacebar':
            // Let Space activate the focused control instead of also starting
            // the lesson (for example when a learner opens the question box).
            if (document.activeElement?.closest('button, a[href], [role="button"], [role="link"]'))
              return;
          ...
          case 'S':
            event.preventDefault();
            handleSidebarChange(!sidebarCollapsed);
            break;
          case 'c':
          case 'C':
            event.preventDefault();
            handleChatAreaChange(!chatAreaCollapsed);
            break;
          default:
            break;
        }
    ...
    return (
      <div
        ref={stageRef}
        className={cn(
          'relative flex-1 flex overflow-hidden bg-background',
        ...
        onKeyDownCapture={(event) => {
          if (
            ...
            (!sidebarCollapsed || !chatAreaCollapsed)
          ) {
            event.preventDefault();
            setSidebarCollapsed(true);
            setChatAreaCollapsed(true);
      ...
      >
        ...
        <div className="flex shrink-0 max-lg:absolute max-lg:inset-y-0 max-lg:left-0 max-lg:z-30">
          <SceneSidebar
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 relative">
          ...
          <div className="overflow-hidden relative flex-1 min-h-0 isolate">
            <CanvasArea
              ...
              onToggleSidebar={() => handleSidebarChange(!sidebarCollapsed)}
              onToggleChat={() => handleChatAreaChange(!chatAreaCollapsed)}
              ...
              isCourseComplete={isCourseComplete}
              onReplayCourse={handleReplayCourse}
              isGenerationFailed={
          ...
          {mode === 'playback' && (
            <div
              className={cn(
                ...
                !isPresenting && 'shrink-0 px-3 pb-3 sm:px-4 sm:pb-4',
        ...
        <div className="flex shrink-0 max-lg:absolute max-lg:inset-y-0 max-lg:right-0 max-lg:z-30">
          <ChatArea
            ref={chatAreaRef}
            width={chatAreaWidth}
            onWidthChange={setChatAreaWidth}
            collapsed={chatAreaCollapsed}
            onCollapseChange={handleChatAreaChange}
