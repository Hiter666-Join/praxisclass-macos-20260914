...
import { ClassroomSaveControls } from '@/components/edit/ClassroomSaveControls';
...
import { isPraxisEditorEnabled } from '@/lib/config/feature-flags';
 ...
 */
export function EditChromeRoot({ scene, isEditable, onToggleEditMode, onExitEdit }: EditChromeRootProps) {
  ...
  useEffect(() => {
    document.body.dataset.praxisEditor = 'true';
    return () => {
      delete document.body.dataset.praxisEditor;
    };
  }, []);
  ...
  const headerControls = inWorkbenchPanel ? undefined : (
    <HeaderControls
      mode="edit"
      canEdit={isEditable}
      onToggleEditMode={isPraxisEditorEnabled() && !inWorkbenchPanel ? onToggleEditMode : undefined}
    />
  );
  ...
  return (
    <EditShell
      ...
      commandTrailing={<><ClassroomSaveControls onExit={onExitEdit} />{headerControls}</>}
