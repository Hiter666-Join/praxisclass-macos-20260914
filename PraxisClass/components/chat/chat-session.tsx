      ...
      <div className="flex gap-1.5 items-center py-1.5 px-1">
        <span
          className={cn(
            ...
            isTeacher ? 'bg-muted-foreground/60' : 'bg-(--classroom-tone-lecture)/70',
          )}
        />
        <span
          className={cn(
    ...
    <div
      className={cn(
        ...
        isUser
          ...
          : isTeacher
            ? 'bg-card text-foreground border border-border/70 rounded-tl-sm shadow-sm'
            : 'bg-(--classroom-tone-lecture-soft) text-foreground border border-(--classroom-tone-lecture)/20 rounded-tl-sm',
      )}
    >
  ...
  if (session.messages.length === 0 && !isActive) {
    return (
      <div className="h-20 flex items-center justify-center text-center px-2">
        <p className="text-[11px] text-muted-foreground">{t('chat.noMessages')}</p>
      </div>
    );
  }
          ...
          return (
            <motion.div
              ...
              animate={
                isActiveBubble
                  ? {
                      ...
                      y: 0,
                      boxShadow: [
                        '0 0 0 0 rgba(198, 79, 60, 0)',
                        '0 0 20px 0 rgba(198, 79, 60, 0.18)',
                        '0 0 8px 0 rgba(198, 79, 60, 0.08)',
                      ],
                    }
                  : {
                      opacity: 1,
                      y: 0,
                      boxShadow: '0 0 0 0 rgba(198, 79, 60, 0)',
                    }
              }
              transition={
                isActiveBubble
              ...
              className={cn(
                ...
                isActiveBubble && 'border-l-primary bg-accent/50',
              )}
            >
              {/* Mini Avatar */}
              <div className="w-5 h-5 rounded-full overflow-hidden bg-muted shrink-0 mt-0.5 ring-1 ring-border/60">
        ...
        {canEnd && onEndSession && (
          ...
          >
            <button
              ...
              className="h-7 bg-destructive/10 text-destructive border border-destructive/30 px-2.5 rounded-md text-[11px] font-semibold flex items-center gap-1.5 transition-colors hover:bg-destructive/20"
            >
            ...
            {isSoftClosing && onContinueSession && (
              <button
                ...
                className="h-7 bg-card text-primary border border-primary/40 px-2.5 rounded-md text-[11px] font-semibold flex items-center gap-1.5 transition-colors hover:bg-accent"
              >
