...
import { isPraxisEditorEnabled, isProWorkbenchEnabled } from '@/lib/config/feature-flags';
 ...
 * published `@praxis/dsl` and is persisted with the stage, whereas "this is
...
export function Stage({
  ...
  const editorEnabled = isPraxisEditorEnabled();
  ...
  const handleExitEdit = useCallback(() => {
    setMode('playback');
    if (hosted) useWorkbenchStore.getState().setPlaybackOn(true);
  }, [hosted, setMode]);

  const handleToggleEditMode = useCallback(async () => {
    if (mode === 'edit') {
      const { saveBeforeLeaving } = await import('@/lib/edit/classroom-save');
      await saveBeforeLeaving(handleExitEdit);
      return;
    }
