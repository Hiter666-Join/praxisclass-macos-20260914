# @praxis/render-service

Isolated MP4 render service for PraxisClass's classroom video export (issue #866).
...
| `HF_STATIC_DEDUP`                        | `false` (set in Compose)    | Temporary PraxisClass-export workaround: these long slide compositions currently exhaust producer's 15-second verification budget and disable dedup anyway. Skipping the doomed verification removes the fixed startup cost without changing frames.                                                                                                                   |
...
Client identity for the per-user guard is taken from the `x-praxis-client`
