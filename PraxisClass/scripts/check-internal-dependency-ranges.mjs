...
const PRAXIS_PACKAGES = ['dsl', 'generation', 'storage', 'renderer', 'editor', 'importer'];

const INTERNAL_DEPENDENTS = {
  generation: ['@praxis/dsl'],
  storage: ['@praxis/dsl'],
  renderer: ['@praxis/dsl'],
  editor: ['@praxis/dsl', '@praxis/renderer'],
  importer: ['@praxis/dsl'],
};

const repositoryRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const packagesDirectory = join(repositoryRoot, 'packages/@praxis');

function readManifest(name) {
  return JSON.parse(readFileSync(join(packagesDirectory, name, 'package.json'), 'utf8'));
}

function assertPackageListIsComplete() {
  const onDisk = readdirSync(packagesDirectory, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort();
  const listed = [...PRAXIS_PACKAGES].sort();
  const problems = [];

  for (const name of onDisk) {
    if (!listed.includes(name)) {
      problems.push(`packages/@praxis/${name} exists but is not in PRAXIS_PACKAGES.`);
    }
  }
  for (const name of listed) {
    if (!onDisk.includes(name)) {
      problems.push(`PRAXIS_PACKAGES lists ${name}, but packages/@praxis/${name} is gone.`);
    }
  }
...
/**
 * An owned @praxis package may be declared as a dependency of another exactly
 ...
 * two copies of `@praxis/dsl`. The dsl carries the schema, the validators and
 ...
 *
 * WHERE THIS RUNS. This source-level check runs in `ci.yml` so a pull request
 * reintroducing an exact pin fails at review time.
 *
 ...
 * public installation contract and is out of scope here.
 */

const OWNED = new Set(PRAXIS_PACKAGES.map((name) => `@praxis/${name}`));
...
if (listProblems.length > 0) {
  report('The shared @praxis package list has drifted:', listProblems);
}
...
for (const name of PRAXIS_PACKAGES) {
  ...
  for (const [dependency, range] of Object.entries(manifest.dependencies ?? {})) {
    ...
    if (range === 'workspace:^') {
      console.log(`@praxis/${name}: dependencies.${dependency} = ${range}`);
      continue;
    }
    failures.push(
      `@praxis/${name} declares dependencies."${dependency}" as ${JSON.stringify(range)}. ` +
  ...
  for (const field of FORBIDDEN_FIELDS) {
    for (const dependency of Object.keys(manifest[field] ?? {})) {
      if (!OWNED.has(dependency)) continue;
      failures.push(
        `@praxis/${name} declares ${field}."${dependency}". An owned @praxis package may ` +
...
for (const [name, expected] of Object.entries(INTERNAL_DEPENDENTS)) {
  ...
  if (observed === undefined) {
    failures.push(
      `@praxis/${name} no longer declares any owned @praxis dependency in \`dependencies\`; ` +
        `it was expected to depend on ${expectedDependencies.join(', ')}. If that is intended, update ` +
        'the in-file INTERNAL_DEPENDENTS map — this check must not go quiet on its own.',
    );
    continue;
  }
  ...
  if (
    ...
    failures.push(
      `@praxis/${name} declares owned dependencies (${observedDependencies.join(', ')}), but ` +
...
for (const [name, observed] of seen) {
  ...
  failures.push(
    `@praxis/${name} declares the owned dependency ${observed.join(', ')} but is absent from ` +
      'the in-file INTERNAL_DEPENDENTS map. Add it there.',
  );
}
...
for (const name of PRAXIS_PACKAGES) {
  for (const dependency of Object.keys(readManifest(name).devDependencies ?? {})) {
    if (!OWNED.has(dependency)) continue;
    failures.push(
      `@praxis/${name} declares devDependencies."${dependency}". An owned @praxis package ` +
...
if (failures.length > 0) {
  report('Internal @praxis dependency declarations are not in the required shape:', failures);
}
