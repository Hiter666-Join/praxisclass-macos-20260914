 ...
 *
 * The app loads the parser at runtime from `/vendor/praxis-importer/index.js` — a
 * gitignored build artifact that `scripts/sync-praxis-importer.mjs` copies into
...
const entry = path.join(root, 'public/vendor/praxis-importer/index.js');
...
try {
  ...
  console.error('[assert-vendor] It is loaded at runtime via /vendor/praxis-importer/index.js');
  console.error('[assert-vendor] and is produced by the postinstall sync step. To fix:');
  console.error(
    '[assert-vendor]   pnpm --filter @praxis/importer build && pnpm run sync:praxis-importer',
  );
